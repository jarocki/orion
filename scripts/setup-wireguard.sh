...
#
# Orion-X Phoenix Edition — Standalone WireGuard Setup
#
# Sets up a point-to-point WireGuard tunnel to a specific server.
# For P2P mesh networking, use `orionx-mesh` instead.
#
# Usage: setup-wireguard.sh [--server <endpoint>] [--pubkey <key>]
#
...
#
# @decision DEC-MESH-STANDALONE-001
# @title Retain standalone WireGuard setup alongside mesh
# @status accepted
# @rationale Not every scenario needs a full mesh. A simple point-to-point
#   WireGuard tunnel to a known server is useful for individual operators,
#   remote access, or connecting to infrastructure VPNs. Renamed from
#   setup-vpn.sh to setup-wireguard.sh for clarity.

set -euo pipefail

# NOTE: For P2P mesh networking with auto-discovery and health checking,
# use `orionx-mesh join` instead. This script is for standalone tunnels.
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --allowed-ips)
                ...
                ;;
            -h|--help)
                echo "Usage: setup-wireguard.sh [OPTIONS]"
                echo ""
                echo "Options:"
                echo "  --server <endpoint>   Server endpoint (IP:Port)"
                echo "  --pubkey <key>        Server public key"
                echo "  --client-ip <cidr>    Client IP address (e.g., 10.10.10.2/24)"
                echo "  --port <port>         Listen port (default: 51820)"
                echo "  --dns <servers>       DNS servers (default: 1.1.1.1,8.8.8.8)"
                echo "  --allowed-ips <ips>   Allowed IPs (default: 0.0.0.0/0, ::/0)"
                echo "  -h, --help            Show this help message"
                echo ""
                echo "For P2P mesh networking, use 'orionx-mesh join' instead."
                exit 0
                ;;
            *)
...
log "Starting WireGuard standalone setup for Orion-X Phoenix Edition v2.0.0"
...
# Function to get configuration inputs interactively
get_config_inputs() {
    ...
    # Optional: Get allowed IPs (only prompt if not provided via CLI)
...
echo ""
echo "========================================"
echo "WireGuard Standalone Setup Complete"
echo "========================================"
...
echo ""
echo "For mesh networking: orionx-mesh join"
echo "========================================"

log "WireGuard standalone setup completed successfully"

...
**Env:** Docker + QEMU + physical hardware | **Status:** Completed 2026-05-14
...
| W7-7 | Physical USB boot validation | Hardware | 5 | W7-3 | S | approve | DEFERRED 2026-05-14 — operator attestation step, not implementation. The `approve` gate per DEC-PHASE7-005 always reserved W7-7 as human-in-the-loop hardware validation that runs in parallel with the software track. Phase 7 closure does not require W7-7 to land first because (a) it is an out-of-band physical-USB attestation, not a CI-runnable slice; (b) the hybrid ISO (BIOS + UEFI) is already proven bootable in QEMU under both firmware paths via W7-3; (c) the goal-contract "physical USB hardware" item moves to a Phase 8 release-gate attestation (operator signs off Phase 7 has been validated on real hardware before tagging v2.0.0). The slice remains available for the operator to execute at any point; its acceptance does not block Phase 7 closure or Phase 8 release prep. |
| W7-8 | Phase 7 closure + Phase 8 activation | Repo | 6 | W7-1..W7-6 (W7-7 deferred to operator attestation) | S | review | IN PROGRESS 2026-05-14 — docs-only planner-journal slice marking Phase 7 closed and activating Phase 8. See DEC-PHASE7-043. |
...
**Env:** macOS + Linux | **Status:** Active (REOPENED 2026-05-15 — W7-7 critical finding; original activation 2026-05-14 / Phase 7 closed at `05b98a3`; DEC-PHASE8-003 SUPERSEDED by DEC-PHASE8-004 per W7-7 attestation gap surfaced in #43)
...
| W7-7 | Physical USB boot validation (operator track, parallel — HARD HUMAN BOUNDARY) | Hardware | * | W7-3, W8-1 | S | approve | **FAILED 2026-05-15** — operator hardware attestation found the ISO MISSING the entire Orion-X application layer (#43): no `/opt/orionx/scripts/` (artifact-analyzer.py, mesh CLI, setup-matrix.sh, storyboard-gen.py, etc.), no `/opt/orionx/theme/`, no `/opt/orionx/data/`, no `/usr/share/doc/orionx/`, no PATH symlinks at `/usr/bin/orionx-mesh` etc. ISO boots a stock Debian Bullseye + Phase 6/7 infrastructure but is NOT a recognizable Orion-X release. v2.0.0-rc1 MUST NOT be tagged in its current form. The Phase 7 test surface gated on infrastructure (hooks ran, services exist, perf passes, restart configs correct) but NEVER on content presence inside the booted ISO — a stock Debian + systemd units + AppArmor profiles passes every Phase 7 test. W7-7 hardware attestation is praised here as the operator gate correctly catching this before tag publication (best-case W7-7 outcome). Re-attestation will be required after wi-w8-content-staging lands; same `approve` gate per DEC-PHASE7-005 / DEC-PHASE8-004. See DEC-PHASE8-004, DEC-PHASE8-005. |
| W8-7 | `v2.0.0` release tag + final GitHub Release publish (HARD HUMAN BOUNDARY) | Repo/CI | 5 | wi-w8-content-staging, wi-w8-finish-B, W7-7 operator re-attestation, operator GPG key provisioning | XS | approve | **BLOCKED 2026-05-15** — pending #43 fix (wi-w8-content-staging) and W7-7 re-attestation. Originally PENDING 2026-05-14 after wi-w8-finish-B `0b8f1b0` made the publish path mechanical, but W7-7 attestation 2026-05-15 invalidated the readiness assumption. New blockers: (1) wi-w8-content-staging lands (stage application layer into ISO, add content-presence test gate); (2) operator re-runs W7-7 against the corrected ISO and signs off; (3) operator provisions GPG secrets; (4) operator retags/version-bumps to `v2.0.0` per `docs/release-process.md`; (5) `gh release edit v2.0.0 --draft=false`. Explicit `approve` gate per DEC-PHASE7-005. Planner records closure DEC-PHASE8-NNN at publish. See DEC-PHASE8-004, DEC-PHASE8-005. |
| wi-w8-content-staging | Stage Orion-X application content into ISO + content-presence test gate (#43 fix) | Repo/CI | 4 | wi-w8-finish-A, wi-w8-finish-B | L | review | **PENDING 2026-05-15** — seeded after W7-7 hardware attestation found ISO missing application layer (#43). Mission: add `stage_application_content()` to `scripts/build-iso.sh` (rsync `scripts/`, `theme/`, `data/`, `docs/` into `iso/config/includes.chroot/{opt/orionx,usr/share/doc/orionx}/` at build time, NOT committed); add `iso/config/hooks/live/0700-orionx-setup.hook.chroot` creating `/usr/bin/` symlinks for the eight named entrypoints (orionx-mesh, setup-matrix.sh, setup-wireguard.sh, artifact-analyzer.py, storyboard-gen.py, toggle-theme.sh, run-lynis.sh, download-samples.sh) and setting executable bits; add `.gitignore` exclusions for staged paths; author `tests/integration/test-iso-content-presence.sh` + two structural unit tests; wire the integration test as an ACTIVE gate (no `continue-on-error`) in `.github/workflows/qemu-test.yml`. Scope Manifest in `tmp/scope-wi-w8-content-staging.json` (allowed: `scripts/build-iso.sh`, new hook, `.gitignore`, three new tests, `qemu-test.yml`; forbidden: source content at `scripts/**`, `theme/**`, `data/**`, `docs/**`, all Phase 6 hooks, `Dockerfile`, `systemd/**`, `MASTER_PLAN.md`, `README.md`, `CHANGELOG.md`, all other workflows). Evaluation Contract in `tmp/eval-wi-w8-content-staging.json`. State authorities: `iso_content_staging_authority` (build-iso.sh as single staging authority), `iso_build_pipeline_authority` (preserved per DEC-PHASE7-002), `release_readiness_gate_authority` (new content-presence test as canonical gate). See DEC-PHASE8-005. |

**Critical path (revised 2026-05-15 post-W7-7-failure reopen, per DEC-PHASE8-004):**
`W8-1 (ACCEPTED) → wi-w8-finish-A (ACCEPTED) → wi-w8-finish-B (ACCEPTED) → wi-w8-content-staging (PENDING — #43 fix) → W7-7 re-attestation (operator track) → W8-7 (approve gate, hard human boundary)`.

Pre-reopen the critical path was three software slices then operator action; the reopen adds a fourth software slice (wi-w8-content-staging) before the operator track can re-engage W7-7. Two hard human boundaries remain at phase end: W7-7 (physical USB re-attestation against the corrected ISO, runs in parallel on operator hardware, must complete before W8-7 publish) and W8-7 (final tag + publish, explicit `approve` gate per DEC-PHASE7-005). Four Phase 8 software slices total post-reopen (W8-1, wi-w8-finish-A, wi-w8-finish-B, wi-w8-content-staging). DEC-PHASE8-003 (premature software-track closure) is SUPERSEDED by DEC-PHASE8-004; cascade-consolidation discipline per DEC-PHASE7-041 is NOT applicable to the content-staging slice — this is a fundamental architectural fix, not a downstream cascade exit, and bundling it with other Phase 8 work would conflate a release-readiness gate fix with unrelated work.
...
**Phase 8 software-track closure (2026-05-14, develop @ `0b8f1b0`) — SUPERSEDED 2026-05-15 by DEC-PHASE8-004:** The
Phase 8 software track was previously declared complete on 2026-05-14. That
closure is SUPERSEDED by DEC-PHASE8-004 per W7-7 attestation findings (#43).
The original closure narrative is retained below for traceability; readers
should treat it as describing the pre-reopen state and note that
wi-w8-content-staging is the new active slice. Five implementation slices landed on
...
**What the operator does next (handoff summary, REVISED 2026-05-15 post-reopen):**

The pre-reopen handoff sequence (cut rc1, perform W7-7, promote to v2.0.0)
is HALTED. v2.0.0-rc1 must NOT be tagged or published while the ISO is
missing the application layer per W7-7 finding #43. The revised sequence:

1. **wi-w8-content-staging lands.** Planner has seeded the slice; next
   canonical dispatch is guardian (provision). When this slice lands,
   the ISO produced by `scripts/build-iso.sh` will contain the Orion-X
   application layer (scripts/theme/data/docs staged into the chroot
   plus PATH symlinks), and `tests/integration/test-iso-content-presence.sh`
   will gate `qemu-test.yml` against any future regression.
2. **W7-7 re-attestation.** Operator downloads the corrected ISO from the
   `qemu-test.yml` CI artifact (`gh run download <run_id> -n
   orionx-iso-<run_id>`); `dd`-writes to physical USB; boots on at least
   one real piece of hardware (UEFI laptop + BIOS desktop or equivalent);
   validates the `docs/User_Guide.md` walkthrough end-to-end (mesh-up,
   Matrix-up, forensic toolkit, all PATH-accessible). Re-engages the
   same `approve` gate per DEC-PHASE7-005; this is not a new boundary.

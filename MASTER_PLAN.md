...
##### W11-9b — Desktop identity (GTK theme + icons + cursor + fonts + xfce4-terminal)

**Workflow ID:** `phase11-w11-9b-desktop`. **Weight:** L. **Deps:** W11-9a landed. **Scope Manifest:** `tmp/phase11-w11-9b-desktop-scope.json` (written by planner at W11-9b provisioning time — deferred to next planner dispatch so scope reflects the state after W11-9a lands).

**Sub-slice intent (planner-preview; detail plan authored at W11-9b provisioning time):** Ship the running-session identity: (d) XFCE GTK theme `Orion-X-Cyberdeck` (dark base, terminal aesthetic, Phoenix red-orange single-accent color, derived from Adwaita-dark LGPL-2.1+ with documented modifications); (e) icon theme `Orion-X-Icons` (Phoenix + orionx-* tool overlays, derived from Papirus GPL-3.0+ or Numix); (f) cursor theme `Orion-X-Cursor` (subtle Phoenix-tinted, derived from Bibata GPL-3.0); (g) fonts (Iosevka OFL-1.1 primary + Hack Apache-2.0-additions secondary, staged at `/usr/share/fonts/opentype/orionx/`, `fc-cache -f` in the 0800 hook, JetBrains Mono NOT present anywhere per DEC-PHASE11-013); xfce4-terminal profile (color scheme matching Orion-X-Cyberdeck palette, font Iosevka 11, staged via `/home/orionx/.config/xfce4/terminal/terminalrc` written by `0100-create-user.hook.chroot`). Extend the DEC-PHASE9-002 xfconf XML in `0100-create-user.hook.chroot` to also write `GtkThemeName=Orion-X-Cyberdeck`, `IconThemeName=Orion-X-Icons`, `CursorThemeName=Orion-X-Cursor`, `MonospaceFontName=Iosevka 11` into xsettings.xml. Upgrade the transitional theme references in W11-9a's LightDM greeter conf from Adwaita-dark → Orion-X-Cyberdeck, Adwaita → Orion-X-Icons, Sans 11 → Iosevka 11.

**W11-9b planner-preview Evaluation Contract (~6 items; full contract authored at W11-9b provisioning time):**

1. GTK theme `Orion-X-Cyberdeck` present in squashfs with `index.theme`, `gtk-3.0/gtk.css`, `gtk-2.0/gtkrc`, `xfwm4/themerc`. Derivation source documented in an `@decision` header of `gtk.css`.
2. Icon theme `Orion-X-Icons` present with `index.theme` and at least the Phoenix + orionx-* application icons.
3. Cursor theme `Orion-X-Cursor` present with `index.theme` and the standard cursor set.
4. Fonts present: `Iosevka-*.otf` (or `.ttf`) under `/usr/share/fonts/opentype/orionx/iosevka/`; `Hack-*.ttf` under `/usr/share/fonts/opentype/orionx/hack/`. `fc-cache -f` run by 0800 hook (visible in build log).
5. xsettings.xml in xfconf writes the Orion-X-Cyberdeck / Orion-X-Icons / Orion-X-Cursor / Iosevka defaults on first login (test via boot + xfconf-query in QEMU or content-presence).
6. LightDM greeter conf upgraded from W11-9a's transitional Adwaita references to Orion-X references.
7. `grep -rn -i "jetbrains" iso/ scripts/ tests/ CHANGELOG.md MASTER_PLAN.md` returns zero hits EXCEPT the permanent historical rows in MASTER_PLAN (row 2406, DEC-PHASE11-010 at 3273, bullet at 2338) which are explicitly documented as superseded by DEC-PHASE11-013.
8. **W11-9a preservation (mechanical diff):** all W11-9a boot-chain files untouched except the LightDM greeter conf which is edited in one place (theme-name / icon-theme-name / font-name lines).
9. Content-presence section 23b green.

...
| W7-7 | Physical USB boot validation (operator track, parallel — HARD HUMAN BOUNDARY) | Hardware | * | W7-3, W8-1 | S | approve | DEFERRED 2026-05-14 — carries forward unchanged from Phase 7 closure. Remains operator attestation per DEC-PHASE7-005. Artifact channels available: (a) qemu-test.yml CI artifact `orionx-iso-<run_id>` per W7-7 enabler `b411c47`; (b) once a tag is cut, the DRAFT release.yml ISO + checksums per wi-w8-finish-B `0b8f1b0`. Operator action only — no software slice can close this. See DEC-PHASE8-003. |
| W8-7 | `v2.0.0` release tag + final GitHub Release publish (HARD HUMAN BOUNDARY) | Repo/CI | 5 | wi-w8-finish-B, W7-7 operator attestation, operator GPG key provisioning | XS | approve | PENDING 2026-05-14 — wi-w8-finish-B `0b8f1b0` makes the publish path mechanical: operator runs (a) `git tag -a v2.0.0-rc1` + `git push origin v2.0.0-rc1` to fire release.yml and emit the DRAFT rc1 release for W7-7 validation; (b) completes W7-7 hardware attestation; (c) provisions `secrets.GPG_PRIVATE_KEY` + `secrets.GPG_PASSPHRASE` (one-time); (d) retags or version-bumps to `v2.0.0` per `docs/release-process.md`; (e) `gh release edit v2.0.0 --draft=false`. Explicit `approve` gate per DEC-PHASE7-005. Planner records closure DEC-PHASE8-004 (or successor) at publish. See DEC-PHASE8-003. |
...
**Phase 8 software-track closure (2026-05-14, develop @ `0b8f1b0`):** The
Phase 8 software track is complete. Five implementation slices landed on
...
**What the operator does next (handoff summary):**

1. **To cut `v2.0.0-rc1` right now:** `git tag -a v2.0.0-rc1 -m "v2.0.0-rc1
   release candidate" && git push origin v2.0.0-rc1`. release.yml will
   fire, build the ISO via `scripts/build-iso.sh`, emit SHA-256 and
   SHA-512 checksums, attempt GPG signing (skipped non-blocking until
   GPG secrets are provisioned), and create a DRAFT GitHub Release with
   notes sourced from `CHANGELOG.md`. See `docs/release-process.md` for
   the full runbook.
2. **To perform W7-7 attestation:** download the ISO from either the
   qemu-test.yml CI artifact (`gh run download <run_id> -n
   orionx-iso-<run_id>`) or the DRAFT rc1 release once cut; `dd`-write to
   physical USB; boot on at least one real piece of hardware (target list
   per the goal contract: a UEFI laptop + a BIOS desktop or equivalent);
   validate the User_Guide walkthrough end-to-end (mesh-up, Matrix-up,
   forensic toolkit). See `docs/User_Guide.md` for the walkthrough and the
   W7-7 row above for the `approve` gate.
3. **To promote `v2.0.0-rc1` → `v2.0.0` final:** complete W7-7
   attestation; provision `secrets.GPG_PRIVATE_KEY` +
   `secrets.GPG_PASSPHRASE` in the GitHub repo settings; perform the
   version bump + retag procedure in `docs/release-process.md`; re-run
   release.yml on the new `v2.0.0` tag (signatures now produced
   unconditionally); verify the GitHub Release contents; then
   `gh release edit v2.0.0 --draft=false` to publish. The planner records
   closure DEC-PHASE8-004 (or successor) at publish.

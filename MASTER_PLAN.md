...
**Env:** Linux (live-build) + QEMU + physical hardware | **Status:** Active — planned 2026-05-25; **W9-1 (rc4 broken-basics) LANDED @ `1c6c87c` (FF to develop, pushed to origin/develop).** **W9-1b (rc4 build-break cascade closure, PR #49) LANDED 2026-05-28 @ merge commit `e634cbef147030c59c30c49257db316dd6a4b285` on develop** (4 CI-iteration cascade `2c2d6e9`→`1c00d08`→`c2ad4be`→`e8eab0a`; CI run 26619266057 on `e8eab0a` GREEN — build + content-presence + BIOS + UEFI; reviewer `ready_for_guardian` @ `e8eab0a`, 0 blockers / 1 cosmetic pre-existing note; 861 unit tests green / 0 failed). **rc4 is now actually buildable.** rc4 awaits the **W9-4 operator tag/publish gate** + the operator's hardware re-test before W9-2/W9-3 are detail-planned. The gecko-legacy port slice (W9-2a) is **seeded (scope + eval contract synced) but parked behind the operator's rc4 hardware re-test**; W9-2 / W9-3 remain sketched and tracked as GitHub issues, **not detail-planned until rc4 is hardware-validated** (operator's chosen sequencing). | **Workflow:** `phase9-cyberdeck-ux`
...
| W9-2a | Gecko legacy port: `pcap-analyzer.py` + fail2ban (W9-2 input) | Linux/CI | 1.7 | W9-1b landed; rc4 operator hardware re-test | S | review | **SEEDED — parked behind operator rc4 hardware re-test.** Scope manifest + evaluation contract synced to runtime (workflow_id=`wi-w9-2a-gecko-legacy`); ready to dispatch on operator confirmation or in parallel at operator discretion. DEC tbd in detail-planning. |
| W9-2 | XFCE panel status widgets + GTK "Orion-X Control Center" | Linux/CI | 2 | W9-1, W9-2a | XL | review | SKETCHED — tracked as GitHub issue; detail-planned after W9-1 lands. DEC-PHASE9-001. |
...
**Critical path:** `W9-1 → W9-1b (CLOSED 2026-05-28 @ e634cbe; rc4 buildable) → operator hardware re-test of rc4 → W9-2a (gecko-legacy port) → W9-2 (panel + Control Center) → W9-3 (threat-posture tiers) → rc5 / v2.0.0 final`. W9-4 (rc4 tag/publish) runs as an operator boundary right after W9-1b lands and re-tests green; mirrors W8-7.

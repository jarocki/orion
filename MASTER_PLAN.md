...
#### W11-5 — Detail plan (Team comms — LAYER A: matrix-commander venv + comms skeleton + .desktop launcher; Layer B gomuks static binary deferred to W11-5b)

**Seeded:** 2026-07-19 planner amendment. **Status:** ready for `guardian:provision`. **Env:** GitHub Actions `ubuntu-latest` + `debian:bullseye` build container; no hardware attestation required for Layer A (content-presence is the acceptance gate; hardware attestation rolls in at W11-10 rc-cut). **Wave:** 3 of Phase 11 (may run in parallel with W11-3 / W11-4 once W11-2 has landed — precisely as scoped by the mainline W11-5 row at line 2402). **Gate:** review. **Weight:** M (Layer A only). **Deps:** W11-2 (landed at `9fe8c4b`) + W11-2b (`98dfc2e`) + W11-2c (`0838a31`) + W11-2d (`7759e84`) + W11-2e (queued) + W11-2f (queued) + W11-3 Layer A (landed) + W11-4 Layer A (landed at `a09a737`). **Workflow ID:** `phase11-w11-5-team-comms`. **Scope Manifest:** `tmp/phase11-w11-5-team-comms-scope.json` (written 2026-07-19 by this planner amendment; synced to runtime via `cc-policy workflow scope-sync`).

**Pragmatic decomposition (Layer A / Layer B split) — WHY the mainline W11-5 row (`MASTER_PLAN.md:2402`) is being split into two landings, not one M-weight slice:**

The mainline W11-5 row folds together three payload classes: (1) `matrix-commander` pip install into a build-time `/opt/orionx/venv/comms/` (Bullseye Python 3.9) + the `/opt/orionx/comms/` staging skeleton (README) + a `.desktop` launcher for matrix-commander running under `xfce4-terminal`; (2) the `gomuks` static binary fetched at build time from the latest tagged `mautrix/gomuks` release tarball and staged under `/opt/orionx/comms/gomuks/` with its own `.desktop` launcher; (3) the `install-element.sh` optional-installer stub for `element-desktop` (bulk fetch deferred to W11-8 optional-installer framework per the mainline row's own wording — this class is NOT touched by W11-5 at all, it lives in W11-8's scope). Payload (1) reuses the exact soft-fail pip-into-venv pattern that already works in-tree at the 0500 hook (see the `pip3 install volatility3 || echo "WARNING..."` block at line 33) and touches only the 0500 hook body + includes.chroot text files — zero new network-fetch cascades, zero new external tarball URL pins. Payload (2) requires a NEW upstream release-asset fetch (mautrix/gomuks GitHub releases → `.tar.gz` extract → chmod + stage) that mirrors the ollama `.tar.zst` fetch pattern from W10-1 iter-7 (see line 62 of the same hook) — soft-fail is acceptable here (comms is non-load-bearing unlike ollama which is LOUD-fail), but the URL-pin + trust-on-first-use SHA-256 discipline still applies (same shape as DEC-PHASE10-008 for the Nebula model, applied to the gomuks tarball). Landing (1)+(2) as a single M-weight slice couples the fast, low-risk pip install (matrix-commander is on PyPI, published by @8go, stable API since 2022) to a heavier upstream-release-fetch cascade that adds one more URL-pin failure mode to the base ISO build. **This planner amendment decouples them, mirroring the W11-3 Layer A / Layer B and W11-4 Layer A / Layer B precedents seeded on the same day** (see W11-3 at line 3250 and W11-4 at line 3346: same rationale, same split shape, same DEC-preserved-verbatim discipline). Layer A (this slice) lands payload (1) + content-presence Section 23 asserting matrix-commander venv presence + comms skeleton + .desktop launcher + a NEGATIVE assertion that `/opt/orionx/comms/gomuks/` is absent (Layer B sentinel), plus a CHANGELOG entry naming Layer B as deferred. Layer B (a follow-up W11-5b slice, seeded at the end of this section) lands payload (2) + expanded Section 23 assertions covering the gomuks binary + its .desktop launcher, and flips the negative sentinel. **The mainline W11-5 row policy is preserved verbatim** — the split is a landing-cadence choice, not a policy change; the same matrix-commander + gomuks bundle lands in the same v2.1.0, just across two commits instead of one. `element-desktop` remains W11-8-owned per the mainline row's original wording (`Add install-element.sh stub to W11-8 optional-installer framework`) and is NOT in W11-5 Layer A or Layer B scope.

**Scope Manifest — allowed / required / forbidden (canonical file at `tmp/phase11-w11-5-team-comms-scope.json`, synced to runtime):**

| Path | Class | Rationale |
| --- | --- | --- |
| `iso/config/hooks/live/0500-install-external-tools.hook.chroot` | allowed + required | Layer A appends a new soft-fail block modeled EXACTLY on the existing `pip3 install volatility3 || echo "WARNING..."` pattern at line 33. Block body: `python3 -m venv /opt/orionx/venv/comms/ || echo "WARNING: comms venv creation failed"` then `/opt/orionx/venv/comms/bin/pip install --no-input matrix-commander || echo "WARNING: matrix-commander install failed"` then `ln -sf /opt/orionx/venv/comms/bin/matrix-commander /usr/local/bin/matrix-commander || echo "WARNING: matrix-commander symlink failed"`. Section-header comment banner: `# W11-5 Layer A: matrix-commander comms venv (DEC-PHASE10-007 soft-fail pattern per volatility3 precedent)`. NO other edits to the hook — no reordering, no touching the ollama LOUD-fail block, no touching the zeek block, no touching the ghidra header comments. The 0500 hook's existing `@decision DEC-PHASE10-007` (line 6) + `@decision DEC-PHASE11-004` (line 17) headers are preserved byte-identical. |
| `iso/config/includes.chroot/opt/orionx/comms/README.md` | required | New file. Documents the team-comms surface per the mainline W11-5 row: matrix-commander (Layer A, this slice), gomuks (Layer B, W11-5b), element-desktop (W11-8 optional installer). Sections: title (`# Orion-X Team Comms`), one-sentence mission rationale (secure team chat over the existing Phase 4 Matrix homeserver + Phase 3 WireGuard mesh — no config changes to Synapse or wg-quick), `## Shipped in base (Layer A, this v2.1.0)` listing matrix-commander + install path + `.desktop` launcher path, `## Deferred to W11-5b (Layer B)` listing gomuks static binary + upstream URL + its planned `.desktop` launcher, `## Deferred to W11-8 optional installer` listing element-desktop with size + license + network requirements, `## Usage` giving a one-liner example (`matrix-commander --login password --homeserver https://matrix.orionx.local --user-login @operator:orionx.local`), `## Related` pointing at `/etc/matrix-synapse/homeserver.yaml` (Phase 4 authority — read only, do not edit) and `/etc/wireguard/wg0.conf` (Phase 3 authority — read only, do not edit). Roughly 50-80 lines. |
| `iso/config/includes.chroot/usr/share/applications/orionx-matrix-commander.desktop` | required | New file. XDG `.desktop` launcher for matrix-commander running inside `xfce4-terminal`. Content: `[Desktop Entry]` section with `Type=Application`, `Name=Matrix Commander (Orion-X)`, `Comment=Matrix CLI client for secure team comms`, `Exec=xfce4-terminal --title="Matrix Commander" -e "/usr/local/bin/matrix-commander --help; bash"` (spawns a persistent terminal that prints help then leaves an interactive shell so operators can inspect flags before running), `Icon=utilities-terminal` (uses the base icon theme's terminal icon; W11-9c may later replace with an Orion-X icon), `Terminal=false` (xfce4-terminal is invoked by Exec directly), `Categories=Network;InstantMessaging;`, `Keywords=matrix;chat;comms;`. Roughly 15-25 lines. |
| `tests/integration/test-iso-content-presence.sh` | required | Append-only NEW Section 23 block after the existing Section 22 (W11-4 Layer A) block. Section-header comment banner following the existing convention: `# 23. W11-5 Layer A: matrix-commander comms venv + skeleton + .desktop launcher`. Assertions: (23a) `/opt/orionx/venv/comms/bin/matrix-commander` present in squashfs and executable; (23b) `/usr/local/bin/matrix-commander` symlink present and resolves into `/opt/orionx/venv/comms/`; (23c) `/opt/orionx/comms/README.md` present in squashfs and contains `matrix-commander` and `Layer A / Layer B` marker phrase; (23d) `/usr/share/applications/orionx-matrix-commander.desktop` present + valid `[Desktop Entry]` section + `Type=Application` + `Exec=` line references matrix-commander; (23e) NEGATIVE assertion that `/opt/orionx/comms/gomuks/` does NOT exist yet (Layer B sentinel — flipped to POSITIVE at W11-5b landing time). Preserve the file's `pass`/`fail` helper convention verbatim. Soft-fail-tolerant: 23a/23b use the same "either dpkg-status OR binary present" fallback shape that W11-4 Layer A's 22a assertion uses, so a matrix-commander install failure at build time still allows the test to report the failure loudly rather than the run erroring out. |
| `CHANGELOG.md` | required | Append a `### W11-5 Layer A: matrix-commander comms venv + skeleton + .desktop launcher (gomuks deferred to W11-5b)` subsection under `## [v2.1.0] - Unreleased`, immediately after the existing W11-4 Layer A entry. Names: matrix-commander pip install into `/opt/orionx/venv/comms/` via 0500 hook soft-fail block (DEC-PHASE10-007 pattern); `/usr/local/bin/matrix-commander` symlink; `/opt/orionx/comms/README.md` skeleton documenting Layer A + Layer B + W11-8 element-desktop; `orionx-matrix-commander.desktop` XDG launcher; content-presence Section 23 (five assertions: 23a-23e); W11-5b explicitly named as deferred (gomuks static binary + its .desktop launcher); NO DEC change. |
| `MASTER_PLAN.md` | allowed | This planner amendment (the section you are reading now). Layer A implementer must NOT edit MASTER_PLAN.md beyond appending the Layer A iter-1 trailer if planner asks; guardian:land appends the landed-at SHA next to the W11-5 seed row (Phase 8/9/10/11 landing-record precedent). |
| `tmp/**` | allowed | Scope manifest + any implementer scratch artifacts. |
| `iso/config/package-lists/**` | forbidden | Layer A adds ZERO Debian packages. matrix-commander is a pip install into a venv (Bullseye Python 3.9), not an apt package. The `xfce4-terminal` and `python3-venv` requirements are already pulled in by the base package list (xfce4 metapackage + python3 stdlib defaults). If a future implementer discovers `python3-venv` is NOT already installed, that is an out-of-scope escalation back to planner — do NOT silently add it to the package list here. |
| `iso/config/includes.chroot/opt/orionx/{nebula,re,yara,scripts,theme,optional,venv,data}/**` | forbidden | Adjacent authorities (nebula = W10-*; re = W11-3; yara = W11-4; scripts = shared source pool; theme = W9-1 / W11-9; optional = W11-8; venv/comms = created by the 0500 hook at build time, NOT staged in includes.chroot — the venv directory itself is a hook-side runtime artifact; data = samples). Note the deliberate asymmetry: `/opt/orionx/comms/` (this slice's stage dir) IS allowed and required for the README; `/opt/orionx/venv/comms/` (a subtree of the same parent) is NOT staged — the hook creates it at build time inside the chroot. |
| `iso/config/hooks/live/{0100,0600,0610,0615,0620,0700,0800}-*.hook.chroot` | forbidden | Only 0500 is touched. 0700 symlink pattern is available (W11-4 used it for `orionx-freshen-yara`) but for W11-5 we prefer the 0500-internal `ln -sf` approach because the symlink target must be created by the same hook that creates the venv, in the same execution context — splitting across 0500 and 0700 introduces an ordering dependency that hurts readability without buying anything. This is a deliberate discipline choice, not an oversight. |
| All other listed forbidden paths in the scope JSON | forbidden | See `tmp/phase11-w11-5-team-comms-scope.json` for the full list. Notably: no touching `.github/workflows/**`, `runtime/**`, `hooks/**`, `agents/**`, `settings.json`, `CLAUDE.md`, `AGENTS.md`, `DECISIONS.md`, `iso/config/nebula-model-manifest.json`, `iso/auto/**`, or any other `orionx-*.sh` script in `scripts/` (including the W11-4 freshen script). |

**Task list (T1–T5, appended verbatim into implementer briefing at provision):**

1. **T1 — 0500 hook comms venv block.** Edit `iso/config/hooks/live/0500-install-external-tools.hook.chroot`. Append near the end of the file (after the last existing install block, before any trailing `echo "=== Done ===" ` line if present), a new section-header comment banner: `# ---------------------------------------------------------------------------` / `# W11-5 Layer A: matrix-commander comms venv (DEC-PHASE10-007 soft-fail pattern)` / `# ---------------------------------------------------------------------------`. Body (3 lines + soft-fail messages, following the volatility3 precedent at line 33):

    ```bash
    python3 -m venv /opt/orionx/venv/comms/ || echo "WARNING: comms venv creation failed"
    /opt/orionx/venv/comms/bin/pip install --no-input matrix-commander || echo "WARNING: matrix-commander install failed"
    ln -sf /opt/orionx/venv/comms/bin/matrix-commander /usr/local/bin/matrix-commander || echo "WARNING: matrix-commander symlink failed"
    ```

    Verify: `bash -n iso/config/hooks/live/0500-install-external-tools.hook.chroot` passes; `shellcheck -S error iso/config/hooks/live/0500-install-external-tools.hook.chroot` exits 0; the file's `@decision DEC-PHASE10-007` (line 6) and `@decision DEC-PHASE11-004` (line 17) headers remain byte-identical (no reordering, no rewording). NO edits to the volatility3, zeek, ollama, or ghidra blocks. Commit: `feat(w11-5): 0500 hook — matrix-commander comms venv soft-fail block (Layer A)`.

2. **T2 — Comms staging README.** Create `iso/config/includes.chroot/opt/orionx/comms/README.md`. Content: title `# Orion-X Team Comms`, one-sentence mission rationale citing Phase 4 Matrix homeserver + Phase 3 WireGuard mesh integration, a `## Shipped in base (Layer A, this v2.1.0)` section listing matrix-commander + venv path + symlink path + `.desktop` launcher path, a `## Deferred to W11-5b (Layer B)` section listing gomuks static binary + upstream URL (`https://github.com/tulir/gomuks/releases/latest`) + its planned `.desktop` launcher, a `## Deferred to W11-8 optional installer` section listing element-desktop with size (~200 MB installed) + license (Apache-2.0) + network requirements (Debian package download at install-time), a `## Usage` section with a one-liner example, and a `## Related` section pointing at Phase 4 + Phase 3 authorities as read-only references. Roughly 50-80 lines. Include the `Layer A / Layer B` marker phrase explicitly so content-presence assertion 23c can grep for it. Commit: `feat(w11-5): stage /opt/orionx/comms/README.md — team comms layout doc (Layer A)`.

3. **T3 — matrix-commander .desktop launcher.** Create `iso/config/includes.chroot/usr/share/applications/orionx-matrix-commander.desktop`. Content per the Scope Manifest row above (roughly 15-25 lines). Validate with `desktop-file-validate` if available in the build container (soft-fail if not — the tool is provided by the `desktop-file-utils` package which is in the base ISO but may not be in the CI runner; the assertion is content-presence, not validate-clean). Commit: `feat(w11-5): stage orionx-matrix-commander.desktop XDG launcher (Layer A)`.

4. **T4 — Content-presence Section 23.** Edit `tests/integration/test-iso-content-presence.sh`. Append a Section 23 block after the existing Section 22 block (around line 1930 per current file inspection). Section-header comment banner: `# 23. W11-5 Layer A: matrix-commander comms venv + skeleton + .desktop launcher (DEC-PHASE10-007 soft-fail pattern)` with a `@decision DEC-PHASE10-007` reference in the block header comment. Assertions 23a-23e per the Scope Manifest table above. Preserve the file's `pass`/`fail` helper convention verbatim. The soft-fail tolerance for 23a/23b is critical: matrix-commander's PyPI install can fail at build time if PyPI is unreachable, and the test must report that as a loud fail without erroring out the entire content-presence run. Commit: `test(w11-5): content-presence section 23 — matrix-commander comms assertions (Layer A)`.

5. **T5 — CHANGELOG + W11-5b seed + MASTER_PLAN iter trailer.** (a) Edit `CHANGELOG.md`. Append the W11-5 Layer A subsection under `## [v2.1.0] - Unreleased` per the Scope Manifest row above. (b) Edit `MASTER_PLAN.md` to append the W11-5b Layer B seed row + iter-1 trailer inside this section (identical structure to the W11-3b and W11-4b seeds; see W11-5b seed spec below). Both edits fold into one commit: `docs(w11-5): CHANGELOG v2.1.0 + W11-5b Layer B seeded + iter-1 trailer (Layer A)`.

**Evaluation Contract (5 items — matches the user-supplied contract verbatim; reviewer + guardian gate against these):**

1. **EC1: 0500 hook comms venv block installed.** `iso/config/hooks/live/0500-install-external-tools.hook.chroot` contains a new block (bounded by the W11-5 section-header comment banner) with exactly three soft-fail lines: `python3 -m venv /opt/orionx/venv/comms/`, `/opt/orionx/venv/comms/bin/pip install --no-input matrix-commander`, and `ln -sf /opt/orionx/venv/comms/bin/matrix-commander /usr/local/bin/matrix-commander` — each followed by `|| echo "WARNING: ..."`. `bash -n` passes; `shellcheck -S error` exits 0. The file's `@decision DEC-PHASE10-007` and `@decision DEC-PHASE11-004` headers are unchanged (byte-identical to the file at the slice's base HEAD).
2. **EC2: /opt/orionx/comms/README.md present.** File present in squashfs at `/opt/orionx/comms/README.md`. Content includes the phrase `matrix-commander` AND the `Layer A / Layer B` marker phrase (assertion 23c matches this). Names gomuks as Layer B (W11-5b) deferred, and names element-desktop as W11-8 optional-installer deferred.
3. **EC3: matrix-commander .desktop launcher present.** `iso/config/includes.chroot/usr/share/applications/orionx-matrix-commander.desktop` present with valid `[Desktop Entry]` section header, `Type=Application`, `Exec=` line references `matrix-commander`, `Categories=Network;InstantMessaging;`. Reachable at `/usr/share/applications/orionx-matrix-commander.desktop` in the built squashfs.
4. **EC4: Content-presence Section 23 assertions.** `tests/integration/test-iso-content-presence.sh` gains Section 23 with assertions 23a-23e per the Scope Manifest row. When run against the built ISO squashfs: 23a (matrix-commander binary + executable in venv), 23b (`/usr/local/bin/matrix-commander` symlink resolves into `/opt/orionx/venv/comms/`), 23c (README present + marker phrase), 23d (`.desktop` launcher present + valid), 23e (NEGATIVE: `/opt/orionx/comms/gomuks/` absent — Layer B sentinel). All five assertions pass. Existing sections 1-22 unchanged. The full test binary (`bash tests/integration/test-iso-content-presence.sh`) exits 0.
5. **EC5: CHANGELOG entry.** `CHANGELOG.md` `## [v2.1.0] - Unreleased` section gains the W11-5 Layer A subsection. Names the matrix-commander pip install, venv path, symlink, comms skeleton (README), `.desktop` launcher, content-presence Section 23, and W11-5b deferral explicitly (gomuks static binary + its `.desktop` launcher).

**Ready-for-guardian definition:** All five EC items pass on the reviewer's evaluation of the ISO build artifact; the reviewer emits `REVIEW_VERDICT: ready_for_guardian` with the head SHA; test-state is `pass_complete`; guardian lease active. Soft-fail semantics acknowledged: if matrix-commander PyPI install fails at build time (e.g., transient PyPI outage), the build itself succeeds (soft-fail per DEC-PHASE10-007), but Section 23 assertions 23a and 23b will fail loudly, blocking guardian:land until reviewer reruns the build with PyPI restored — this is the intended failure mode, not a bug.

**Forbidden shortcuts (explicitly banned):**

- Do NOT add `matrix-commander` to `iso/config/package-lists/orionx.list.chroot` — matrix-commander is not in Debian Bullseye apt (verified: no `matrix-commander` package in Bullseye main or backports). It ships only via PyPI. Adding a bogus package-list line would cause `apt-get install` to fail and abort the build (LOUD-fail on unknown package). The venv+pip pattern per the volatility3 precedent is the only correct install path.
- Do NOT ship the gomuks static binary in Layer A. That is Layer B (W11-5b). Adding it here would inflate the Layer A blast radius by adding a NEW upstream release-asset URL pin (mautrix/gomuks releases) + a NEW tarball extract cascade — exactly the coupling the Layer A / Layer B split is designed to prevent.
- Do NOT ship the gomuks `.desktop` launcher in Layer A. It has no binary to launch until W11-5b lands.
- Do NOT ship an `install-element.sh` optional installer in Layer A. That stub is owned by W11-8's optional-installer framework slice per the mainline W11-5 row's own wording. Adding it here would violate W11-8's scope and cause a double-authority for the element-desktop staging decision.
- Do NOT modify Phase 4 Matrix homeserver config (`iso/config/includes.chroot/etc/matrix-synapse/**`) or Phase 3 WireGuard config (`iso/config/includes.chroot/etc/wireguard/**`). The mainline W11-5 row is explicit: `no config changes to Synapse or wg-quick`. matrix-commander is a client that connects to the existing homeserver over the existing mesh; nothing about the server or mesh needs to change.
- Do NOT modify `DECISIONS.md` directly; it is auto-generated from `@decision` annotations by `stop.sh`.
- Do NOT introduce a new hook file. The 0500 hook body extension is the only mechanism for Layer A. If the 0500 hook is deemed "too large" by a future reader, that is an out-of-scope refactor discussion, not a W11-5 concern.
- Do NOT combine this slice with W11-3, W11-4, W11-5b, W11-6, W11-7, or W11-8 — each is its own workflow.
- Do NOT edit `.github/workflows/**` — not in scope. Content-presence Section 23 runs under the existing CI job; no runner changes needed.
- Do NOT edit `MASTER_PLAN.md` beyond the two allowed edits: the W11-5b Layer B seed row inside this section (T5b), and the W11-5 iter-1 trailer at the end of this section (T5c). Do NOT touch the mainline W11-5 row at line 2402, the Phase 11 reconciliation table row for W11-5 at line 2367, the Phase 11 whole-slice EC line 2388 mentioning section 23, or any Decision Log row.

**Downstream consumers (what this slice unblocks):**

- **W11-5b** (Layer B) — gomuks static binary from `mautrix/gomuks` releases + gomuks `.desktop` launcher + expanded Section 23 assertions covering the binary + launcher + LOCKFILE-style SHA-256 pin (trust-on-first-use per DEC-PHASE10-008 precedent, applied to the gomuks tarball). Seeded below.
- **W11-8** (optional-installer framework) — will ship `install-element.sh` referencing this slice's `/opt/orionx/comms/README.md` for the operator-facing team-comms surface documentation. W11-5 Layer A stages the README documenting element-desktop as W11-8-owned; W11-8 provisions the installer.
- **W10-3+** (Nebula MCP registry / Awareness pane) — matrix-commander's presence in the base ISO is a base-capability signal the Control Center Awareness pane may surface in a future slice; no coordination needed at Layer A landing time.

**W11-5b — Layer B seed row (SEEDED, not planned):**

Deferred payloads: (i) build-time fetch of the `mautrix/gomuks` static binary from the latest tagged release (upstream URL pattern: `https://github.com/tulir/gomuks/releases/download/v<X.Y.Z>/gomuks-linux-amd64`; pin exact version at W11-5b planner-dispatch time — anticipated ~10 MB binary + ~5 MB assets); (ii) tarball extract (if the release is a `.tar.gz`) or direct binary drop (if the release is a bare ELF), staged into `iso/config/includes.chroot/opt/orionx/comms/gomuks/`; (iii) `/usr/local/bin/gomuks` symlink into the staging dir; (iv) `iso/config/includes.chroot/usr/share/applications/orionx-gomuks.desktop` XDG launcher (Type=Application, Exec=xfce4-terminal launching gomuks TUI, Categories=Network;InstantMessaging;); (v) SHA-256 pin via trust-on-first-use per DEC-PHASE10-008 precedent (same shape as the ollama `.tar.zst` pin and the Qwen2.5-3B model pin — first successful CI build's SHA-256 is captured into a LOCKFILE-style pin file OR baked directly into the 0500 hook's fetch block); (vi) content-presence Section 23 assertion 23e flipped from NEGATIVE (`/opt/orionx/comms/gomuks/` absent) to POSITIVE (gomuks binary present + executable + symlinked + `.desktop` launcher present + SHA-256 matches pin); (vii) CHANGELOG v2.1.0 W11-5b entry naming all of the above + the SHA-256 pin outcome. Landing constraints: the mautrix/gomuks release channel must be reachable at build time from GitHub Actions runners (verified stable at planner time; trust-on-first-use handles transient outages via LOUD-fail-then-retry — same pattern as ollama). SoftFail vs LoudFail decision: Layer B should follow the SAME soft-fail discipline as matrix-commander (comms is non-load-bearing; the Control Center still boots without gomuks) — the ollama LOUD-fail pattern is reserved for load-bearing headline features per DEC-PHASE10-007 file-header rationale. ISO size delta target is +30 MB combined with Layer A per the Phase 11 reconciliation table (line 2367). Tracking mechanism: GitHub issue filed by guardian:land at Layer A landing time (title: `W11-5b: Layer B — gomuks static binary + .desktop launcher (defer from W11-5)`; labels: `phase-11`, `w11-5b`, `priority:medium`, `deferred-from:w11-5`). If for any reason guardian:land cannot file the issue, an inline TODO under this seed row is the fallback record; the operator triages via the backlog skill at next dispatch.

**Risk register (W11-5-specific):**

- **R1 — matrix-commander PyPI availability.** matrix-commander is published on PyPI as `matrix-commander` (verified at planner time; @8go maintainer; stable since 2022). If PyPI is unreachable at build time, the pip install fails soft and Section 23 assertions 23a/23b fail loudly. **Mitigation:** T1's soft-fail pattern per DEC-PHASE10-007 precedent + Section 23 assertion 23a/23b's "either dpkg-status OR binary present" fallback shape (from W11-4's 22a precedent). If PyPI is chronically unreachable in CI, that is an infrastructure escalation, not a W11-5 concern.
- **R2 — Bullseye Python 3.9 compatibility drift.** matrix-commander's latest PyPI release may drop Python 3.9 support in a future version, breaking the pip install on Bullseye. **Mitigation:** planner intentionally does NOT pin a matrix-commander version in Layer A — the soft-fail block absorbs the failure and Section 23 reports it. If a version pin becomes necessary, that is a Layer A follow-up hotfix (W11-5a1), not a Layer A concern. Alternatively, W11-5b can add a version pin to the pip install line as part of its own scope.
- **R3 — `python3-venv` package absent from Bullseye base.** `python3 -m venv` on Debian requires the `python3-venv` package (or `python3.9-venv` on Bullseye). Bullseye's XFCE + Python stdlib package pull `python3` but NOT necessarily `python3-venv`. **Mitigation:** verified at planner time via the existing package list — the volatility3 install at line 33 uses `pip3 install` which does NOT require venv (pip installs into system-wide site-packages when running as root inside the chroot), but `python3 -m venv` explicitly requires the ensurepip module which the `python3-venv` package provides. If the venv creation soft-fails with `The virtual environment was not created successfully because ensurepip is not available`, that is an in-scope diagnostic signal for a Layer A follow-up (add `python3-venv` to the package list — but note that would consume a `iso/config/package-lists/**` edit which is forbidden in this slice's Scope Manifest). **Escalation path:** if venv creation fails at first CI build, the reviewer verdict is `needs_changes` and planner is re-invoked to widen scope to allow the `python3-venv` package addition. Do NOT silently widen scope in Layer A.
- **R4 — `xfce4-terminal` absent from Bullseye base.** The matrix-commander `.desktop` launcher's `Exec=` line invokes `xfce4-terminal`. Bullseye's XFCE metapackage pulls xfce4-terminal by default; verified at planner time. **Mitigation:** if the invocation fails at first-boot GUI verification (a future W10-* Nebula-runtime concern, NOT this slice's acceptance gate), that is out of scope for content-presence and out of scope for W11-5.
- **R5 — Confusion with W11-8 element-desktop.** A future reader could confuse this slice's optional-installer README references with W11-8's actual installer scripts and try to ship `install-element.sh` here. **Mitigation:** T2 README explicitly names element-desktop as W11-8-owned; Forbidden shortcuts row above bans the `install-element.sh` addition explicitly.

**W11-5 iter-1 (planner detail-plan): 2026-07-19.** Detail plan appended to `MASTER_PLAN.md` on `develop`; NO new DEC (DEC-PHASE10-007 soft-fail pattern reused, DEC-PHASE11-004 optional-installer authority referenced); Scope Manifest at `tmp/phase11-w11-5-team-comms-scope.json` (synced to runtime via `cc-policy workflow scope-sync`). Ready for `guardian:provision` under workflow_id `phase11-w11-5-team-comms`.

---

#### W11-6 — Detail plan (Suricata IDS — LAYER A: package + lazy-start systemd override + /var/lib/suricata skeleton + freshen script + 0700 symlink; Layer B Nebula tier-selector runtime wiring deferred to W11-6b)

**Seeded:** 2026-07-19 planner amendment. **Status:** ready for `guardian:provision`. **Env:** GitHub Actions `ubuntu-latest` + `debian:bullseye` build container; content-presence is the acceptance gate. **Wave:** 4 of Phase 11 (folds W9-3 tier-selector residue that W10-5 elevated but never landed as a discrete Suricata deliverable). **Gate:** review. **Weight:** M (Layer A only; mainline row's XL was for the combined package+override+ruleset+contextualizer-wiring bundle — this planner amendment lands the package+override+skeleton+freshen slice only). **Deps:** W11-2 (landed at `9fe8c4b`) + W11-2b (`98dfc2e`) + W11-2c (`0838a31`) + W11-2d (`7759e84`) + W11-2e (`c4d78d1`) + W11-2f (landed) + W11-3 Layer A (landed at `1d2f6c5`) + W11-4 Layer A (landed at `a09a737`) + W11-5 Layer A (landed at `cb5dd4b`). **Workflow ID:** `phase11-w11-6-suricata`. **Scope Manifest:** `tmp/phase11-w11-6-suricata-scope.json` (audit/full-shape manifest); runtime scope-triad at `tmp/phase11-w11-6-suricata-scope-runtime.json` (synced to runtime via `cc-policy workflow scope-sync`).

**Pragmatic decomposition (Layer A / Layer B split) — WHY the mainline W11-6 row (`MASTER_PLAN.md:2403`) is being split into two landings, not one XL slice:** the mainline row folds four payload classes: (1) `suricata` Debian package + lazy-start systemd override at `/etc/systemd/system/suricata.service.d/orionx-lazy.conf`; (2) an ET-Open ruleset snapshot baked into the ISO at build time (if BSD-2-Clause licensing verifies) + operator freshen mechanism; (3) Nebula tier-selector RUNTIME wiring — Tier 0 passive (existing arp-scan/netdiscover from W9-3 sketch) → Tier 1 Suricata alert output → Tier 2 contained deception, feeding the W10-5 Nebula detection contextualizer; (4) alert-output-to-Nebula plumbing (JSON eve.log tail → Nebula MCP input adapter). Payload (1) is a low-risk includes-chroot + package-list edit that mirrors the W11-4 Layer A skeleton pattern EXACTLY (see W11-4 detail plan starting at line ~3346). Payload (2) requires a build-time network fetch cascade from `rules.emergingthreats.net` PLUS a fresh license re-verification at provision-time (planner defers licensing check per DEC-PHASE11-008 wording) — same shape as W11-4b deferred YARA ruleset snapshots. Payload (3) requires touching the Nebula runtime authorities (W10-3/W10-5) and either extending the existing tier-selector-authority state model OR introducing a new one — both surfaces are outside Phase 11's core scope-tightening axis and belong in a separate slice where they can be reviewed against the DEC-PHASE9-002 single-authority preservation rule head-on. Payload (4) depends on (3) landing first. **This planner amendment decouples them, mirroring the W11-3 Layer A / Layer B, W11-4 Layer A / Layer B, and W11-5 Layer A / Layer B precedents seeded on the same day** (see W11-3 at line 3250, W11-4 at line 3346, W11-5 at line 3424: same rationale, same split shape, same DEC-preserved-verbatim discipline). Layer A (this slice) lands payload (1) + skeleton `/var/lib/suricata/orionx-README.md` documenting the Layer A/B split + `scripts/orionx-freshen-suricata.sh` (mirrors `orionx-freshen-yara.sh` naming + shape) + 0700 hook symlink + content-presence Section 24 asserting all of the above + a NEGATIVE assertion that `/var/lib/suricata/rules/` does NOT exist yet (Layer B sentinel), plus a CHANGELOG entry naming Layer B as deferred. Layer B (a follow-up W11-6b slice, seeded at the end of this section) lands payloads (2) + (3) + (4) + expanded Section 24 assertions covering the rules directory + Nebula MCP adapter + tier-selector state extension, and flips the negative sentinel. **The mainline W11-6 row policy is preserved verbatim** — the split is a landing-cadence choice, not a policy change; the same Suricata + tier-selector bundle lands in the same v2.1.0, just across two commits instead of one. DEC-PHASE11-008 is preserved BYTE-IDENTICAL and DEC-PHASE9-002's tier-selector single-authority rule is preserved BYTE-IDENTICAL — no policy changes to the mainline row's decision surface.

**State-authority map (single-authority discipline — Sacred Practice #12):**

| State domain | Canonical authority (Layer A) | Consumers / cross-references |
|---|---|---|
| Suricata package presence | `iso/config/package-lists/orionx.list.chroot` (one new line: `suricata`) | Debian `apt-get install` in live-build chroot; content-presence 24a asserts dpkg-status contains `suricata`; Phase 11 reconciliation table row line 2364 (+35 MB accounted). |
| Suricata systemd lazy-start policy | `iso/config/includes.chroot/etc/systemd/system/suricata.service.d/orionx-lazy.conf` — sets `[Unit] ConditionPathExists=/var/lib/suricata/orionx-enabled` so the shipped `suricata.service` (Type=notify from the Debian package) is a no-op until an operator (or a future W11-6b tier-selector daemon) creates the sentinel file. | systemd at boot; content-presence 24b asserts override file present + contains `ConditionPathExists=/var/lib/suricata/orionx-enabled`; W11-6b will provision the sentinel file and the tier-selector daemon that touches/removes it. |
| Suricata runtime data directory (skeleton) | `iso/config/includes.chroot/var/lib/suricata/orionx-README.md` — documents the Layer A/B split, names DEC-PHASE11-008, points at `orionx-freshen-suricata.sh` for post-boot rules download. The `/var/lib/suricata/` path itself IS created by the `suricata` Debian package's postinst; Layer A only stages a marker README under that path via includes.chroot (live-build copies includes.chroot AFTER packages install, so the README lands cleanly inside the Debian-created dir). | Operators; content-presence 24c; W11-6b will populate `/var/lib/suricata/rules/` at build time. |
| Orion-X freshen scripts (naming convention) | `scripts/orionx-freshen-suricata.sh` — mirrors `scripts/orionx-freshen-yara.sh` (landed W11-4 Layer A, line 84 of the 0700 hook). Structure: `#!/usr/bin/env bash`, `set -euo pipefail`, `@decision DEC-PHASE11-008` header, root-only check, network-refuse on air-gap (`getent hosts rules.emergingthreats.net`), LOUD-fail on missing `/var/lib/suricata/` (Debian package should have created it — if missing, the environment is broken and the operator needs to reinstall suricata), downloads latest ET-Open rules tarball from `https://rules.emergingthreats.net/open/suricata-6.0.0/emerging.rules.tar.gz` (BSD-2-Clause), extracts under `/var/lib/suricata/rules/`, prints a one-line hint about running `suricata -T -c /etc/suricata/suricata.yaml` to validate. | Operators post-boot; content-presence 24d asserts script staged + executable + `/usr/bin/orionx-freshen-suricata` symlink present. |
| 0700 hook PATH symlinks | `iso/config/hooks/live/0700-orionx-setup.hook.chroot` — the SCRIPT_MAP array (currently ends at line 84 with `orionx-freshen-yara`). One new entry added: `orionx-freshen-suricata -> /opt/orionx/scripts/orionx-freshen-suricata.sh`. Byte-identical to the W11-4 pattern; NO other edits to the hook body. | Live-build hook execution; content-presence 24d verifies the resulting symlink. |
| Content-presence test sections | `tests/integration/test-iso-content-presence.sh` — a new Section 24 appended after Section 23 (W11-5 Layer A, currently ends at line ~1929). Section-header banner + `section "24. W11-6 Layer A: Suricata IDS skeleton + freshen script (section 20 per plan numbering, renumbered to 24 per insertion order — DEC-PHASE11-008)"` line, following the W11-9a numbering-comment precedent at line 1067. Six assertions (24a-24f) per the EC below. | CI; the section number in echo output uses `20` to match the plan-line-2388 numbering and DEC-PHASE11-008's provision-time framing; the file-insertion order number is `24` matching the `section()` call convention. |
| CHANGELOG v2.1.0 | `CHANGELOG.md` `## [v2.1.0] - Unreleased` section — a new `### W11-6 Layer A: Suricata IDS skeleton + lazy-start override + freshen script (rules snapshot + tier selector deferred to W11-6b)` subsection appended after the existing W11-5 Layer A entry. Names: `suricata` package, `orionx-lazy.conf` systemd override + `ConditionPathExists` sentinel semantics, `/var/lib/suricata/orionx-README.md`, `scripts/orionx-freshen-suricata.sh`, 0700 hook symlink, content-presence Section 24, and W11-6b explicitly named as deferred (ET-Open rules snapshot at build time + Nebula tier-selector runtime wiring + eve.log → Nebula MCP adapter). No DEC change. | Operators reading release notes; W11-10 rc-cut references. |

**Scope Manifest — allowed / required / forbidden (canonical file at `tmp/phase11-w11-6-suricata-scope.json`; runtime scope-triad at `tmp/phase11-w11-6-suricata-scope-runtime.json`, synced to runtime):**

| Path | Class | Rationale |
| --- | --- | --- |
| `iso/config/package-lists/orionx.list.chroot` | allowed + required | Add one line: `suricata` (Debian Bullseye 6.0.x main). Under an appropriate section-comment banner (`# W11-6 Layer A: Suricata IDS (DEC-PHASE11-008) — lazy-start behind /var/lib/suricata/orionx-enabled sentinel`). Count of non-comment non-blank lines increases by exactly 1 vs the file at the slice's base HEAD. No other packages added or removed. |
| `iso/config/includes.chroot/etc/systemd/system/suricata.service.d/orionx-lazy.conf` | required | New file (drop-in override). Contents: `[Unit]` section with `ConditionPathExists=/var/lib/suricata/orionx-enabled` and a header comment naming DEC-PHASE11-008 + W11-6 Layer A. Roughly 15-25 lines including comment header. The `suricata.service.d/` directory is a NEW path — Layer A creates it via includes.chroot staging (live-build copies the tree verbatim). Do NOT touch the shipped `/lib/systemd/system/suricata.service` — override files are the correct single-authority mechanism for tier-selector policy per systemd conventions. |
| `iso/config/includes.chroot/var/lib/suricata/orionx-README.md` | required | New file. Documents: purpose of `/var/lib/suricata/` (Suricata's runtime data + rules dir per Debian packaging convention); Layer A shipped items (package + override + this README + freshen script); Layer B deferred items (ET-Open rules snapshot at build time + Nebula tier-selector runtime wiring + eve.log → Nebula MCP adapter); ConditionPathExists sentinel semantics (operator `touch /var/lib/suricata/orionx-enabled && systemctl start suricata` to opt into Tier 1); pointer to `sudo orionx-freshen-suricata` for post-boot ET-Open rules download; DEC-PHASE11-008 reference; explicit `Layer A / Layer B` marker phrase for the content-presence grep. Roughly 50-80 lines. |
| `scripts/orionx-freshen-suricata.sh` | required | New file. Structure mirrors `scripts/orionx-freshen-yara.sh` (line-by-line reference for the implementer): `#!/usr/bin/env bash`, `set -euo pipefail`, header comment block with `@decision DEC-PHASE11-008`, `@title Orion-X Suricata ET-Open ruleset freshen script`, `@status accepted`, `@rationale` naming the Layer A/B split. Body: (i) EUID root check with error message; (ii) network-refuse check via `getent hosts rules.emergingthreats.net`; (iii) LOUD-fail if `/var/lib/suricata/` missing (Debian package should have created it); (iv) `curl -fL --retry 3 -o /tmp/emerging.rules.tar.gz https://rules.emergingthreats.net/open/suricata-6.0.0/emerging.rules.tar.gz`; (v) `tar -xzf /tmp/emerging.rules.tar.gz -C /var/lib/suricata/` (extracts `rules/` subdirectory per the tarball's own layout); (vi) cleanup of `/tmp/emerging.rules.tar.gz`; (vii) one-line hint about running `suricata -T -c /etc/suricata/suricata.yaml` to validate. `chmod +x` at file creation. Verify: `bash -n scripts/orionx-freshen-suricata.sh` passes; `shellcheck -S error scripts/orionx-freshen-suricata.sh` exits 0. |
| `iso/config/hooks/live/0700-orionx-setup.hook.chroot` | allowed + required | Add one new entry to the `SCRIPT_MAP` associative array, immediately after the existing `orionx-freshen-yara` entry (currently line 84). Section-header comment banner following the W11-4 precedent: `# W11-6 Layer A: Suricata ET-Open ruleset freshen script (DEC-PHASE11-008). / # Operators run \`sudo orionx-freshen-suricata\` post-boot to populate /var/lib/suricata/rules/. / # Layer B (W11-6b) will automate this at build time. Single PATH entry; no .desktop launcher / # (this is a root-only maintenance command, not an end-user GUI tool).` Then: `["orionx-freshen-suricata"]="/opt/orionx/scripts/orionx-freshen-suricata.sh"`. NO other edits to the hook — no touching the existing symlinks, no touching the .desktop entries section, no touching the MOTD section, no touching the DEC-PHASE8-005 or DEC-PHASE9-001 header comments (byte-identical preservation). |
| `tests/integration/test-iso-content-presence.sh` | required | Append-only NEW Section 24 block after the existing Section 23 (W11-5 Layer A) block. Section-header comment banner following the W11-9a numbering convention (line 1067 precedent): `# Section label note: section 20 per plan numbering (line 2388 W11-6 slot), renumbered to 24 per insertion order matching the W11-9a precedent.` Then `section "24. W11-6 Layer A: Suricata IDS skeleton + lazy-start override + freshen script (section 20 — DEC-PHASE11-008)"`. Six assertions (24a-24f) per the EC. Preserve the file's `pass`/`fail` helper convention verbatim. |
| `CHANGELOG.md` | required | Append a `### W11-6 Layer A: Suricata IDS skeleton + lazy-start override + freshen script (ET-Open rules snapshot + Nebula tier selector deferred to W11-6b)` subsection under `## [v2.1.0] - Unreleased`, immediately after the existing W11-5 Layer A entry. Names all shipped items + W11-6b deferral explicitly. NO DEC change. |
| `MASTER_PLAN.md` | allowed | This planner amendment (the section you are reading now). Layer A implementer must NOT edit MASTER_PLAN.md beyond appending the Layer A iter-1 trailer if planner asks; guardian:land appends the landed-at SHA to the trailer (Phase 8/9/10/11 landing-record precedent). |
| `tmp/**` | allowed | Scope manifest files + any implementer scratch artifacts. |
| `iso/config/nebula-model-manifest.json` | forbidden | This is the W10-1/W11-1 Nebula authority. Layer A does NOT touch the Nebula manifest. Any Nebula MCP adapter registration is W11-6b concern. |
| `iso/config/hooks/live/{0100,0500,0600,0610,0615,0620,0800}-*.hook.chroot` | forbidden | Only 0700 is touched (for the freshen symlink). The 0500 hook is NOT touched because Suricata is a Debian package installed by the standard package list mechanism, not a pip/tarball fetch. If a future reader believes the 0500 hook needs a Suricata-specific soft-fail block, that is an out-of-scope escalation to planner. |
| `iso/config/includes.chroot/opt/orionx/**` | forbidden | Layer A stages ZERO content under `/opt/orionx/`. Suricata lives at Debian's standard `/etc/suricata/` (package-owned, not touched by Layer A) + `/var/lib/suricata/` (package-owned dir + our README). The freshen script lives at `/opt/orionx/scripts/orionx-freshen-suricata.sh` via the SAME staging mechanism as `orionx-freshen-yara.sh` — but that staging is done by `stage_application_content()` in `scripts/build-iso.sh` (build-host stage, NOT this slice's concern), NOT by an includes.chroot copy. Layer A only edits the 0700 hook's SCRIPT_MAP and the script file itself. |
| `iso/config/includes.chroot/etc/matrix-synapse/**`, `etc/wireguard/**`, `etc/apparmor.d/**`, `etc/plymouth/**`, `etc/lightdm/**`, `etc/skel/**`, `etc/xdg/**` | forbidden | Adjacent Phase 3/4/9/10/11 authorities. No cross-slice edits. |
| `iso/config/includes.chroot/usr/share/{plymouth,grub/themes,themes,lightdm-gtk-greeter,applications}/**` | forbidden | W11-9a/b/c and W11-4/W11-5 XDG authorities. Suricata is a headless daemon; no `.desktop` launcher (this is a root-only systemd service, not an end-user GUI tool). |
| `iso/config/includes.binary/**`, `iso/auto/**` | forbidden | Boot chain / live-build config. Not touched. |
| `scripts/build-iso.sh` | forbidden | Build-host stage authority (DEC-PHASE11-012). The freshen script is staged automatically because `stage_application_content()` rsyncs `scripts/*.sh` into includes.chroot at build time — no build-iso.sh edit needed. If a future reader believes an explicit stage entry is required, verify by inspecting `stage_application_content()` first: the W11-4 orionx-freshen-yara.sh landed without any build-iso.sh edit, and Layer A relies on the same pathway. |
| `scripts/{orionx-freshen-yara.sh,nebula/**,mesh/**,control_center/**,security/**,release/**}` | forbidden | Sibling script authorities. Do NOT touch orionx-freshen-yara.sh (this slice creates a sibling, does not edit the yara one). |
| `runtime/**`, `hooks/**`, `agents/**`, `settings.json`, `CLAUDE.md`, `AGENTS.md`, `DECISIONS.md`, `.github/workflows/**` | forbidden | Control-plane / governance authorities. Not touched. |
| `tests/integration/{test-iso-hooks-applied.sh,test_build_iso.sh}` | forbidden | Sibling test authorities not exercised by Layer A. Only `test-iso-content-presence.sh` is edited. |

**Task list (T1-T6, appended verbatim into implementer briefing at provision):**

1. **T1 — Package list.** Edit `iso/config/package-lists/orionx.list.chroot`. Add one new line reading `suricata` under a section-header comment banner: `# W11-6 Layer A: Suricata IDS (DEC-PHASE11-008) — lazy-start behind /var/lib/suricata/orionx-enabled sentinel per orionx-lazy.conf drop-in.` Place the entry in a spot consistent with the file's existing organization (e.g. near other network/monitoring tools). Verify: file parses as valid live-build package-list (one package per line, `#`-prefixed comments, no blank-line syntax errors); count of non-comment non-blank lines increases by exactly 1 vs the file at the slice's base HEAD. Commit: `feat(w11-6): add suricata to package list (Layer A)`.

2. **T2 — systemd lazy-start override.** Create `iso/config/includes.chroot/etc/systemd/system/suricata.service.d/orionx-lazy.conf` (the parent dirs are new — includes.chroot staging creates them automatically at build time). Contents: header comment block naming DEC-PHASE11-008 + W11-6 Layer A + the sentinel-path semantics; `[Unit]` section with `ConditionPathExists=/var/lib/suricata/orionx-enabled`. Roughly 15-25 lines. NO other keys — do not add `[Service]` overrides, do not add `[Install]` overrides; the Debian-shipped `suricata.service` is the authority for those. Verify: `systemd-analyze verify` should be a no-op (cannot fully verify a drop-in without a running systemd; content-presence 24b is the acceptance gate); `grep -c 'ConditionPathExists=/var/lib/suricata/orionx-enabled' iso/config/includes.chroot/etc/systemd/system/suricata.service.d/orionx-lazy.conf == 1`. Commit: `feat(w11-6): stage suricata.service.d/orionx-lazy.conf lazy-start override (Layer A)`.

3. **T3 — /var/lib/suricata/orionx-README.md skeleton.** Create `iso/config/includes.chroot/var/lib/suricata/orionx-README.md`. Content: title `# Orion-X Suricata IDS — Layer A skeleton`; one-sentence mission rationale citing the DEC-PHASE11-008 mission-verb `detect` gap fill; `## Shipped in base (Layer A, this v2.1.0)` section listing suricata package + lazy-start override + this README + freshen script + 0700 hook symlink; `## Deferred to W11-6b (Layer B)` section listing ET-Open rules snapshot (build-time git-clone-equivalent) + Nebula tier-selector runtime wiring (Tier 0 → Tier 1 → Tier 2) + eve.log → Nebula MCP adapter; `## ConditionPathExists sentinel semantics` section explaining `touch /var/lib/suricata/orionx-enabled && systemctl start suricata` opt-in; `## Freshen rules` section pointing at `sudo orionx-freshen-suricata`; `## Related` pointing at `/etc/suricata/suricata.yaml` (Debian package authority — do not edit) and DEC-PHASE9-002 tier-selector plan (referenced but not activated in Layer A). Include the `Layer A / Layer B` marker phrase explicitly so content-presence assertion 24c can grep for it. Roughly 50-80 lines. Commit: `feat(w11-6): stage /var/lib/suricata/orionx-README.md skeleton (Layer A)`.

4. **T4 — orionx-freshen-suricata.sh.** Create `scripts/orionx-freshen-suricata.sh`. Structure mirrors `scripts/orionx-freshen-yara.sh` byte-for-byte on the header pattern, root check, network check, and error idioms; body differs per the state-authority-map row above (ET-Open tarball download + extract instead of git clone loop). `chmod +x`. Verify: `bash -n scripts/orionx-freshen-suricata.sh` passes; `shellcheck -S error scripts/orionx-freshen-suricata.sh` exits 0; `test -x scripts/orionx-freshen-suricata.sh` passes. Commit: `feat(w11-6): add scripts/orionx-freshen-suricata.sh — ET-Open rules freshen (Layer A)`.

5. **T5 — 0700 hook SCRIPT_MAP entry.** Edit `iso/config/hooks/live/0700-orionx-setup.hook.chroot`. Immediately after the existing `["orionx-freshen-yara"]="/opt/orionx/scripts/orionx-freshen-yara.sh"` entry (current line 84), add a comment block + entry mirroring the W11-4 shape (see Scope Manifest row above for exact wording). NO other edits — DEC-PHASE8-005 + DEC-PHASE9-001 headers preserved byte-identical, all other SCRIPT_MAP entries unchanged, .desktop and MOTD sections untouched. Verify: `bash -n iso/config/hooks/live/0700-orionx-setup.hook.chroot` passes; `shellcheck -S error iso/config/hooks/live/0700-orionx-setup.hook.chroot` exits 0; `grep -c 'orionx-freshen-suricata' iso/config/hooks/live/0700-orionx-setup.hook.chroot == 1`. Commit: `feat(w11-6): 0700 hook — orionx-freshen-suricata PATH symlink (Layer A)`.

6. **T6 — Content-presence Section 24 + CHANGELOG + planner-iter trailer.** (a) Edit `tests/integration/test-iso-content-presence.sh`. Append a Section 24 block after the existing Section 23 block. Six assertions per the EC below. (b) Edit `CHANGELOG.md`: append the W11-6 Layer A subsection under `## [v2.1.0] - Unreleased`, immediately after the W11-5 Layer A entry. (c) Edit `MASTER_PLAN.md` to append the W11-6b seed row + iter-1 trailer inside this section (identical structure to the W11-3b/W11-4b/W11-5b seeds; see W11-6b seed spec below). Fold into one commit: `docs(w11-6): CHANGELOG v2.1.0 + content-presence section 24 + W11-6b Layer B seeded + iter-1 trailer (Layer A)`.

**Evaluation Contract (6 items — matches the user-supplied contract verbatim; reviewer + guardian gate against these):**

1. **EC1: `suricata` Debian package in list.** `iso/config/package-lists/orionx.list.chroot` contains a new line for the `suricata` package. Count of non-comment non-blank lines increases by exactly 1 vs the file at the slice's base HEAD. No other packages added or removed. Content-presence 24a asserts the built ISO squashfs `/var/lib/dpkg/status` contains a `Package: suricata` stanza.
2. **EC2: systemd lazy-start override present.** `iso/config/includes.chroot/etc/systemd/system/suricata.service.d/orionx-lazy.conf` present. Content contains `ConditionPathExists=/var/lib/suricata/orionx-enabled` on a line under a `[Unit]` section header. Content-presence 24b asserts file present in squashfs at `/etc/systemd/system/suricata.service.d/orionx-lazy.conf` AND grep for the ConditionPathExists line succeeds.
3. **EC3: `/var/lib/suricata/orionx-README.md` skeleton present.** File present in squashfs at `/var/lib/suricata/orionx-README.md`. Content includes the phrases `DEC-PHASE11-008` AND `Layer A / Layer B` (marker phrase for the content-presence grep). Names ET-Open ruleset snapshot as Layer B (W11-6b) deferred, and names Nebula tier-selector runtime wiring as W11-6b deferred. Content-presence 24c matches.
4. **EC4: `scripts/orionx-freshen-suricata.sh` present + executable + shellcheck-clean.** File present at `scripts/orionx-freshen-suricata.sh`; `test -x` passes; `shellcheck -S error scripts/orionx-freshen-suricata.sh` exits 0; staged into squashfs at `/opt/orionx/scripts/orionx-freshen-suricata.sh` (via the existing `stage_application_content()` rsync); symlinked to `/usr/bin/orionx-freshen-suricata` in squashfs via the 0700 hook SCRIPT_MAP addition. Content-presence 24d asserts all three (script staged, executable bit set, symlink present + resolves into `/opt/orionx/scripts/`).
5. **EC5: Content-presence Section 24 assertions (six items).** `tests/integration/test-iso-content-presence.sh` gains Section 24 with assertions 24a-24f: (24a) `Package: suricata` in dpkg-status; (24b) `orionx-lazy.conf` present + `ConditionPathExists` line; (24c) `/var/lib/suricata/orionx-README.md` present + marker phrase `Layer A / Layer B` + `DEC-PHASE11-008`; (24d) `/opt/orionx/scripts/orionx-freshen-suricata.sh` present + executable + `/usr/bin/orionx-freshen-suricata` symlink present + resolves; (24e) NEGATIVE: `/var/lib/suricata/rules/` does NOT exist in the squashfs (Layer B sentinel — flipped to POSITIVE at W11-6b landing time); (24f) NEGATIVE: `/var/lib/suricata/orionx-enabled` does NOT exist in the squashfs (operator-only opt-in sentinel — Layer A ships the mechanism, not the activation). When run against the built ISO squashfs, all six assertions pass. The full test binary (`bash tests/integration/test-iso-content-presence.sh`) exits 0.
6. **EC6: CHANGELOG entry.** `CHANGELOG.md` `## [v2.1.0] - Unreleased` section gains the W11-6 Layer A subsection. Names the `suricata` package, systemd lazy-start override + ConditionPathExists sentinel semantics, `/var/lib/suricata/orionx-README.md` skeleton, `orionx-freshen-suricata.sh` freshen script, 0700 hook symlink, content-presence Section 24 (6 assertions), and W11-6b deferral explicitly (ET-Open rules snapshot at build time + Nebula tier-selector runtime wiring + eve.log → Nebula MCP adapter).

**Ready-for-guardian definition:** All six EC items pass on the reviewer's evaluation of the ISO build artifact; the reviewer emits `REVIEW_VERDICT: ready_for_guardian` with the head SHA; test-state is `pass_complete`; guardian lease active. Soft-fail semantics: Layer A ships ONLY a skeleton — there is no build-time network fetch, so PyPI/network transient outages cannot break this slice's CI (unlike W11-5 Layer A's matrix-commander pip install). If `suricata` fails to install from Debian main, that is a Debian mirror outage — LOUD-fail is correct behavior (the reviewer verdict is `needs_changes` and the build is retried when the mirror recovers). No opt-in occurs until an operator creates `/var/lib/suricata/orionx-enabled` post-boot; this is intentional per DEC-PHASE9-002 tier-selector plan (Tier 0 = Suricata installed but idle).

**Forbidden shortcuts (explicitly banned):**

- Do NOT ship the ET-Open ruleset snapshot bundled in the ISO in Layer A. That is Layer B (W11-6b) per the mainline W11-6 row's own "planner defers final license check to provision-time" wording. Adding it here would inflate the Layer A blast radius by adding a build-time network-fetch cascade (rules.emergingthreats.net or the `emerging-threats/ETOpenRuleSet` GitHub mirror) exactly the coupling the Layer A / Layer B split is designed to prevent.
- Do NOT wire Suricata's `eve.log` JSON output to the Nebula detection contextualizer in Layer A. That is Layer B (W11-6b). Layer A ships the DAEMON POLICY (lazy-start override) but does NOT plumb the alert-output path — the Nebula MCP adapter for eve.log tail is W10-3/W10-5-adjacent surface that requires its own scope review.
- Do NOT extend or introduce a new tier-selector state authority in Layer A. The mainline W11-6 row says `does NOT introduce a new authority — DEC-PHASE9-002 preserved` — Layer A honors this by shipping ONLY the sentinel-file mechanism (`ConditionPathExists=/var/lib/suricata/orionx-enabled`) without shipping the sentinel file itself or any daemon that manages it. The tier selector's state model extension lands with W11-6b.
- Do NOT touch `iso/config/nebula-model-manifest.json` or any file under `scripts/nebula/` or `iso/config/includes.chroot/opt/orionx/nebula/`. Nebula wiring is W11-6b concern.
- Do NOT ship the `/var/lib/suricata/orionx-enabled` sentinel file itself in Layer A. Shipping it would flip Suricata to auto-start on first boot, defeating the lazy-start policy. Operators opt in post-boot; content-presence 24f asserts the sentinel is ABSENT.
- Do NOT create a `.desktop` launcher for Suricata. It is a headless systemd service, not an end-user GUI tool. `orionx-freshen-suricata` is a root-only maintenance command per the W11-4 freshen-script precedent — no .desktop entry.
- Do NOT edit the Debian-shipped `/etc/suricata/suricata.yaml`. The Debian package is authoritative for daemon configuration; Layer A only ships a systemd unit override. Any yaml customization is a Layer B / W11-6b concern with its own scope review.
- Do NOT modify `DECISIONS.md` directly; it is auto-generated from `@decision` annotations by `stop.sh`.
- Do NOT introduce a new live-build hook file. The 0700 SCRIPT_MAP extension is the only mechanism for Layer A. `orionx-freshen-yara`'s W11-4 landing established this pattern; W11-6 Layer A follows it byte-for-byte.
- Do NOT combine this slice with W11-3b, W11-4b, W11-5b, W11-6b, W11-7, or W11-8 — each is its own workflow.
- Do NOT edit `.github/workflows/**` — not in scope. Content-presence Section 24 runs under the existing CI job; no runner changes needed.
- Do NOT edit `scripts/build-iso.sh`. The freshen script is picked up by `stage_application_content()` automatically (W11-4's freshen script landed the same way without a build-iso.sh edit). If the implementer discovers otherwise at build time, that is an out-of-scope escalation to planner.
- Do NOT edit `MASTER_PLAN.md` beyond the two allowed edits: the W11-6b Layer B seed row inside this section (T6b), and the W11-6 iter-1 trailer at the end of this section (T6c). Do NOT touch the mainline W11-6 row at line 2403, the Phase 11 reconciliation table row at line 2364 or line 2370, the whole-slice EC line 2388 mentioning section 20, or any Decision Log row.

**Downstream consumers (what this slice unblocks):**

- **W11-6b** (Layer B) — ET-Open rules snapshot at build time + Nebula tier-selector runtime wiring (Tier 0 → Tier 1 → Tier 2 per DEC-PHASE9-002) + eve.log → Nebula MCP adapter + expanded Section 24 assertions covering the rules dir + tier-selector daemon + Nebula MCP registry entry + LOCKFILE-style SHA-256 pin for the ET-Open snapshot (trust-on-first-use per DEC-PHASE10-008 precedent). Seeded below.
- **W9-3** — the sketched threat-posture / paranoia-level subsystem (issue #46) is fulfilled by the W11-6 Layer A + Layer B combined landing. Layer A ships the detection mechanism (daemon + skeleton); Layer B ships the tier-selector wiring. Issue #46 closes at W11-6b landing time (not at Layer A landing, because the tier-selector is the sketch's core deliverable and it does not exist yet in Layer A).
- **W10-5** (AI-augmented detection daemon) — will consume Suricata's `eve.log` as one input class to the Nebula detection contextualizer; the eve.log → Nebula MCP adapter is provisioned in W11-6b. Layer A stages the daemon; Layer B stages the plumbing.
- **W11-10** (rc-cut + hardware attestation) — will verify Suricata is installed AND idle at boot (no CPU usage from a suricata process, no active packet capture) — the lazy-start ConditionPathExists sentinel is the mechanism that makes this true. Layer A's content-presence 24b + 24f assertions are the CI-time evidence; hardware attestation is the runtime evidence.

**W11-6b — Layer B seed row (SEEDED, not planned):**

Deferred payloads: (i) build-time fetch of an ET-Open ruleset snapshot from `https://rules.emergingthreats.net/open/suricata-6.0.0/emerging.rules.tar.gz` (BSD-2-Clause; re-verify licensing at W11-6b provision-time — planner note: BSD-2-Clause has been stable on this feed since 2011, but ProofPoint could re-license upstream at any time) into `iso/config/includes.chroot/var/lib/suricata/rules/` under an appropriate 0500-hook block or a new 0605 build-time-fetch hook (implementer to choose, matching the closest existing precedent — likely the ollama `.tar.zst` fetch pattern at 0500 line 62); (ii) SHA-256 pin via trust-on-first-use per DEC-PHASE10-008 precedent (LOCKFILE-style pin file or baked into the 0500 hook's fetch block); (iii) Nebula tier-selector runtime wiring: a lightweight daemon or systemd oneshot that reads a runtime tier state (`0`/`1`/`2`) and toggles `/var/lib/suricata/orionx-enabled` accordingly — landing surface is either a new `scripts/nebula/tier-selector.py` (extending the Nebula CLI dispatcher pattern from W10-1) or a standalone `orionx-tier-selector` script; (iv) eve.log → Nebula MCP adapter: a new MCP server that tails `/var/log/suricata/eve.json` and emits alerts as MCP tool responses to the W10-3 Nebula MCP registry (mirrors the remnux-mcp-server stdio LOCAL MCP pattern at W11-3); (v) content-presence Section 24 assertions 24e + 24f flipped from NEGATIVE to POSITIVE (24e: rules dir present + at least one `.rules` file + LOCKFILE SHA matches; 24f: sentinel remains ABSENT at ISO ship time — it's still operator opt-in at boot, but the tier-selector daemon is present and can toggle it under Nebula's direction); (vi) new content-presence assertions 24g, 24h, 24i covering the tier-selector daemon + MCP adapter registration + LOCKFILE presence; (vii) CHANGELOG v2.1.0 W11-6b entry naming all of the above + closing GitHub issue #46 (W9-3 threat-posture / paranoia-level subsystem). Landing constraints: the rules.emergingthreats.net feed must be reachable from GitHub Actions runners at build time (verified stable at planner time; trust-on-first-use handles transient outages via LOUD-fail-then-retry — same pattern as ollama); the Nebula tier-selector daemon must be small enough to fit inside the Phase 11 ISO size budget (target: ~10 KB of Python — negligible); the MCP adapter must register with the W10-3 Nebula MCP registry without introducing a new authority (registry authority preserved). SoftFail vs LoudFail decision: rules snapshot fetch is LOUD-fail (a shipped Suricata with no rules is a footgun, per the same rationale as the W11-1 Nebula manifest LOUD-fail); tier-selector daemon activation is soft-fail (system can boot without it; operator can still `touch /var/lib/suricata/orionx-enabled` manually). ISO size delta target is +15 MB per the Phase 11 reconciliation table (line 2370). Tracking mechanism: GitHub issue filed by guardian:land at Layer A landing time (title: `W11-6b: Layer B — ET-Open rules snapshot + Nebula tier selector + eve.log MCP adapter (defer from W11-6; closes #46)`; labels: `phase-11`, `w11-6b`, `priority:medium`, `deferred-from:w11-6`, `closes-46`). If for any reason guardian:land cannot file the issue, an inline TODO under this seed row is the fallback record; the operator triages via the backlog skill at next dispatch.

**Risk register (W11-6-specific):**

- **R1 — Debian package availability.** `suricata` 6.0.x is in Bullseye main (verified at planner time). If the Debian mirror is unreachable at build time, live-build's `apt-get install` LOUD-fails — the whole ISO build errors out. **Mitigation:** none needed at Layer A — this is the correct failure mode (a broken mirror should not ship a broken ISO). Retry the build when the mirror recovers.
- **R2 — systemd override drop-in path.** `/etc/systemd/system/<unit>.d/*.conf` is systemd's canonical override mechanism for shipping distribution-provided units. Verified at planner time: Debian's `suricata` package ships `/lib/systemd/system/suricata.service` (as `Type=simple` on Bullseye 6.0.x, NOT `Type=notify` as the mainline W11-6 row's wording suggested — planner acknowledges the row wording is aspirational, and the drop-in is authoritative regardless of the base unit's Type= value). **Mitigation:** T2 requires only a `[Unit]` `ConditionPathExists=` line — no `Type=` override — so the base unit's Type= is not touched. If a future reader believes `Type=notify` needs enforcing, that is a Layer B / W11-6b concern with its own scope review.
- **R3 — Sentinel path race.** If a caller runs `systemctl start suricata` before the sentinel file exists, the `ConditionPathExists` check fires and the start is a no-op — systemd reports success but the daemon does not run. This is INTENTIONAL (Tier 0 = installed but idle), but a naive operator may believe Suricata is running when it is not. **Mitigation:** T3 README explicitly names the ConditionPathExists sentinel semantics + gives the operator the exact `touch /var/lib/suricata/orionx-enabled && systemctl start suricata` command. This is the correct discoverable pattern; hiding the sentinel behind a wrapper CLI is a Layer B concern.
- **R4 — Content-presence assertion 24a — dpkg-status parsing.** Assertion 24a asserts `Package: suricata` in the built ISO squashfs `/var/lib/dpkg/status`. If Debian's dpkg-status format changes (unlikely on Bullseye stable) or if live-build strips dpkg-status (does not, verified), the assertion fires false-positive/negative. **Mitigation:** T6 uses the SAME `grep -F "Package: <name>"` idiom the W11-3 Layer A + W11-4 Layer A tests use — a fault in that idiom would break the existing landed tests, not just this one, and would be caught by the CI regression signal.
- **R5 — `orionx-freshen-suricata` shellcheck cleanliness.** The freshen script downloads a tarball from a static URL — if the URL scheme (currently `https://rules.emergingthreats.net/open/suricata-6.0.0/emerging.rules.tar.gz`) shifts to a newer Suricata major version (e.g. `suricata-7.0.0`), the download 404s and the script fails silently at the extract step. **Mitigation:** T4's LOUD-fail via `set -euo pipefail` + `curl -f` means a 404 exits non-zero and the script errors out visibly; operators can escalate to planner for a URL update. Note: Debian Bullseye ships Suricata 6.0.x; when the base ISO is rebased to Debian Bookworm (Suricata 7.0.x), the URL pins in W11-6b's LOCKFILE need updating — planner will re-verify at the Debian rebase slice, whenever that is scheduled.

**W11-6 iter-1 (planner detail-plan): 2026-07-19.** Detail plan appended to `MASTER_PLAN.md` on `develop`; NO new DEC (DEC-PHASE11-008 Suricata authority reused verbatim; DEC-PHASE9-002 tier-selector single-authority rule preserved verbatim; DEC-PHASE10-007 soft-fail pattern referenced conceptually but NOT reused — Suricata is a Debian apt install, not a pip/tarball fetch, so LOUD-fail is correct); Scope Manifest (audit shape) at `tmp/phase11-w11-6-suricata-scope.json`; runtime scope-triad at `tmp/phase11-w11-6-suricata-scope-runtime.json` (synced to runtime via `cc-policy workflow scope-sync` — 3 paths written, status=ok). Ready for `guardian:provision` under workflow_id `phase11-w11-6-suricata`.

---

#### W11-7 — Detail plan (ClamAV drop-to-optional MICRO-SLICE — remove clamav from base ISO + author install-clamav.sh operator stub; Control Center Awareness badge deferred to W11-8)

**Seeded:** 2026-07-19 planner amendment. **Status:** ready for `guardian:provision`. **Env:** GitHub Actions `ubuntu-latest` + `debian:bullseye` build container; content-presence is the acceptance gate. **Wave:** 5 of Phase 11 (per critical-path row at line 2409). **Gate:** review (mainline row's `approve` gate — approve boundary is DEC-PHASE11-009 sign-off, which is ALREADY LOCKED 2026-07-02; no live user gate remains for the operator-side sign-off, so the effective gate for this landing cadence is `review`). **Weight:** S (per mainline row). **Deps:** W11-2 (landed at `9fe8c4b`; W11-2b `98dfc2e`, W11-2c `0838a31`, W11-2d `7759e84`, W11-2e `c4d78d1`, W11-2f landed). Independent of W11-3 Layer A, W11-4 Layer A, W11-5 Layer A, W11-6 Layer A landings (this slice touches an orthogonal Debian package + a NEW top-level optional-installer path). **Workflow ID:** `phase11-w11-7-clamav-drop`. **Scope Manifest:** `tmp/phase11-w11-7-clamav-drop-scope.json` (audit/full-shape manifest); runtime scope-triad at `tmp/phase11-w11-7-clamav-drop-scope-runtime.json` (synced to runtime via `cc-policy workflow scope-sync` — 3 paths written, status=ok).

**Trigger.** DEC-PHASE11-009 LOCKED 2026-07-02 (operator sign-off: drop ClamAV from the base ISO, defer to `/opt/orionx/optional/install-clamav.sh`). All 11 Phase 11 decisions are pre-locked; the mainline W11-7 row at line 2404 owes its landing to the plan. This is the trivial MICRO-SLICE that discharges that landing: three edits (package-list line removal, installer script authoring, content-presence Section 25). The Control Center Awareness pane badge named in the mainline row is EXPLICITLY W11-8-owned per DEC-PHASE11-011's optional-installer-framework authority; W11-7 stages the FIRST installer under the `/opt/orionx/optional/` layout convention but does not author the framework README, sentinel-file mechanism, or Awareness-pane wiring — those land as one cohesive W11-8 slice.

**Slice intent.** Remove one line (`clamav`) from `iso/config/package-lists/orionx.list.chroot`. Author `iso/config/includes.chroot/opt/orionx/optional/install-clamav.sh` as a root-run, network-required, apt-get-driven installer stub with the DEC-PHASE11-009 header and the minimum viable body (`apt-get update && apt-get install -y clamav clamav-freshclam`). Append Section 25 to `tests/integration/test-iso-content-presence.sh` with three assertions: 25a (clamav ABSENT from base squashfs dpkg-status), 25b (installer script present in squashfs + executable bit set), 25c (POSITIVE grep on the script for DEC-PHASE11-009 header + `apt-get install clamav`). Add a CHANGELOG entry. Net delta from rc9: **-350 MB** per Phase 11 reconciliation table line 2359. No new DEC.

**State-authority map (single-authority discipline — Sacred Practice #12):**

| State domain | Canonical authority (this slice) | Consumers / cross-references |
|---|---|---|
| ClamAV package presence | `iso/config/package-lists/orionx.list.chroot` — remove exactly one line (`clamav`, currently at line 111 in that file). No other list edits. | Debian `apt-get install` in live-build chroot; content-presence 25a asserts squashfs `/var/lib/dpkg/status` contains NO `Package: clamav` stanza. Reconciliation table row line 2359 (-350 MB accounted). |
| `/opt/orionx/optional/` layout (Layer A skeleton — FIRST installer) | `iso/config/includes.chroot/opt/orionx/optional/install-clamav.sh` — NEW file, executable (`chmod +x`), with `#!/usr/bin/env bash` + `set -euo pipefail` + DEC-PHASE11-009 `@decision` header + body invoking `apt-get update && apt-get install -y clamav clamav-freshclam`. The parent directory `/opt/orionx/optional/` is NEW (does not yet exist in the includes.chroot tree); includes.chroot staging creates it automatically at build time. | Operators run `sudo /opt/orionx/optional/install-clamav.sh` on a network-connected node post-boot; content-presence 25b + 25c assert presence, executable bit, and DEC/`apt-get` header markers. W11-8 will consume this file as-is (does NOT rewrite it) and add the framework README + sentinel-file mechanism + Control Center awareness pane wiring alongside it. |
| Content-presence test | `tests/integration/test-iso-content-presence.sh` — append-only NEW Section 25 block immediately after the existing Section 24 (W11-6 Layer A). Three assertions (25a/25b/25c). | CI acceptance gate. The section-header banner uses plan-numbering slot `21` per line 2388 whole-slice-EC wording, renumbered to insertion order `25` per the W11-6 / W11-9a precedent (see W11-6 detail plan line ~3528 for the numbering-comment convention). |
| CHANGELOG v2.1.0 | `CHANGELOG.md` `## [v2.1.0] - Unreleased` section — append `### W11-7: ClamAV drop-to-optional (DEC-PHASE11-009 landing)` subsection immediately after the existing W11-6 Layer A entry. Names: clamav removed from base package list; `install-clamav.sh` staged as first optional installer under `/opt/orionx/optional/`; content-presence Section 25 (3 assertions); DEC-PHASE11-009 (LOCKED 2026-07-02) as authority; -350 MB net delta from rc9; Control Center Awareness pane badge deferred to W11-8 per DEC-PHASE11-011. | Operators reading release notes; W11-10 rc-cut references. |

**Scope Manifest — allowed / required / forbidden (canonical audit manifest at `tmp/phase11-w11-7-clamav-drop-scope.json`; runtime scope-triad at `tmp/phase11-w11-7-clamav-drop-scope-runtime.json`, synced to runtime):**

| Path | Class | Rationale |
| --- | --- | --- |
| `iso/config/package-lists/orionx.list.chroot` | allowed + required | Remove exactly one line (`clamav`, currently line 111 per the operator directive verified at planner time via `sed -n '105,115p'`). Preserve the surrounding `# Security tools` section-header comment. Count of non-comment non-blank lines decreases by exactly 1 vs base HEAD. NO other package changes. |
| `iso/config/includes.chroot/opt/orionx/optional/install-clamav.sh` | required | NEW file. Structure: `#!/usr/bin/env bash` shebang + `# shellcheck shell=bash` + `@decision DEC-PHASE11-009` + `@title Orion-X ClamAV optional installer (post-boot, network-required)` + `@status accepted` + `@rationale` naming the drop-to-optional decision. Body: `set -euo pipefail`; EUID root check with error message ("ERROR: install-clamav.sh requires root (installs Debian packages)"); network-refuse check via `getent hosts deb.debian.org` (LOUD-fail on air-gap); `apt-get update` + `apt-get install -y clamav clamav-freshclam`; one-line hint about `freshclam` for signature-DB freshening. `chmod +x` at file creation. Verify: `bash -n install-clamav.sh` passes; `shellcheck -S error install-clamav.sh` exits 0. Roughly 30-50 lines including comment header. |
| `tests/integration/test-iso-content-presence.sh` | required | Append-only NEW Section 25 block after the existing Section 24 (W11-6 Layer A, ends ~line 2116 in the tracked file). Section-header comment banner: `# Section label note: section 21 per plan numbering (line 2388 W11-7 slot), renumbered to 25 per insertion order matching the W11-9a / W11-6 precedent.` Then `section "25. W11-7: ClamAV drop-to-optional (section 21 — DEC-PHASE11-009)"`. Three assertions per the EC. Preserve the file's `pass`/`fail` helper convention verbatim. |
| `CHANGELOG.md` | required | Append a `### W11-7: ClamAV drop-to-optional (DEC-PHASE11-009 landing)` subsection under `## [v2.1.0] - Unreleased`, immediately after the existing W11-6 Layer A entry. Names all shipped items + W11-8 Awareness-pane deferral explicitly. NO DEC change. |
| `MASTER_PLAN.md` | allowed | This planner amendment (the section you are reading). Implementer must NOT edit MASTER_PLAN.md beyond appending the W11-7 iter-1 trailer inside this section (guardian:land appends landed-at SHA per Phase 8/9/10/11 landing-record precedent). |
| `tmp/**` | allowed | Scope manifest files + any implementer scratch artifacts. |
| `iso/config/hooks/**` | forbidden | The installer is a post-boot operator command, NOT wired into any live-build hook. If a future reader believes the 0500 or 0700 hook needs a symlink for `install-clamav` on `$PATH`, that is an out-of-scope escalation — the mainline W11-7 row does not require a `$PATH` entry, and W11-8's optional-installer framework will decide the discovery UX (operators are expected to `ls /opt/orionx/optional/` or read the W11-8 README to discover installers). |
| `iso/config/includes.chroot/opt/orionx/{nebula,re,yara,comms,theme,scripts,venv,data}/**` | forbidden | Adjacent Phase 10 / Phase 11 authorities (nebula = W10-*; re = W11-3; yara = W11-4; comms = W11-5; theme = W9-1 / W11-9; scripts = shared source pool staged by `stage_application_content()`; venv/re + venv/comms = created by 0500 hook at build time; data = samples). Only `/opt/orionx/optional/` is touched. |
| `iso/config/includes.chroot/etc/**`, `var/lib/**`, `usr/share/**` | forbidden | ClamAV is a headless system service when installed post-boot; it lives at Debian's standard `/etc/clamav/` + `/var/lib/clamav/` + `/usr/bin/clamscan` after the operator runs the installer — those paths are Debian-package-owned and NOT staged by this slice. NO systemd override, NO AppArmor profile, NO XDG entries. |
| `iso/config/includes.binary/**`, `iso/auto/**` | forbidden | Boot chain / live-build config. Not touched. |
| `iso/config/nebula-model-manifest.json` | forbidden | W10-1 / W11-1 Nebula authority. Not touched. |
| `scripts/build-iso.sh` | forbidden | Build-host stage authority (DEC-PHASE11-012). The installer script under `iso/config/includes.chroot/opt/orionx/optional/` is picked up automatically by live-build's includes.chroot copy pass — no build-iso.sh edit needed. This is intentionally DIFFERENT from the W11-4 / W11-6 freshen scripts (those live under `scripts/orionx-freshen-*.sh` and are staged into `/opt/orionx/scripts/` via `stage_application_content()`); the optional-installer authority is `/opt/orionx/optional/` per DEC-PHASE11-011 and is NOT a `stage_application_content()` consumer. |
| `scripts/orionx-freshen-{yara,suricata}.sh`, `scripts/{nebula,mesh,control_center,security,release}/**` | forbidden | Sibling script authorities. |
| `runtime/**`, `hooks/**`, `agents/**`, `settings.json`, `CLAUDE.md`, `AGENTS.md`, `DECISIONS.md`, `.github/workflows/**` | forbidden | Control-plane / governance authorities. Not touched. |
| `tests/integration/{test-iso-hooks-applied.sh,test_build_iso.sh,test_iso_serial_console.sh}`, `tests/unit/**` | forbidden | Sibling test authorities not exercised by W11-7. Only `test-iso-content-presence.sh` is edited. |

**Task list (T1-T4, implementer briefing at provision):**

1. **T1 — Package list.** Edit `iso/config/package-lists/orionx.list.chroot`. Delete the single line `clamav` (currently at line 111 per operator-directive verification `sed -n '105,115p'`). Preserve the surrounding `# Security tools` section-header comment (line ~105) verbatim; the removal is a single-line delete, not a section rewrite. Verify: `grep -Ec '^clamav$' iso/config/package-lists/orionx.list.chroot == 0`; file parses as valid live-build package-list; count of non-comment non-blank lines decreases by exactly 1 vs base HEAD. Commit: `feat(w11-7): drop clamav from base package list (DEC-PHASE11-009)`.

2. **T2 — install-clamav.sh optional installer stub.** Create `iso/config/includes.chroot/opt/orionx/optional/install-clamav.sh`. Structure: header block with `#!/usr/bin/env bash` + `# shellcheck shell=bash` + `# @decision DEC-PHASE11-009` + `# @title Orion-X ClamAV optional installer (post-boot, network-required)` + `# @status accepted` + `# @rationale` naming the drop-to-optional decision + 3-4 lines of usage prose (`# Usage: sudo /opt/orionx/optional/install-clamav.sh`). Body: `set -euo pipefail`; EUID root check with `if [[ $EUID -ne 0 ]]; then echo "ERROR: install-clamav.sh requires root (installs Debian packages)" >&2; exit 1; fi`; network-refuse check via `if ! getent hosts deb.debian.org >/dev/null 2>&1; then echo "ERROR: install-clamav.sh requires network reachability to deb.debian.org — refusing to run on air-gap" >&2; exit 1; fi`; `DEBIAN_FRONTEND=noninteractive apt-get update` + `DEBIAN_FRONTEND=noninteractive apt-get install -y clamav clamav-freshclam`; final echo line hinting operator to run `sudo freshclam` for a fresh signature database. `chmod +x iso/config/includes.chroot/opt/orionx/optional/install-clamav.sh` at file creation. Verify: `bash -n install-clamav.sh` passes; `shellcheck -S error install-clamav.sh` exits 0; `test -x install-clamav.sh` passes. Commit: `feat(w11-7): stage /opt/orionx/optional/install-clamav.sh operator installer (DEC-PHASE11-009)`.

3. **T3 — Content-presence Section 25.** Edit `tests/integration/test-iso-content-presence.sh`. Append after the existing Section 24 block (W11-6 Layer A) a new Section 25 following the file's existing section-banner + `pass`/`fail` conventions. Section header: `section "25. W11-7: ClamAV drop-to-optional (section 21 — DEC-PHASE11-009)"` preceded by the numbering-note comment banner (see Scope Manifest row above). Three assertions verbatim: (25a) `if ! grep -Fq 'Package: clamav' "$SQF/var/lib/dpkg/status"; then pass "25a: clamav absent from base squashfs dpkg-status (W11-7, DEC-PHASE11-009)"; else fail "25a: clamav absent from base squashfs dpkg-status" "clamav should NOT be in the base ISO per DEC-PHASE11-009 drop-to-optional"; fi`; (25b) `local INST_SCRIPT="$SQF/opt/orionx/optional/install-clamav.sh"; if [[ -f "$INST_SCRIPT" && -x "$INST_SCRIPT" ]]; then pass "25b: /opt/orionx/optional/install-clamav.sh present + executable (W11-7)"; else fail "25b: /opt/orionx/optional/install-clamav.sh present + executable" "installer script missing or not executable in squashfs"; fi`; (25c) `if [[ -f "$INST_SCRIPT" ]] && grep -Fq 'DEC-PHASE11-009' "$INST_SCRIPT" && grep -Eq 'apt-get[[:space:]]+install[[:space:]].*clamav' "$INST_SCRIPT"; then pass "25c: install-clamav.sh contains DEC-PHASE11-009 header + apt-get install clamav body"; else fail "25c: install-clamav.sh contains DEC-PHASE11-009 header + apt-get install clamav body" "installer script missing required markers or install command"; fi`. Verify: `bash -n tests/integration/test-iso-content-presence.sh` passes; running the test against the built ISO squashfs emits `PASS: 25a`, `PASS: 25b`, `PASS: 25c` and no `FAIL: 25`. Commit part of T4's shared docs commit, OR a separate `test(w11-7): content-presence Section 25 — clamav drop + install-clamav.sh present` commit at implementer discretion.

4. **T4 — CHANGELOG + planner-iter trailer.** (a) Edit `CHANGELOG.md`: append the `### W11-7: ClamAV drop-to-optional (DEC-PHASE11-009 landing)` subsection under `## [v2.1.0] - Unreleased`, immediately after the W11-6 Layer A entry. Cover: clamav removed from `iso/config/package-lists/orionx.list.chroot` (single-line delete); `/opt/orionx/optional/install-clamav.sh` staged as the FIRST optional installer under the `/opt/orionx/optional/` layout convention (per DEC-PHASE11-011); content-presence Section 25 (3 assertions 25a/25b/25c); net delta from rc9: -350 MB per Phase 11 reconciliation table line 2359; Control Center Awareness pane badge deferred to W11-8 optional-installer framework slice per DEC-PHASE11-011 (planner defers to W11-8 for the framework README + sentinel-file mechanism + Awareness-pane wiring; W11-7 lands the FIRST installer only). (b) Edit `MASTER_PLAN.md` to append the W11-7 iter-1 trailer inside this section (identical structure to W11-3/W11-4/W11-5/W11-6 iter-1 trailers). NO other MASTER_PLAN.md edits: do NOT touch the mainline W11-7 row at line 2404, Phase 11 reconciliation table line 2359, whole-slice EC line 2388 (mentioning section 21), Decision Log rows, or any DEC-PHASE11-001..-012 rationale text. Fold T3 + T4 into one commit: `docs(w11-7): CHANGELOG v2.1.0 + content-presence Section 25 + iter-1 trailer`.

**Suggested commit shapes (implementer discretion — cascade-consolidation per DEC-PHASE7-041):**
- **Option A (2 commits, recommended):** T1 + T2 as one feat commit (`feat(w11-7): drop clamav from base list + stage install-clamav.sh optional installer (DEC-PHASE11-009)`), T3 + T4 as one docs/test commit (`docs(w11-7): CHANGELOG v2.1.0 + content-presence Section 25 + iter-1 trailer`).
- **Option B (single commit):** All four tasks in one `feat(w11-7): ClamAV drop-to-optional — DEC-PHASE11-009 landing (list + installer + section 25 + CHANGELOG)` commit. Also acceptable at this scope (matches W11-2b's single-commit shape).

**Evaluation Contract (4 items — testable, verbatim):**

1. **EC1: `clamav` absent from base package list.** `grep -Ec '^clamav$' iso/config/package-lists/orionx.list.chroot == 0` at branch HEAD. Count of non-comment non-blank lines in that file decreases by exactly 1 vs the base HEAD `git show HEAD:iso/config/package-lists/orionx.list.chroot`. No other package added or removed. Content-presence 25a asserts the built ISO squashfs `/var/lib/dpkg/status` contains NO `Package: clamav` stanza — this is the CI-time acceptance gate; local verification is source-only.

2. **EC2: `install-clamav.sh` present, executable, DEC-header + apt-get body, shellcheck-clean.** File present at `iso/config/includes.chroot/opt/orionx/optional/install-clamav.sh`; `test -x` passes; `bash -n` passes; `shellcheck -S error` exits 0. Content contains: `#!/usr/bin/env bash` on line 1; `@decision DEC-PHASE11-009` in the header comment block; `set -euo pipefail`; an EUID root check; a network-refuse check via `getent hosts`; `apt-get install -y clamav clamav-freshclam` (or byte-equivalent invocation with `DEBIAN_FRONTEND=noninteractive` prefix). Content-presence 25b + 25c assert squashfs presence + executable bit + DEC header + apt-get install grep.

3. **EC3: Content-presence Section 25 — three assertions.** `tests/integration/test-iso-content-presence.sh` gains Section 25 with assertions 25a (clamav absent from dpkg-status), 25b (install-clamav.sh present + executable in squashfs), 25c (DEC-PHASE11-009 header + apt-get install clamav grep on the installer). When run against the built ISO squashfs, all three assertions pass. The full test binary (`bash tests/integration/test-iso-content-presence.sh`) exits 0. CI log emits `PASS: 25a`, `PASS: 25b`, `PASS: 25c` and no `FAIL: 25`.

4. **EC4: CHANGELOG entry.** `CHANGELOG.md` `## [v2.1.0] - Unreleased` section gains the `### W11-7: ClamAV drop-to-optional (DEC-PHASE11-009 landing)` subsection immediately after the W11-6 Layer A entry. Names: clamav base-list removal; `/opt/orionx/optional/install-clamav.sh` first optional installer; content-presence Section 25 (3 assertions); -350 MB net delta from rc9; W11-8 Awareness-pane deferral per DEC-PHASE11-011. `grep -Ec '^### W11-7:' CHANGELOG.md >= 1` AND `grep -Fq 'DEC-PHASE11-009' CHANGELOG.md` AND `grep -Fq 'install-clamav.sh' CHANGELOG.md`.

**Ready-for-guardian definition:** All four EC items pass on the reviewer's evaluation; reviewer emits `REVIEW_VERDICT: ready_for_guardian` with head SHA; test-state `pass_complete`; guardian lease active. The mainline W11-7 row's `approve` gate is discharged by DEC-PHASE11-009's LOCKED 2026-07-02 sign-off — no live user gate remains at the review→land boundary for this specific landing cadence.

**Forbidden shortcuts (explicitly banned):**

- Do NOT comment-out the clamav line (`# clamav`) — delete cleanly per DEC-PHASE7-041 cascade-consolidation discipline. Retention of superseded lines as comments is a hygiene debt, not a fallback.
- Do NOT ship a Control Center Awareness pane badge in W11-7. The badge is W11-8-owned per the mainline W11-7 row wording (line 2404: "Ship a Control Center Awareness pane badge…") — but W11-8 is the OPTIONAL-INSTALLER FRAMEWORK slice per the mainline W11-8 row (line 2405) and DEC-PHASE11-011 explicitly names the framework's Awareness-pane extension as W11-8's authority. This planner amendment discharges W11-7's mainline scope EXCEPT the badge, which lands as one cohesive W11-8 deliverable (framework README + sentinel-file mechanism + Awareness-pane wiring together). This is a landing-cadence decision, NOT a policy change; DEC-PHASE11-009 + DEC-PHASE11-011 preserved verbatim.
- Do NOT author `install-ghidra.sh`, `install-element.sh`, `install-devel.sh`, or `install-signature-base.sh` here. Those are W11-8's scope. W11-7 lands ONLY `install-clamav.sh` as the first optional installer.
- Do NOT stage a `/opt/orionx/optional/README.md` framework document. That is W11-8's authority. `install-clamav.sh` alone (plus Section 25 assertions) is enough for W11-7's landing gate.
- Do NOT wire `install-clamav.sh` into any 0500/0700 hook, `.desktop` launcher, systemd unit, or `$PATH` symlink. It is a root-run operator command, invoked manually by absolute path (`sudo /opt/orionx/optional/install-clamav.sh`). W11-8 will decide the discovery UX (framework README + Awareness-pane badge together).
- Do NOT touch any other Debian package line in `orionx.list.chroot`. `clamav` is the ONLY removal in this slice. If a future reader believes another package should also drop, that is a separate slice's scope.
- Do NOT edit `iso/config/nebula-model-manifest.json` or any file under `scripts/nebula/`, `iso/config/includes.chroot/opt/orionx/nebula/`, or the W10-1 systemd chain. Nebula authority is orthogonal.
- Do NOT edit `scripts/build-iso.sh`. The installer is under `iso/config/includes.chroot/opt/orionx/optional/` which live-build copies verbatim; no `stage_application_content()` extension is required (this is the DIFFERENCE from W11-4 / W11-6 freshen scripts, which live under `scripts/orionx-freshen-*.sh` and DO require the `stage_application_content()` pathway).
- Do NOT modify `DECISIONS.md` directly; it is auto-generated from `@decision` annotations by `stop.sh`.
- Do NOT edit `.github/workflows/**` — not in scope. Section 25 runs under the existing content-presence step; no runner setup changes needed (unlike W11-2c's `python3-gi` runner extension for 16f).
- Do NOT edit `MASTER_PLAN.md` beyond appending the W11-7 iter-1 trailer at the end of this detail-plan section. Do NOT touch the mainline W11-7 row at line 2404, the Phase 11 reconciliation table row at line 2359 (-350 MB), the whole-slice EC line 2388 (mentioning section 21), any Decision Log row, or DEC-PHASE11-009 / DEC-PHASE11-011 rationale text.
- Do NOT combine this slice with W11-8 (optional-installer framework), W11-9a/b/c (branding), W11-6b (Suricata Layer B), W11-5b (gomuks), W11-4b (YARA rulesets), or W11-3b (Layer B) — each is its own workflow.

**Downstream consumers (what this slice unblocks):**

- **W11-8** (optional-installer framework) — consumes `install-clamav.sh` as-is (does NOT rewrite it) and adds the framework README at `/opt/orionx/optional/README.md`, the sentinel-file mechanism (`/opt/orionx/optional/.installed/<name>.state`), the preflight-banner convention per DEC-PHASE11-011, and the Control Center Awareness pane extension. W11-7 seeds the FIRST installer under the layout convention so W11-8's framework slice has one existing installer to test the sentinel mechanism against + a working exemplar for `install-ghidra.sh` / `install-element.sh` / `install-devel.sh` / `install-signature-base.sh`.
- **W11-10** (rc-cut + hardware attestation) — will verify (a) `dpkg -l clamav` on the booted system reports NOT installed; (b) `test -x /opt/orionx/optional/install-clamav.sh` on the booted system reports true; (c) ISO size delta vs rc9 accounts for the -350 MB (partially — full delta is the sum across all W11-* slices). Content-presence Section 25 provides the CI-time evidence; hardware attestation provides the runtime evidence.
- **Phase 11 whole-slice EC item 8 (line 2388)** — the "section 21 W11-7 ClamAV verdict" slot; discharged by this slice's Section 25 (numbering renumbered per insertion order following the W11-9a / W11-6 precedent).

**Risk register (W11-7-specific — 2 items, proportional to S weight):**

- **R1 — Debian mirror unreachable at operator install time.** When the operator runs `install-clamav.sh` post-boot, it requires `deb.debian.org` reachability. **Mitigation:** T2's `getent hosts deb.debian.org` LOUD-fail with a clear error message; operator retries when the mirror is reachable. This is the correct failure mode (a silent no-op or fake success would be worse). NOT a CI concern (the installer never runs at build time).
- **R2 — Boot-time signature-DB freshness.** ClamAV's `freshclam` signature DB is 90% stale within days per DEC-PHASE11-009's rationale — even the operator who runs `install-clamav.sh` should immediately follow with `sudo freshclam` to fetch a current DB. **Mitigation:** T2's final hint echo names `sudo freshclam` explicitly. This is a documentation-only mitigation (the installer does NOT invoke freshclam itself — the operator decides when to freshen, and the installer must not silently pull hundreds of MB of signatures without operator consent). NOT a Section 25 concern (freshness is a runtime property, not a squashfs content property).

**Cross-references:**
- W11-6 Layer A detail plan (this file, section starting line 3513) — closest structural analog for section-numbering + append-only content-presence pattern.
- DEC-PHASE11-009 (ClamAV DROP-to-optional, LOCKED 2026-07-02, `MASTER_PLAN.md` line 2337).
- DEC-PHASE11-011 (optional-installer framework layout convention, `MASTER_PLAN.md` line 2339) — W11-7 stages the FIRST installer under this convention; framework README + sentinel + Awareness pane wiring is W11-8's scope.
- Mainline W11-7 row (`MASTER_PLAN.md` line 2404) — preserved verbatim; this slice discharges its landing.
- Phase 11 reconciliation table row (`MASTER_PLAN.md` line 2359) — -350 MB net delta accounted.
- Phase 11 whole-slice EC item 8 (`MASTER_PLAN.md` line 2388) — Section 21 W11-7 slot renumbered to Section 25 per insertion order.

**W11-7 iter-1 (planner detail-plan): 2026-07-19.** Detail plan appended to `MASTER_PLAN.md` on `develop`; NO new DEC (DEC-PHASE11-009 authority reused verbatim; DEC-PHASE11-011 optional-installer layout referenced; DEC-PHASE7-041 cascade-consolidation discipline observed); Scope Manifest (audit shape) at `tmp/phase11-w11-7-clamav-drop-scope.json`; runtime scope-triad at `tmp/phase11-w11-7-clamav-drop-scope-runtime.json` (synced to runtime via `cc-policy workflow scope-sync` — 3 paths written, status=ok). Ready for `guardian:provision` under workflow_id `phase11-w11-7-clamav-drop`.

---

#### W11-8 — Detail plan (Optional-installer framework LAYER A — shared library + 5 new installer stubs + refactor W11-7 clamav; Awareness pane + install-remnux-mcp + install-devel + install-signature-base deferred to W11-8b)

**Seeded:** 2026-07-19 planner amendment. **Status:** Layer A detail plan authored; ready for `guardian:provision` under workflow_id `phase11-w11-8-optional-installers`. **Env:** Linux/CI. **Wave:** 6. **Gate:** review. **Weight:** M (Layer A only; L umbrella row at line 2405 stands). **Deps:** W11-7 (`install-clamav.sh` present as the FIRST installer under `/opt/orionx/optional/` per its own iter-1 trailer at line 3716). Deps to W11-3 and W11-5 stated in the mainline row at line 2405 are **relaxed for Layer A** — the ghidra/element/floss/trid/gomuks stubs authored here are apt/wget shells that **do not** ingest any staged content from those earlier slices; they invoke Debian repos and upstream tarball URLs at operator-run time only. Layer B (W11-8b) re-tightens the deps when the Awareness pane wiring and content-consuming installers (`install-remnux-mcp.sh` which requires W11-3b's Node 20 LTS runtime; `install-signature-base.sh` which requires W11-4b's YARA freshen mechanism) are authored.

**Slice intent (LAYER A — LAND TONIGHT).** Discharge the mainline W11-8 row at line 2405 by landing the **framework skeleton** — a shared installer library at `/opt/orionx/optional/lib/orionx-installer-common.sh`, five new installer stubs that source that library, and a refactor of the existing W11-7 `install-clamav.sh` (currently inlined per its 27-line body at `iso/config/includes.chroot/opt/orionx/optional/install-clamav.sh:1-27`) to use the library — so every future `install-<name>.sh` picks up the same DEC-PHASE11-011 behavioral contract (root check, network preflight, log helpers, LOUD-fail on air-gap) by sourcing one file instead of re-implementing five bash boilerplate patterns per installer. The framework is proven under Layer A by six installers (5 new + 1 refactored) all exercising the shared library at shellcheck time; content-presence Section 26 asserts library + stubs + refactor. Layer B (W11-8b, seeded at the end of this section) lands the Control Center Awareness pane badge (extends the W9-2 GTK source at `scripts/control_center/sections/awareness.py` to read `/opt/orionx/optional/.installed/<name>.state` sentinel files and display per-installer state), plus the two mainline-row installers deferred here (`install-devel.sh` for the W11-2 dev toolchain restore; `install-signature-base.sh` for the W11-4 Florian Roth CC-BY-NC ruleset acceptance), plus `install-remnux-mcp.sh` (gated on W11-3b Node 20 LTS runtime landing). DEC-PHASE11-011 is preserved verbatim — the split is a landing-cadence choice, not a policy change; the framework skeleton lands in v2.1.0 with 6 installers exercising it, and the Awareness-pane + 3 additional installers land in the same v2.1.0 across a follow-up commit.

**Scope Manifest (LAYER A):**

| Path | Status | Rationale |
|------|--------|-----------|
| `iso/config/includes.chroot/opt/orionx/optional/lib/orionx-installer-common.sh` | required (new file — this slice CREATES the `lib/` subdirectory) | Shared bash library sourced by every `install-<name>.sh`. Defines the DEC-PHASE11-011 behavioral contract as reusable functions: `orionx_require_root` (EUID check, LOUD-fail with clear error), `orionx_require_network <host>` (`getent hosts` check, LOUD-fail on air-gap with clear error), `orionx_log_info <msg>` (`echo "[install-$name] $msg"` where `$name` derives from `$0`), `orionx_log_error <msg>` (same but to stderr with `ERROR:` prefix), `orionx_apt_install <packages…>` (idempotent `DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y ...`), `orionx_wget_extract <url> <dest_dir>` (fetch tarball to tmp + verify + extract; LOUD-fail on 404/checksum mismatch), `orionx_verify_sha256 <file> <expected>` (LOUD-fail on mismatch; used by installers that ship SHA-256-pinned upstream tarballs per the DEC-PHASE10-008 trust-on-first-use pattern). Header: `#!/usr/bin/env bash` + `# shellcheck shell=bash` + `# @decision DEC-PHASE11-011` + `# @title Orion-X optional-installer common library` + `# @status accepted` + `# @rationale Shared framework for /opt/orionx/optional/install-*.sh — DRY the DEC-PHASE11-011 contract (root + network preflight + log helpers + apt/wget helpers) so every installer picks up the same behavior by sourcing this file.` File is NOT executable (`chmod 644`) — it is sourced, not run. `set -euo pipefail` at the top is NOT set here (that is the caller's responsibility so a sourced library does not hijack the caller's shell options). `shellcheck -S error` exits 0. Roughly 60–90 lines including header + 7 functions. |
| `iso/config/includes.chroot/opt/orionx/optional/install-ghidra.sh` | required (new file) | Ghidra optional installer stub. Header: `#!/usr/bin/env bash` + `# shellcheck shell=bash` + `# @decision DEC-PHASE11-004` (Ghidra defer from base ISO to optional installer) + `# @decision DEC-PHASE11-011` (optional-installer framework) + `# @title Orion-X Ghidra optional installer (post-boot, network-required)` + `# @status accepted` + `# @rationale` naming the Ghidra defer decision. Body: `set -euo pipefail`; `source /opt/orionx/optional/lib/orionx-installer-common.sh`; preflight banner via `orionx_log_info` (name / size / license / network / mission verb per DEC-PHASE11-011); `orionx_require_root`; `orionx_require_network github.com`; `orionx_wget_extract` invocation against a NationalSecurityAgency/ghidra release URL (planner defers exact URL + SHA-256 to implementer at slice-provision — trust-on-first-use per DEC-PHASE10-008; the first green CI build of W11-8 computes the SHA-256 and a follow-up commit pins it). Extract to `/opt/orionx/ghidra/`. Executable (`chmod +x`). `shellcheck -S error` exits 0. Roughly 30–50 lines. |
| `iso/config/includes.chroot/opt/orionx/optional/install-element.sh` | required (new file) | element-desktop optional installer stub, per the W11-5 mainline row (line 2402) + the W11-5 Layer A detail plan's explicit deferral (line 3483: "Do NOT ship an install-element.sh optional installer in Layer A. That stub is owned by W11-8's optional-installer framework slice"). Header: `#!/usr/bin/env bash` + `# shellcheck shell=bash` + `# @decision DEC-PHASE11-007` (team comms) + `# @decision DEC-PHASE11-011` + `# @title Orion-X Element desktop optional installer (post-boot, network-required)` + `# @status accepted` + `# @rationale` naming DEC-PHASE11-007 element-desktop optional. Body: `set -euo pipefail`; `source /opt/orionx/optional/lib/orionx-installer-common.sh`; preflight banner; `orionx_require_root`; `orionx_require_network packages.element.io`; `orionx_apt_install element-desktop` — Element ships an official Debian repo (add apt-key + `/etc/apt/sources.list.d/element-io.list` first per Element's install docs; details deferred to implementer). Executable. `shellcheck -S error` exits 0. Roughly 30–50 lines. |
| `iso/config/includes.chroot/opt/orionx/optional/install-floss.sh` | required (new file) | FLOSS (Mandiant obfuscated-string solver) optional installer stub, per the W11-3 Layer A detail plan's explicit deferral (line 3309: "Do NOT ship FLOSS, TrID, Node.js 20 LTS, or remnux-mcp-server payloads in this slice. Those are Layer B / W11-3b"). Layer A of W11-8 authors the operator-side installer stub so operators who do not want to wait for W11-3b can install post-boot; W11-3b's build-time staging into `/opt/orionx/re/floss/` still lands separately. Header: `#!/usr/bin/env bash` + `# @decision DEC-PHASE11-004` (RE toolkit) + `# @decision DEC-PHASE11-011` + `# @title Orion-X FLOSS optional installer (post-boot, network-required)` + `# @status accepted` + `# @rationale` naming FLOSS as Layer B deferred content that operators can install directly from Mandiant releases post-boot. Body: `set -euo pipefail`; `source ... orionx-installer-common.sh`; preflight banner (Apache-2.0, ~15 MB, mission verb: `triage`); `orionx_require_root`; `orionx_require_network github.com`; `orionx_wget_extract` from a `mandiant/flare-floss` GitHub releases URL; extract to `/opt/orionx/floss/`. Executable. `shellcheck -S error` exits 0. Roughly 30–50 lines. |
| `iso/config/includes.chroot/opt/orionx/optional/install-trid.sh` | required (new file) | TrID (file identifier) optional installer stub, per the same W11-3 Layer B deferral. Header: `#!/usr/bin/env bash` + `# @decision DEC-PHASE11-004` + `# @decision DEC-PHASE11-011` + `# @title Orion-X TrID optional installer (post-boot, network-required)` + `# @status accepted` + `# @rationale` naming TrID as Layer B deferred content installable post-boot from Marco Pontello's upstream tarball. Body: `set -euo pipefail`; `source ... orionx-installer-common.sh`; preflight banner (freeware, ~5 MB, mission verb: `triage`); `orionx_require_root`; `orionx_require_network mark0.net`; `orionx_wget_extract` from the upstream `mark0.net/download/trid_linux_64.zip` URL (planner note: TrID is `.zip` not `.tar.gz` — `orionx_wget_extract` must handle both, or implementer authors a `orionx_wget_extract_zip` sibling helper at library-design time). Extract to `/opt/orionx/trid/`. Executable. `shellcheck -S error` exits 0. Roughly 30–50 lines. |
| `iso/config/includes.chroot/opt/orionx/optional/install-gomuks.sh` | required (new file) | gomuks (Matrix TUI) optional installer stub, per the W11-5 Layer B deferral (line 2402 mainline row names gomuks in base; the W11-5 Layer A detail plan defers gomuks staging to W11-5b). Layer A of W11-8 authors the operator-side installer stub so operators can install directly from `mautrix/gomuks` releases post-boot without waiting for W11-5b's build-time staging. Header: `#!/usr/bin/env bash` + `# @decision DEC-PHASE11-007` + `# @decision DEC-PHASE11-011` + `# @title Orion-X gomuks optional installer (post-boot, network-required)` + `# @status accepted` + `# @rationale` naming gomuks as Layer B deferred content installable post-boot from mautrix/gomuks upstream tarball. Body: `set -euo pipefail`; `source ... orionx-installer-common.sh`; preflight banner (Apache-2.0, ~25 MB, mission verb: `defend`); `orionx_require_root`; `orionx_require_network github.com`; `orionx_wget_extract` from a `mautrix/gomuks` GitHub releases URL; install binary to `/usr/local/bin/gomuks` (or `/opt/orionx/gomuks/`, implementer to choose but must be consistent with W11-5b's planned staging so the Layer B build-time staging does not conflict with a Layer A operator-run installation). Executable. `shellcheck -S error` exits 0. Roughly 30–50 lines. |
| `iso/config/includes.chroot/opt/orionx/optional/install-clamav.sh` | required (existing file — REFACTOR) | The W11-7 installer at `iso/config/includes.chroot/opt/orionx/optional/install-clamav.sh` currently inlines the root check (lines 10-13), network check (lines 15-18), log echo (lines 20, 24, 27), apt-install (lines 21-22), and freshclam optional step (lines 24-25). Refactor to source the shared library and replace the inlined checks with library calls. Preserve verbatim: the DEC-PHASE11-009 header block (lines 1-6, especially `@rationale`), `set -euo pipefail` (line 8), the freshclam invocation with soft-fail (line 25), the final `Usage: clamscan <file>` hint (line 27). Replace: lines 10-13 with `orionx_require_root`; lines 15-18 with `orionx_require_network deb.debian.org`; lines 20-22 with `orionx_log_info "Installing ClamAV + freshclam..."; orionx_apt_install clamav clamav-freshclam`; line 24 with `orionx_log_info "Running initial freshclam..."`. `set -euo pipefail` MUST remain in the installer (not the library — the caller owns shell options). Result: net line count SHRINKS (roughly 27 lines → roughly 15 lines including header). `shellcheck -S error` exits 0. `bash -n` passes. Content-presence Section 25 (W11-7's existing three assertions 25a/25b/25c) MUST still pass unchanged — the DEC-PHASE11-009 header grep + `apt-get install clamav` grep still succeed because `orionx_apt_install` expands `apt-get install -y clamav clamav-freshclam` verbatim at exec time, and the shellcheck-visible source line reads `orionx_apt_install clamav clamav-freshclam` which content-presence 25c matches via its `apt-get[[:space:]]+install[[:space:]].*clamav` regex ONLY if the regex is loosened. **Planner note for implementer:** update the Section 25c regex to `(apt-get|orionx_apt_install)[[:space:]].*clamav` in the same commit that refactors install-clamav.sh, so the assertion continues to catch both the un-refactored and refactored forms (protects against a future revert). |
| `tests/integration/test-iso-content-presence.sh` | required (existing file — APPEND Section 26 + adjust 25c) | Append new `section "26. W11-8 Layer A: Optional-installer framework — shared lib + 5 new installer stubs + clamav refactor (DEC-PHASE11-011)"` block after the existing Section 25 (W11-7 ClamAV). Assertions: (26a) `/opt/orionx/optional/lib/orionx-installer-common.sh` present in squashfs + shellcheck-error clean at build time (checked via a shellcheck run in the same test file, gated on `command -v shellcheck`; SKIP with `warn` if shellcheck absent); (26b) library defines all 7 expected functions (positive grep for `orionx_require_root`, `orionx_require_network`, `orionx_log_info`, `orionx_log_error`, `orionx_apt_install`, `orionx_wget_extract`, `orionx_verify_sha256` — matching `^[[:space:]]*orionx_<name>[[:space:]]*\(\)`); (26c) each of the 5 new installer stubs (`install-ghidra.sh`, `install-element.sh`, `install-floss.sh`, `install-trid.sh`, `install-gomuks.sh`) present in squashfs + executable bit set + sources the library via positive grep for `source[[:space:]]+/opt/orionx/optional/lib/orionx-installer-common.sh`; (26d) `install-clamav.sh` refactor — positive grep for `source[[:space:]]+/opt/orionx/optional/lib/orionx-installer-common.sh` AND `orionx_apt_install` AND `orionx_require_root` (the refactor is proven by presence of the three library calls); (26e) `install-clamav.sh` LOUD-fail smoke test — running the installer with `EUID=1000` (fake non-root via bash) exits non-zero with an error mentioning `root` on stderr (compiles the library semantics into an executable check — no network needed). Follow the section-scaffolding conventions of Section 25 (pass/fail helpers, sub-check IDs `26a`, `26b`, `26c`, `26d`, `26e`). ALSO in the same commit: loosen the existing Section 25c regex from `apt-get[[:space:]]+install[[:space:]].*clamav` to `(apt-get|orionx_apt_install)[[:space:]].*clamav` so the W11-7 assertion survives the W11-8 refactor. Verify: `bash -n tests/integration/test-iso-content-presence.sh` returns 0; grep for `section "26` returns exactly 1 hit; grep for `section "25` returns exactly 1 hit (i.e., the 25c edit did not accidentally add a duplicate section). |
| `CHANGELOG.md` | required (existing file — APPEND) | Append `### W11-8 Layer A: Optional-installer framework — shared library + 5 new installer stubs + clamav refactor (DEC-PHASE11-011 landing)` subsection under `## [v2.1.0] - Unreleased`, immediately after the existing W11-7 entry. Cover: (i) shared library at `/opt/orionx/optional/lib/orionx-installer-common.sh` defining the DEC-PHASE11-011 contract as 7 reusable functions (root/network preflight, log helpers, apt/wget helpers, SHA-256 verify); (ii) 5 new installer stubs — `install-ghidra.sh` (Ghidra defer from base per DEC-PHASE11-004), `install-element.sh` (element-desktop per DEC-PHASE11-007, Layer C deferral from W11-5), `install-floss.sh` + `install-trid.sh` (RE toolkit Layer B deferrals from W11-3 per DEC-PHASE11-004), `install-gomuks.sh` (team comms Layer B deferral from W11-5 per DEC-PHASE11-007); (iii) `install-clamav.sh` refactored to source the shared library (net line count shrinks; behavior identical); (iv) content-presence Section 26 (5 assertions 26a/26b/26c/26d/26e) + Section 25c regex loosened for refactor survival; (v) W11-8b (Layer B) deferred payloads named explicitly (Control Center Awareness pane badge extending W9-2's `scripts/control_center/sections/awareness.py`; `install-devel.sh` for the W11-2 dev toolchain restore; `install-signature-base.sh` for the W11-4 Florian Roth CC-BY-NC ruleset acceptance; `install-remnux-mcp.sh` gated on W11-3b Node 20 LTS runtime); (vi) NO DEC change (DEC-PHASE11-011 covers both Layers). |
| `MASTER_PLAN.md` | required (this file — this slice CREATES this Detail plan section + the W11-8b Layer B seed row + iter-1 trailer) | Detail-plan section, seed row for W11-8b, iter-1 trailer. NO OTHER MASTER_PLAN.md edits: do NOT touch the mainline W11-8 row at line 2405, Phase 11 reconciliation, whole-slice EC line 2388 (mentioning section 22 — which becomes section 26 per insertion order, same renumber convention W11-7 used to shift its section 21→25), Decision Log rows, DEC-PHASE11-011 rationale, or any adjacent phase content. |
| `tmp/phase11-w11-8-optional-installers-scope.json` | required (new file — this slice CREATES) | Scope Manifest JSON with LEGAL keys only (`allowed_paths`, `required_paths`, `forbidden_paths`, `authority_domains`, `state_domains`); synced to runtime via `cc-policy workflow scope-sync phase11-w11-8-optional-installers --work-item-id wi-phase11-w11-8-optional-installers --scope-file tmp/phase11-w11-8-optional-installers-scope.json` at planner-authoring time. |
| `iso/config/hooks/**` | forbidden | Optional installers are post-boot operator commands, NOT wired into any live-build hook. If a future reader believes the 0500 or 0700 hook needs a symlink for `install-<name>` on `$PATH`, that is an out-of-scope escalation. W11-8b's Awareness pane provides the discovery UX. |
| `iso/config/package-lists/**` | forbidden | No package-list edits; the base ISO package set is untouched by W11-8. Ghidra removal from the base was W11-2's landing; element/floss/trid/gomuks were never in the base per their originating decisions; clamav removal was W11-7's landing. |
| `iso/config/includes.chroot/opt/orionx/{nebula,re,yara,comms,theme,scripts,venv,data}/**` | forbidden | Adjacent Phase 10 / Phase 11 authorities. Only `/opt/orionx/optional/` is touched. |
| `scripts/control_center/**` | forbidden | Control Center Awareness pane extension is W11-8b Layer B scope; Layer A does NOT edit `scripts/control_center/sections/awareness.py` or any Control Center source. |
| `scripts/build-iso.sh` | forbidden | Build-host stage authority (DEC-PHASE11-012). Files under `iso/config/includes.chroot/opt/orionx/optional/` are picked up automatically by live-build's includes.chroot copy pass — no build-iso.sh edit needed. |
| `iso/config/includes.chroot/opt/orionx/optional/install-{devel,signature-base,remnux-mcp}.sh` | forbidden (LAYER A) | These three installers are named by the mainline W11-8 row at line 2405 but are DEFERRED to W11-8b per this Layer A / Layer B split. `install-devel.sh` needs a curated dev-toolchain package list that mirrors what W11-2 removed (avoid re-fragmenting DEC-PHASE11-003); `install-signature-base.sh` needs the W11-4 YARA freshen mechanism to accept the Florian Roth CC-BY-NC terms locally (both landing surfaces owned by W11-4); `install-remnux-mcp.sh` needs W11-3b's Node 20 LTS runtime to be present at operator install-time (currently deferred). |

**Task decomposition (LAYER A only; canonical 3-4 commit shape):**

1. **T1 — Shared library `lib/orionx-installer-common.sh`.** Create `iso/config/includes.chroot/opt/orionx/optional/lib/orionx-installer-common.sh`. Header block per the Scope Manifest row above. Define 7 functions: `orionx_require_root` (`if [[ $EUID -ne 0 ]]; then echo "ERROR: ${0##*/} requires root" >&2; exit 1; fi`); `orionx_require_network` (`local host="${1:?host required}"; if ! getent hosts "$host" >/dev/null 2>&1; then echo "ERROR: $host unreachable (air-gap or DNS). ${0##*/} requires network." >&2; exit 1; fi`); `orionx_log_info` (`echo "[${0##*/}] $*"`); `orionx_log_error` (`echo "ERROR: [${0##*/}] $*" >&2`); `orionx_apt_install` (`DEBIAN_FRONTEND=noninteractive apt-get update; DEBIAN_FRONTEND=noninteractive apt-get install -y "$@"`); `orionx_wget_extract` (`local url="${1:?url required}"; local dest="${2:?dest required}"; local tmp; tmp="$(mktemp -d)"; ...`); `orionx_verify_sha256` (`local file="${1:?}"; local expected="${2:?}"; local actual; actual="$(sha256sum "$file" | awk '{print $1}')"; if [[ "$actual" != "$expected" ]]; then echo "ERROR: SHA-256 mismatch on $file (expected $expected, got $actual)" >&2; exit 1; fi`). Do NOT set `set -euo pipefail` in the library (caller's responsibility). Verify: `bash -n lib/orionx-installer-common.sh` passes; `shellcheck -S error lib/orionx-installer-common.sh` exits 0. Commit: `feat(w11-8): stage /opt/orionx/optional/lib/orionx-installer-common.sh — shared installer framework (DEC-PHASE11-011)`.

2. **T2 — Five new installer stubs.** Create the five files per the Scope Manifest rows above: `install-ghidra.sh`, `install-element.sh`, `install-floss.sh`, `install-trid.sh`, `install-gomuks.sh`. Each is `chmod +x`, starts with `#!/usr/bin/env bash`, sources the library via absolute path (`source /opt/orionx/optional/lib/orionx-installer-common.sh`), sets `set -euo pipefail`, calls `orionx_require_root`, calls `orionx_require_network <upstream host>`, invokes either `orionx_apt_install` (element) or `orionx_wget_extract` (ghidra, floss, trid, gomuks) against the upstream URL, and emits an `orionx_log_info` completion line. Verify each: `bash -n`, `shellcheck -S error`, `test -x`. Commit: `feat(w11-8): stage 5 new optional installers — ghidra, element, floss, trid, gomuks (DEC-PHASE11-011)`.

3. **T3 — Refactor `install-clamav.sh` to source library.** Edit `iso/config/includes.chroot/opt/orionx/optional/install-clamav.sh` per the Scope Manifest row above. Preserve the DEC-PHASE11-009 header block (lines 1-6) verbatim. Preserve `set -euo pipefail` on line 8. Replace lines 10-13 with `orionx_require_root`. Replace lines 15-18 with `orionx_require_network deb.debian.org`. Replace lines 20-22 with `orionx_log_info "Installing ClamAV + freshclam..."` + `orionx_apt_install clamav clamav-freshclam`. Replace line 24 with `orionx_log_info "Running initial freshclam to populate signatures..."`. Preserve line 25 (`freshclam || ...` soft-fail) verbatim. Preserve line 27 (final usage hint) via `orionx_log_info` rewrite. Insert the `source /opt/orionx/optional/lib/orionx-installer-common.sh` line immediately after `set -euo pipefail`. Verify: `bash -n`, `shellcheck -S error`, `test -x`. Commit part of T4's shared commit, OR standalone `refactor(w11-8): install-clamav.sh sources shared framework library`.

4. **T4 — Content-presence Section 26 + Section 25c regex loosen + CHANGELOG + planner iter-1 trailer.** (a) Edit `tests/integration/test-iso-content-presence.sh` — append Section 26 per the Scope Manifest row above (5 assertions 26a/26b/26c/26d/26e); loosen Section 25c regex from `apt-get[[:space:]]+install[[:space:]].*clamav` to `(apt-get|orionx_apt_install)[[:space:]].*clamav`. (b) Edit `CHANGELOG.md` — append the `### W11-8 Layer A: ...` subsection per the Scope Manifest row above. (c) Edit `MASTER_PLAN.md` — append the W11-8 iter-1 trailer at the end of this section (does NOT touch mainline row line 2405 or Phase 11 whole-slice EC line 2388's section-22 slot — insertion order convention handles the 22→26 shift, mirroring W11-7's 21→25 shift). Verify: `bash -n tests/integration/test-iso-content-presence.sh`; running the test against a built ISO squashfs emits `PASS: 26a`, `PASS: 26b`, `PASS: 26c`, `PASS: 26d`, `PASS: 26e` AND `PASS: 25a`, `PASS: 25b`, `PASS: 25c`. Commit: `docs(w11-8): CHANGELOG v2.1.0 + content-presence Section 26 + Section 25c regex + iter-1 trailer`.

**Commit strategy:**
- **Option A (2 commits, recommended):** T1 + T2 as one feat commit (`feat(w11-8): stage optional-installer framework — shared library + 5 new installer stubs (DEC-PHASE11-011)`); T3 + T4 as one refactor/docs commit (`refactor(w11-8): install-clamav.sh sources shared library + content-presence Section 26 + CHANGELOG`).
- **Option B (4 commits, one per T):** Only if a T1 or T2 individual failure is likely and small-blast-radius rollback is preferred.

**Evaluation Contract (LAYER A — 7 items, guardian-executable):**

1. **EC1: `lib/orionx-installer-common.sh` present + shellcheck clean + defines 7 expected functions.** File present at `iso/config/includes.chroot/opt/orionx/optional/lib/orionx-installer-common.sh`; `test -f` passes; `bash -n` passes; `shellcheck -S error` exits 0; the DEC-PHASE11-011 `@decision` header comment present; positive grep for each of the 7 function definitions (`orionx_require_root`, `orionx_require_network`, `orionx_log_info`, `orionx_log_error`, `orionx_apt_install`, `orionx_wget_extract`, `orionx_verify_sha256`) matching `^[[:space:]]*<name>[[:space:]]*\(\)`; file is NOT executable (`test ! -x` — sourced, not run).
2. **EC2: 5 new installer stubs present + executable + shellcheck clean + source library.** For each of `install-ghidra.sh`, `install-element.sh`, `install-floss.sh`, `install-trid.sh`, `install-gomuks.sh`: `test -f` passes; `test -x` passes; `bash -n` passes; `shellcheck -S error` exits 0; positive grep for `source[[:space:]]+/opt/orionx/optional/lib/orionx-installer-common.sh`; positive grep for `orionx_require_root` + `orionx_require_network`.
3. **EC3: `install-clamav.sh` refactored to use library — behavior preserved.** Positive grep for `source[[:space:]]+/opt/orionx/optional/lib/orionx-installer-common.sh` + `orionx_require_root` + `orionx_require_network deb.debian.org` + `orionx_apt_install`. Negative grep for the old inlined patterns (`if \[\[ \$EUID -ne 0 \]\]` and `if ! getent hosts deb.debian.org`) — those must be GONE (dual-authority hazard if both persist). DEC-PHASE11-009 header block (lines 1-6 of the pre-refactor file) preserved verbatim. `bash -n` passes; `shellcheck -S error` exits 0; `test -x` passes; net line count SHRINKS vs pre-refactor.
4. **EC4: All installer stubs LOUD-fail on air-gap.** For each of the 6 installers (5 new + 1 refactored), running the installer with a `getent hosts` that returns non-zero (implementer chooses fault injection — e.g., stubbed `getent` in PATH front, or a network-namespaced smoke test) exits non-zero AND emits `ERROR:` + the upstream hostname + `unreachable` on stderr. Framework-level assertion: at least one installer is exercised in CI (Section 26e assertion on `install-clamav.sh` with faked non-root EUID captures the shared-library semantics; explicit air-gap fault injection deferred to the reviewer's manual smoke or a QEMU-boot test if time permits).
5. **EC5: Content-presence Section 26 — five assertions all pass.** `tests/integration/test-iso-content-presence.sh` gains Section 26 with assertions 26a (`lib/orionx-installer-common.sh` present + shellcheck clean), 26b (7 function definitions grep-detected in library), 26c (all 5 new installer stubs present + executable + source library), 26d (`install-clamav.sh` refactored — sources library + calls `orionx_require_root` + `orionx_apt_install`), 26e (`install-clamav.sh` LOUD-fails when run with non-root EUID). When run against the built ISO squashfs, all 5 sub-assertions pass. Section 25c regex loosened to `(apt-get|orionx_apt_install)[[:space:]].*clamav` — verify Section 25a/25b/25c all still PASS on the refactored `install-clamav.sh`. `grep -c '^section "26' == 1`, `grep -c '^section "25' == 1`. Full test binary (`bash tests/integration/test-iso-content-presence.sh`) exits 0.
6. **EC6: CHANGELOG entry.** `CHANGELOG.md` `## [v2.1.0] - Unreleased` section gains `### W11-8 Layer A: Optional-installer framework — shared library + 5 new installer stubs + clamav refactor (DEC-PHASE11-011 landing)` subsection immediately after the W11-7 entry. Names: shared library at `lib/orionx-installer-common.sh` + 7 functions; 5 new installer stubs (ghidra, element, floss, trid, gomuks); install-clamav.sh refactored; content-presence Section 26 (5 assertions) + Section 25c regex loosen; W11-8b Layer B deferred payloads named (Awareness pane; install-devel/install-signature-base/install-remnux-mcp). `grep -Ec '^### W11-8 Layer A:' CHANGELOG.md >= 1` AND `grep -Fq 'DEC-PHASE11-011' CHANGELOG.md` AND `grep -Fq 'orionx-installer-common.sh' CHANGELOG.md` AND `grep -Fq 'install-ghidra.sh' CHANGELOG.md` AND `grep -Fq 'install-clamav.sh refactored' CHANGELOG.md` AND `grep -Fq 'W11-8b' CHANGELOG.md`.
7. **EC7: W11-8b Layer B seeded + iter-1 trailer.** MASTER_PLAN.md gains the W11-8b seed row at the end of this section (naming Awareness pane extension of `scripts/control_center/sections/awareness.py`; `install-devel.sh`; `install-signature-base.sh`; `install-remnux-mcp.sh` gated on W11-3b) AND the iter-1 trailer identical in structure to W11-7 iter-1 (line 3716). NO other MASTER_PLAN.md edits (mainline row line 2405, Phase 11 whole-slice EC line 2388's section-22 slot, Decision Log rows, DEC-PHASE11-011 rationale — all untouched). `grep -c '^#### W11-8 — Detail plan' MASTER_PLAN.md == 1` (this section itself). `grep -Fq 'W11-8 iter-1 (planner detail-plan):' MASTER_PLAN.md`.

**Forbidden shortcuts (LAYER A):**
- Do NOT ship `install-devel.sh`, `install-signature-base.sh`, or `install-remnux-mcp.sh` in this slice. Those are W11-8b's scope; adding them here couples Layer A to W11-3b (Node runtime) and W11-4b (Florian Roth acceptance) which are not landed yet.
- Do NOT edit `scripts/control_center/sections/awareness.py` or any Control Center source. The Awareness-pane badge is W11-8b's scope; adding it here couples Layer A to the W9-2 Python widget authority.
- Do NOT wire any installer into a 0500/0700 hook, `.desktop` launcher, systemd unit, or `$PATH` symlink. Optional installers are root-run operator commands invoked by absolute path.
- Do NOT set `set -euo pipefail` inside `lib/orionx-installer-common.sh`. The library is sourced; the caller owns shell options. Setting it in the library would hijack any future caller that intentionally uses different options.
- Do NOT make `lib/orionx-installer-common.sh` executable (`chmod +x`). It is sourced, not run. `test ! -x` MUST hold — an executable library is a Chesterton's Fence for a future reader who might try to invoke it and get confused when it does nothing.
- Do NOT SHA-256-pin upstream tarballs in Layer A. Follow the DEC-PHASE10-008 trust-on-first-use pattern: first green CI build of W11-8 with the installers exercised computes the real SHA-256; a follow-up commit pins it before the v2.1.0 tag. Layer A ships without pin sentinels (planner note: the pin follow-up is a MICRO-SLICE, not W11-8b — W11-8b is Awareness pane + 3 more installers, both scope-orthogonal).
- Do NOT re-open the base package list. `iso/config/package-lists/**` is forbidden. If a reader thinks an installer needs a base-list package as a dep (e.g., `install-element.sh` needs `apt-transport-https`), that install-time apt install is the installer's responsibility, not the base ISO's.

**Ready-for-guardian definition (LAYER A):**

The reviewer may declare `READY_FOR_GUARDIAN` when EC1..EC7 are all executable-verifiable green on the current implementer HEAD SHA AND the shellcheck-clean bar holds AND content-presence Section 25 + Section 26 both PASS on the built ISO squashfs AND the DEC-PHASE11-009 header block in `install-clamav.sh` is preserved verbatim (byte-diff against pre-refactor lines 1-6 = 0 changes).

**W11-8b — Layer B seed row (SEEDED, not planned):**

Deferred payloads: (i) Control Center Awareness pane badge extending `scripts/control_center/sections/awareness.py` (W9-2 authority; MUST NOT introduce a new UI authority — extend the existing pane) to read `/opt/orionx/optional/.installed/<name>.state` sentinel files (dropped by each installer at successful-completion time, per DEC-PHASE11-011's `.installed/<name>.state` mechanism) and display per-installer "installed / not installed / installer available" state in the Awareness pane's existing row rendering; (ii) `install-devel.sh` — restores the W11-2-removed dev toolchain (`build-essential`, `gcc`, `make`, `libssl-dev`, `python3-dev`) as an optional installer per DEC-PHASE11-011's mainline W11-8 row (line 2405); (iii) `install-signature-base.sh` — accepts the Florian Roth `signature-base` CC-BY-NC ruleset locally at operator-run time (network-required; downloads from `Neo23x0/signature-base`; unpacks into `/opt/orionx/yara/rules-signature-base/`; drops sentinel), tie-in to W11-4b's YARA freshen mechanism; (iv) `install-remnux-mcp.sh` — gated on W11-3b Node 20 LTS runtime landing; installs remnux-mcp-server GPL-3.0 via `npm install --production` under the W11-3b-staged Node runtime; drops sentinel. Sentinel-drop mechanism: each installer, upon successful completion, `mkdir -p /opt/orionx/optional/.installed/` + `touch /opt/orionx/optional/.installed/<name>.state`; the shared library gains an `orionx_mark_installed` helper in W11-8b (Layer A ships without the helper; the sentinel-drop is a Layer B concern because it is meaningless without the Awareness pane that reads it). Landing constraints: `install-remnux-mcp.sh` cannot land before W11-3b (dependency on Node 20 LTS runtime); `install-signature-base.sh` cannot land before W11-4b (dependency on YARA freshen authority); Awareness pane extension is W9-2 authority — the extending reviewer must verify no dual-authority arises with the existing Awareness pane state rendering; content-presence Section 27 (or renumbered per insertion order at W11-8b provision time) asserts the sentinel-file directory + Awareness pane wiring + 3 new installer stubs + `orionx_mark_installed` helper. Tracking mechanism: GitHub issue filed by `guardian:land` at Layer A landing time (title: `W11-8b: Layer B — Awareness pane + install-devel + install-signature-base + install-remnux-mcp (defer from W11-8)`; labels: `phase-11`, `w11-8b`, `priority:medium`, `deferred-from:w11-8`). If for any reason `guardian:land` cannot file the issue, an inline TODO under this seed row is the fallback record; the operator triages via the backlog skill at next dispatch.

**References:**
- Mainline W11-8 row (`MASTER_PLAN.md` line 2405) — preserved verbatim; this Layer A slice discharges the framework skeleton + 5 of the 5 originally-named installers minus install-devel + install-signature-base (both deferred to W11-8b with rationale) minus a shift in cast (adds install-floss + install-trid + install-gomuks — Layer B deferrals from W11-3 and W11-5 that arrived at W11-8 authoring time; adds install-remnux-mcp — deferred to W11-8b for Node dep).
- DEC-PHASE11-011 (optional-installer framework layout convention, `MASTER_PLAN.md` line 2339) — authority reused verbatim; this slice implements the "shared behavior" side of that DEC's contract.
- DEC-PHASE11-004 (Ghidra defer + RE toolkit, line 2331) — cited by `install-ghidra.sh` + `install-floss.sh` + `install-trid.sh` headers.
- DEC-PHASE11-007 (team comms, line 2335) — cited by `install-element.sh` + `install-gomuks.sh` headers.
- DEC-PHASE11-009 (ClamAV drop-to-optional, line 2337) — `install-clamav.sh`'s existing header authority; MUST be preserved verbatim across the refactor.
- W11-7 detail plan (this file, section starting line 3619) — closest structural analog; `install-clamav.sh` is the refactor target.
- W11-3 Layer A detail plan (line 3250) — FLOSS + TrID Layer B deferrals absorbed into W11-8 Layer A stubs.
- W11-5 Layer A detail plan (line 3424) — gomuks + element-desktop Layer B / Layer C deferrals absorbed into W11-8 Layer A stubs.
- Phase 11 whole-slice EC item 8 (line 2388) — Section 22 W11-8 slot renumbered to Section 26 per insertion order (same convention as W11-7 21→25).

**W11-8 iter-1 (planner detail-plan): 2026-07-19.** Detail plan appended to `MASTER_PLAN.md` on `develop`; NO new DEC (DEC-PHASE11-011 authority reused verbatim; DEC-PHASE11-004/007/009 referenced; DEC-PHASE7-041 cascade-consolidation discipline observed via the Layer A / Layer B split); Scope Manifest at `tmp/phase11-w11-8-optional-installers-scope.json` (LEGAL keys only: `allowed_paths`, `required_paths`, `forbidden_paths`, `authority_domains`, `state_domains`); synced to runtime via `cc-policy workflow scope-sync phase11-w11-8-optional-installers --work-item-id wi-phase11-w11-8-optional-installers --scope-file tmp/phase11-w11-8-optional-installers-scope.json` — 3 paths written, status=ok. Ready for `guardian:provision` under workflow_id `phase11-w11-8-optional-installers`.
...
#### W11-12 — Detail plan (Host-side `orionx-imager` — LAYER A: Python+tkinter GUI + bash CLI wrapper, macOS/Linux only; Windows + packaged installers deferred to W11-12b)

**Seeded:** 2026-07-19 planner amendment (this section). **Status:** ready for `guardian:provision`. **Env:** Host macOS/Linux (dev machine + operator laptop; NOT executed inside the ISO — this is a **host-side companion tool** that is versioned in-repo and downloaded/cloned by operators onto their imaging workstation). **Wave:** 7 (rider — depends on release-artifact shape from `.github/workflows/release.yml`; DOES NOT depend on any in-ISO Phase 11 slice landing; MAY land in parallel with any other Phase 11 slice; sequenced after W11-11 by planner-order only). **Gate:** review. **Weight:** L (substantial new feature under a NEW repo path `scripts/orionx-imager/`: ~500-line Python GUI + ~200-line bash CLI + 3 stdlib-only Python library modules + unit test + README; Layer B deferrals keep this from XL). **Deps:** none of the in-ISO W11-* slices; implicit dependency on `.github/workflows/release.yml` emitting `SHA256SUMS` next to `orionx-*.iso` at GitHub Releases (verified 2026-07-19 at release.yml lines 106-114).

**Operator directive verbatim (2026-07-18, incorporated into this planner amendment 2026-07-19):** "Ship a host-side tool modeled after Raspberry Pi Imager that: downloads latest release ISO from jarocki/orion GitHub Releases; verifies image integrity (SHA256); guides user through USB imaging with safety; ensures correct write to bootable USB."

**Problem.** Today, an operator preparing an Orion-X live-DVD USB stick must (a) find the correct GitHub Release, (b) pick the right ISO among possibly-multiple assets, (c) manually verify SHA256 against `SHA256SUMS` (many operators skip this), (d) identify the target USB device without picking the wrong disk, (e) run `dd if=... of=... bs=4M` correctly. Step (d) is the sharpest edge — writing to `/dev/disk0` (macOS system disk) or `/dev/sda` (Linux OS disk) is a data-loss-and-brick event. Step (c) is a supply-chain trust step that many operators skip because it's tedious. Raspberry Pi Imager solved this exact class of problem for RPi users; Orion-X needs the same shape of tool for the same reasons — a defensive live-DVD product only helps operators who can safely image the USB in the first place. This slice ships **Layer A**: a working CLI + basic tkinter GUI on macOS/Linux, stdlib-only (no pip install), that automates download + verify + write with safety guards. **Layer B** (Windows support, real GUI polish, packaged installers) is deferred to W11-12b so this slice can land tonight.

**Layer split rationale (DEC-PHASE7-041 cascade-consolidation discipline applied).** The full RPi-Imager equivalent (GUI polish, cross-platform packaging, code-signing, notarization, auto-update) is XL-plus and would cascade over multiple provisioning iterations. Layer A / Layer B split lets us land a real, working tool tonight — CLI + basic GUI, host-Python-3-stdlib-only, no new dependencies to install, on the two platforms where the developer + primary operator population lives (macOS + Linux). Layer B extends to Windows (which needs a PowerShell writer OR WSL2 bridge — non-trivial), proper progress bars, icons, pyinstaller/py2app bundles, code-signing. Layer B is a follow-up planner slice, not this slice.

**Scope.** Under a NEW directory `scripts/orionx-imager/`:
- `orionx-imager` — Python 3 executable (shebang `#!/usr/bin/env python3`), ~500 lines, tkinter GUI. Left panel = device picker (populated from `lib/devices.py`), right panel = ISO source picker (three modes: latest GitHub release, specific release tag, local file), center = write button (disabled until both selections + confirmation checkbox), progress area, log window.
- `orionx-imager-cli.sh` — bash CLI wrapper, ~200 lines, shellcheck-clean, for headless / scripted use.
- `lib/__init__.py` — empty (namespace marker).
- `lib/downloader.py` — GitHub Releases REST API v3 client (uses `urllib.request` stdlib only, no `requests`), ISO download with streaming + progress callback, SHA256 verification against the release's `SHA256SUMS` asset.
- `lib/writer.py` — platform-specific USB writer. macOS = `diskutil unmountDisk <target>` then `dd if=<iso> of=<target> bs=4m` (macOS `dd` uses lowercase `bs=4m`); Linux = `umount` any mounted partitions then `dd if=<iso> of=<target> bs=4M oflag=direct,sync` then `sync`. Elevation via `sudo` prompt (Layer A); pkexec / notarized helper deferred to Layer B.
- `lib/devices.py` — USB device enumeration. macOS = `diskutil list -plist` parsed via `plistlib` stdlib; Linux = `lsblk -Jo NAME,TYPE,SIZE,MODEL,VENDOR,RM,MOUNTPOINT` parsed via `json` stdlib. Filters to `RM=1` (removable) on Linux; on macOS filters to disks whose IORegistryEntryName includes "External" OR whose `RemovableMedia=true`.
- `README.md` — operator instructions: install (git clone or direct download), macOS quickstart (`./orionx-imager`), Linux quickstart (`sudo ./orionx-imager` or the CLI form), safety warnings, troubleshooting, Layer B deferral note (Windows → use WSL2 with the CLI for now).

Also creates/appends:
- `tests/unit/test_orionx_imager.sh` — host-side unit test asserting file presence + executable bits + Python syntax + shellcheck-clean + `--help` exits 0 + `--list-devices` exits 0 on the host runner + `--dry-run` refuses `/dev/disk0` and `/dev/sda`.
- `CHANGELOG.md` — new W11-12 entry appended under v2.1.0 section.
- `MASTER_PLAN.md` — this section stamp at T-final.

Scope Manifest lives at `tmp/phase11-w11-12-imager-tool-scope.json` (already authored by this planner slice; see keys in **Scope Manifest** subsection below).

**Explicitly out-of-scope for W11-12 (deferred to W11-12b Layer B):**
- Windows support (PowerShell writer OR WSL bridge OR native Windows Python packaging).
- Real progress-bar polish (Layer A shows a text log + a coarse tkinter ttk.Progressbar in indeterminate mode during write; deterministic progress requires teeing `dd` output through `pv` or reading `dd`'s SIGINFO on macOS / SIGUSR1 on Linux — deferred).
- Packaged installers (pyinstaller `.app` bundle for macOS, AppImage for Linux, `.msi` for Windows).
- Code-signing / notarization (macOS Gatekeeper + Linux distro-signed packages).
- Auto-update of the tool itself.
- Ship a `orionx-imager` icon and .desktop file.
- ISO caching policy beyond "keep last download in XDG cache; overwrite on re-download of same tag".
- Multi-ISO write (writing the same ISO to multiple USB sticks in parallel).
- Verification-after-write (comparing SHA256 of first N MB of the written USB to the ISO). Layer A prints "write complete + sync flushed"; verification-after-write is Layer B.

Rationale for Layer split: cascade risk. This slice already touches (a) a new repo directory (5 new files), (b) network I/O (GitHub API + release downloads), (c) block-device I/O (`dd` writes), (d) two platforms (macOS + Linux); adding Windows or notarization would push it to XL and increase iteration risk. Ship Layer A tonight; Layer B is a small, well-scoped follow-up (est. M-weight) once operators have used Layer A and reported the sharp edges.

**Design decisions embedded (all captured under DEC-PHASE11-015):**

1. **Stdlib-only Python.** No pip install, no venv, no third-party deps. macOS ships Python 3.9+ with tkinter and plistlib; Debian Bullseye + newer ships `python3` + `python3-tk` (tk requires the `python3-tk` apt package on Debian — the README documents this one preflight step for Linux operators). This keeps installation to "git clone and run"; no dependency drift over time; no supply-chain audit of a `requirements.txt` for a defensive-product tool.

2. **GitHub Releases REST API v3 (`api.github.com/repos/jarocki/orion/releases`).** No GitHub token required for public read (60 req/hr rate limit per IP is more than enough). "Latest release" = `/releases/latest`; specific tag = `/releases/tags/<tag>`. Asset selection: pick the FIRST asset whose `name` matches `orionx-*.iso` (planner asserts today's release ISO naming; if future releases add multiple ISOs, the tool falls back to prompting the user to choose). SHA256 manifest = the `SHA256SUMS` asset (release.yml lines 108-114 emit this).

3. **SHA256 verification is MANDATORY by default; `--skip-verify` is EXPLICIT opt-out.** The CLI accepts `--skip-verify` but prints a red warning + requires explicit `--yes-really-skip-verify` (two-flag opt-out). The GUI does not expose a skip-verify option in Layer A (defense-in-depth for the graphical user path).

4. **Safety guards — refuse-list, not allow-list.** macOS refuses `/dev/disk0` (always the boot volume container on Apple Silicon + Intel Macs); Linux refuses `/dev/sda` when `/dev/sda` is mounted OR contains the root filesystem (checks via `findmnt --target / --output SOURCE`), else warns loudly and requires the `--i-really-know-what-im-doing` two-word confirmation flag on the CLI (GUI hides that entire code path — no way to select a system disk in the GUI Layer A). This is a **refuse-list** because operators legitimately image USB drives that show up at `/dev/disk4`, `/dev/sdb`, `/dev/sdc`, etc.; an allow-list would be over-restrictive for the real diversity of imaging workstations.

5. **User confirmation of target — mandatory, unskippable in interactive modes.** Interactive CLI: prints target device name/vendor/size/mount-status, requires typing the target device path back (echo-confirmation). Interactive GUI: requires a checkbox + a modal "You are about to erase <device> — this cannot be undone. Type 'ERASE' to confirm." Confirmation is **unskippable** in interactive modes; only `--force` on the CLI bypasses (for scripting; documented as "for CI/testing only" in the README).

6. **Dry-run mode is the DEFAULT for first invocation.** The CLI's `--dry-run` prints "would download <URL>, verify against <SHA256>, then run: `sudo dd if=... of=... bs=4m` — NOT executing." This lets operators sanity-check every choice before committing. The GUI has a "Dry Run" button next to "Write" in Layer A.

7. **Elevation model — CLI runs unprivileged and re-invokes itself via sudo for the write step.** The Python tool detects if it can open the target device for write; if EACCES, it re-execs itself via `sudo -n` first (fails cleanly if sudo requires a password), then `sudo` with password prompt. On macOS the same pattern works; `diskutil unmountDisk` requires sudo too. This is Layer A's simplest model; Layer B may add a pkexec policy on Linux and an SMJobBless helper on macOS.

8. **XDG cache for downloaded ISOs.** ISOs land at `${XDG_CACHE_HOME:-$HOME/.cache}/orionx-imager/orionx-<tag>.iso` + `.sha256`. Re-downloading the same tag skips the network fetch if the cached SHA256 matches. `--clear-cache` empties the directory. No default cache-eviction policy in Layer A (operators can `rm -rf` the directory).

**Explicitly rejected alternatives (documented so future implementers do not re-propose them for this Layer A scope):**
- **Third-party libs (`requests`, `tqdm`, `PySide6`, etc.).** Requires pip / venv on the host; adds supply-chain audit surface; every version bump is a new audit; tkinter + stdlib is enough for Layer A. REJECTED.
- **Etcher fork / bundling.** Electron-based, ~200 MB per platform, not a fit for a "git clone and run" model. REJECTED.
- **`dd` progress via `pv` piping.** Requires `pv` installed on the host (not universal on macOS). Layer A uses indeterminate progress + a byte-count log line every ~5 seconds via a background thread reading `os.stat(target).st_size` on Linux OR sending `SIGINFO` to `dd` on macOS. Layer B may add `pv` opportunistically.
- **BitTorrent / IPFS distribution.** Nice for future decentralization but out-of-scope for Layer A (GitHub Releases is the current release authority). REJECTED for W11-12.
- **Building a full "Orion-X Setup" GUI (Wi-Fi config, first-boot options).** That is the first-boot wizard's job inside the ISO (Phase 6 authority). The imager writes the ISO; it does not configure the OS on the ISO. REJECTED for W11-12.
- **Cross-platform electron / Tauri GUI.** Same Etcher problem — not stdlib. REJECTED for Layer A; possibly revisit in Layer B or a separate long-arc slice.

**State-Authority Map:**

| State domain | Authority (this slice writes it) | Reader / consumer |
|--------------|----------------------------------|-------------------|
| Host tool source (Python + bash + libs + README) | `scripts/orionx-imager/**` (NEW single-authority path for the imager) | Host operator runs the tool from this path OR from a clone the operator makes |
| ISO download cache | `${XDG_CACHE_HOME:-$HOME/.cache}/orionx-imager/` | The tool re-reads on subsequent invocations to skip re-download; operator may inspect / clean manually |
| SHA256 integrity manifest | `SHA256SUMS` GitHub Release asset (emitted by `.github/workflows/release.yml` lines 108-114 — pre-existing authority, this slice READS only) | This tool verifies downloaded ISO against this manifest; does NOT re-emit or fork the SHA256SUMS format |
| USB target block device | Operator-selected `/dev/diskN` (macOS) or `/dev/sdX` (Linux); the tool writes via `dd` | The USB stick becomes bootable Orion-X media; consumed by BIOS/UEFI at operator's target hardware |
| Host tool unit test | `tests/unit/test_orionx_imager.sh` (NEW file) | CI runs on push to `develop`; may run locally via `bash tests/unit/test_orionx_imager.sh` |

**Preserves (untouched by this slice):**
- ISO release pipeline (`.github/workflows/release.yml`) — this slice READS the release artifacts + SHA256SUMS but does not modify the release pipeline.
- All in-ISO authorities (build-iso.sh, hooks, package lists, Control Center, Nebula, branding pipeline) — the imager is host-side; no ISO content changes.
- Existing tests (`tests/integration/**`, `tests/unit/**` other than the NEW `test_orionx_imager.sh`).
- DEC-PHASE7-002 single version authority `ORIONX_VERSION` — the imager reads release tags via the GitHub API, does not fork a version authority.
- DEC-PHASE10-014 CI pipefail — the new unit test script uses `set -euo pipefail`.
- All Phase 11 in-ISO decisions (DEC-PHASE11-001..-014) — the imager writes the shipped ISO to a USB; it does not re-open any in-ISO decision.

**Scope Manifest (LEGAL keys only: `allowed_paths`, `required_paths`, `forbidden_paths`, `authority_domains`, `state_domains`).** File: `tmp/phase11-w11-12-imager-tool-scope.json` (authored by this planner slice).

- `allowed_paths`: `scripts/orionx-imager/**`, `tests/unit/test_orionx_imager.sh`, `CHANGELOG.md`, `MASTER_PLAN.md`, `tmp/**`.
- `required_paths`: `scripts/orionx-imager/orionx-imager`, `scripts/orionx-imager/orionx-imager-cli.sh`, `scripts/orionx-imager/lib/__init__.py`, `scripts/orionx-imager/lib/downloader.py`, `scripts/orionx-imager/lib/writer.py`, `scripts/orionx-imager/lib/devices.py`, `scripts/orionx-imager/README.md`, `tests/unit/test_orionx_imager.sh`, `CHANGELOG.md`, `MASTER_PLAN.md`.
- `forbidden_paths`: `iso/**`, `.github/workflows/**`, `scripts/build-iso.sh`, `scripts/release/**`, `scripts/control_center/**`, `tests/integration/**`, `tests/unit/test_build_iso.sh`, `hooks/**`, `runtime/**`, `agents/**`, `settings.json`.
- `authority_domains`: `host_tool_imager_authority` (NEW — `scripts/orionx-imager/` is the single-authority path for downloader/verify/write logic), `release_download_source_authority` (GitHub Releases REST API v3 + `SHA256SUMS` asset — pre-existing, this tool READS), `usb_device_enumeration_authority` (macOS `diskutil list -plist` + Linux `lsblk -J`).
- `state_domains`: `host_filesystem` (XDG cache write), `host_block_devices` (operator-chosen USB target; refuse-list guards internal disks), `network_egress` (HTTPS to api.github.com + github.com).

Sync command (implementer to run BEFORE `guardian:provision` dispatch):
```
cc-policy workflow scope-sync phase11-w11-12-imager-tool \
  --work-item-id wi-phase11-w11-12-imager-tool \
  --scope-file tmp/phase11-w11-12-imager-tool-scope.json
```

**Task order (implementer executes T1..T-final in order; each is one commit):**

1. **T1 — Directory + Python namespace marker.** Create `scripts/orionx-imager/` and `scripts/orionx-imager/lib/`. Add `scripts/orionx-imager/lib/__init__.py` (empty; namespace only). Commit: `feat(w11-12): create scripts/orionx-imager/ layout (DEC-PHASE11-015)`.

2. **T2 — `lib/devices.py`.** Implement `list_usb_devices() -> list[dict]` returning `[{path, size_bytes, model, vendor, removable, mounted}]`. macOS branch: `subprocess.check_output(['diskutil', 'list', '-plist'])` + `plistlib.loads()` walk; filter by `RemovableMedia=True` OR IORegistryEntryName contains `External`. Linux branch: `subprocess.check_output(['lsblk', '-Jo', 'NAME,TYPE,SIZE,MODEL,VENDOR,RM,MOUNTPOINT'])` + `json.loads()` walk; filter to `TYPE=disk` AND `RM=True`. Return sorted by path. Include `is_system_disk(path) -> bool` helper: macOS returns `path == '/dev/disk0'`; Linux returns `path in ('/dev/sda',)` OR `findmnt --target / --output SOURCE` output starts with `path`. Includes a `refuse_write(path) -> str|None` returning a human-readable refusal reason if the path is a system disk, else `None`. Type hints via `from __future__ import annotations` (Bullseye Python 3.9 compat for CI). Commit: `feat(w11-12): lib/devices.py — USB enumeration + system-disk refuse-list (DEC-PHASE11-015)`.

3. **T3 — `lib/downloader.py`.** Implement `list_releases() -> list[dict]`, `get_release(tag: str = 'latest') -> dict`, `find_iso_asset(release: dict) -> dict`, `find_sha256sums_asset(release: dict) -> dict`, `download(url: str, dest: pathlib.Path, progress_cb=None) -> None` (streaming download with 64 KiB chunks; progress_cb receives `(bytes_read, total_bytes)`), `verify_sha256(iso_path: pathlib.Path, sha256sums_path: pathlib.Path) -> tuple[bool, str]` (parses SHA256SUMS, finds line matching `os.path.basename(iso_path)`, computes SHA256 of the file, returns `(matched, expected_sha)`). Uses `urllib.request` + `hashlib` stdlib. Handles GitHub API 403 rate-limit with a clear error message. XDG cache path: `os.environ.get('XDG_CACHE_HOME', str(pathlib.Path.home() / '.cache')) + '/orionx-imager/'`. Commit: `feat(w11-12): lib/downloader.py — GitHub Releases client + SHA256 verify (DEC-PHASE11-015)`.

4. **T4 — `lib/writer.py`.** Implement `unmount_target(path: str) -> None` (macOS: `diskutil unmountDisk <path>`; Linux: `umount <path>*` for each partition), `write_iso(iso_path: pathlib.Path, target: str, progress_cb=None) -> None` (macOS: `sudo dd if=<iso> of=<target> bs=4m`; Linux: `sudo dd if=<iso> of=<target> bs=4M oflag=direct,sync`; both invoke `sync` after). progress_cb receives `(bytes_written_estimate, total_bytes)` — implementation reads `os.stat(target).st_size` on Linux (block device size doesn't grow but wKB counter does — read `/sys/block/<name>/stat` column 6 for sectors written, multiply by 512); on macOS sends SIGINFO to dd every 2 seconds and captures stderr line. Elevation via `_reexec_sudo_if_needed()` at the top of `write_iso()`. Refuses to run if `devices.refuse_write(target)` returns non-None; the refusal reason is raised as a `WriteRefused` exception. Commit: `feat(w11-12): lib/writer.py — platform dd writer + safety refuse-list (DEC-PHASE11-015)`.

5. **T5 — `orionx-imager` (Python tkinter GUI).** Shebang `#!/usr/bin/env python3`, `chmod +x`. Layout: `ttk.Frame` root; left panel `ttk.Labelframe(text='SELECT DEVICE')` with a `ttk.Combobox` populated from `devices.list_usb_devices()`; right panel `ttk.Labelframe(text='SELECT ISO')` with three `ttk.Radiobutton`s (Latest release / Specific tag / Local file) + a `ttk.Entry` for tag / a `ttk.Button('Browse')` for local file; center a `ttk.Checkbutton('I confirm I want to erase this device')` + `ttk.Button('Write')` + `ttk.Button('Dry Run')`; bottom a `ttk.Progressbar(mode='indeterminate')` + a `tk.Text` log area. Write button is disabled until (device selected) AND (ISO source resolved) AND (confirm checkbox checked). On Write: pop a `tkinter.messagebox.askstring` requiring the operator to type `ERASE` to confirm; if confirmed, disable all inputs, start a background thread that (a) downloads if needed via `downloader`, (b) verifies SHA256, (c) unmounts + writes via `writer`, streaming log updates back via `queue.Queue`. No skip-verify option exposed in GUI Layer A. Approx 500 lines. Commit: `feat(w11-12): orionx-imager tkinter GUI (DEC-PHASE11-015)`.

6. **T6 — `orionx-imager-cli.sh` (bash CLI wrapper).** `#!/usr/bin/env bash` + `set -euo pipefail`. Options `--list-devices`, `--iso <path|url>`, `--iso-release latest|<tag>`, `--target <path>`, `--dry-run`, `--skip-verify --yes-really-skip-verify`, `--force`, `--clear-cache`, `--help`, `--version`. Delegates to Python via `python3 -c "from lib.downloader import ...; ..."` shellouts, OR (simpler) wraps a companion Python entry point `orionx-imager-cli` that lives in the same directory alongside the tkinter script (planner defers this decision to implementer; either shape is acceptable as long as shellcheck-clean bash is the operator-facing entry). Interactive confirmation flow: prints target device details, waits for the operator to type the device path back. `--force` bypasses interactive confirmation. Refuses `/dev/disk0` / `/dev/sda` per `refuse_write()`; only `--i-really-know-what-im-doing` bypasses (documented as CI/testing use only). Prints a colored PASS/FAIL summary line on completion. Commit: `feat(w11-12): orionx-imager-cli.sh — bash CLI wrapper (DEC-PHASE11-015)`.

7. **T7 — `README.md`.** Sections: What this is (RPi-Imager-like host tool for Orion-X); Install (git clone; macOS prereqs `python3 --version` ≥ 3.9; Linux prereqs `sudo apt install python3-tk`); Quickstart GUI (`cd scripts/orionx-imager && ./orionx-imager`); Quickstart CLI (`./orionx-imager-cli.sh --iso-release latest --target /dev/diskN`); Safety notes (refuse-list, mandatory confirm, mandatory SHA256 by default); Troubleshooting (rate-limit → wait 60 min or use `--iso <local-file>`; Linux "Permission denied" → run under sudo OR add self to `disk` group; macOS "Resource busy" → the tool auto-unmounts, if persistent unmount manually via `diskutil unmountDisk /dev/diskN` first); Layer B deferrals (Windows, GUI polish, packaged installers, verification-after-write); License (project license inherits). Commit: `docs(w11-12): README for orionx-imager (DEC-PHASE11-015)`.

8. **T8 — `tests/unit/test_orionx_imager.sh`.** `#!/usr/bin/env bash` + `set -euo pipefail`. Asserts: (a) all 6 required files exist; (b) `orionx-imager` + `orionx-imager-cli.sh` are executable; (c) `python3 -c "import ast; ast.parse(open('scripts/orionx-imager/orionx-imager').read())"` for each Python file (syntax valid); (d) `bash -n scripts/orionx-imager/orionx-imager-cli.sh` (bash syntax valid); (e) `shellcheck scripts/orionx-imager/orionx-imager-cli.sh` (installed on CI; skip locally with a note if shellcheck absent); (f) `scripts/orionx-imager/orionx-imager-cli.sh --help` exits 0 and prints `--iso-release`, `--target`, `--dry-run`, `--skip-verify`; (g) `scripts/orionx-imager/orionx-imager-cli.sh --list-devices` exits 0 (may return empty list on a CI runner with no USB — that's fine, tool must not crash); (h) `scripts/orionx-imager/orionx-imager-cli.sh --dry-run --iso /tmp/fake.iso --target /dev/disk0` (macOS) OR `/dev/sda` (Linux) exits NON-ZERO with a message matching `refus|system disk` (refuse-list guard); (i) `scripts/orionx-imager/orionx-imager-cli.sh --dry-run --iso /tmp/fake.iso --target /dev/disk99 --force` (a non-existent path that is not the refuse-list) exits 0 or exits with a "does not exist" message (dry-run doesn't actually write). Commit: `test(w11-12): tests/unit/test_orionx_imager.sh — 9 assertions (DEC-PHASE11-015)`.

9. **T9 — CHANGELOG v2.1.0 W11-12 entry.** Prepend `### W11-12 Layer A: host-side orionx-imager (Python + tkinter GUI + bash CLI)` block under the existing v2.1.0 section. Content: names the tool location + macOS/Linux support (Windows deferred to W11-12b) + GitHub Releases download + mandatory SHA256 verify + `/dev/disk0`/`/dev/sda` refuse-list + interactive confirmation + dry-run default + DEC-PHASE11-015 reference + defers Windows + GUI polish + packaged installers + verification-after-write to W11-12b. Commit: `docs(w11-12): CHANGELOG v2.1.0 W11-12 entry (DEC-PHASE11-015)`.

10. **T10 — Decision Log DEC-PHASE11-015 row.** Append one new row `| DEC-PHASE11-015 | 2026-07-19 | [PHASE 11 W11-12 HOST-SIDE ORIONX-IMAGER — LAYER A] ...` under the DEC-PHASE11-014 row (the last Phase 11 DEC). Rationale text captures the operator directive verbatim + the Layer A/B split + stdlib-only Python design + refuse-list safety model + mandatory SHA256 + explicit rejection of Etcher fork / third-party libs / BitTorrent / cross-platform Electron / Windows-in-Layer-A. Commit: `docs(w11-12): DEC-PHASE11-015 host-side imager tool (Layer A)`.

11. **T-final — MASTER_PLAN W11-12 iter annotation.** Append `**W11-12 iter-2 (implementer landing): 2026-07-XX @ <commit_sha>.** Detail plan iter-1 authored 2026-07-19; implementer landed at <sha>; DEC-PHASE11-015 recorded; Scope Manifest at `tmp/phase11-w11-12-imager-tool-scope.json`.` to this W11-12 section. Do NOT edit permanent Decision Log rows, DEC-PHASE11-014, or the umbrella W11-12 mainline row. DEC-PHASE11-015 goes as a NEW row appended to the Decision Log table.

**Evaluation Contract (8 items; all must PASS for `REVIEW_VERDICT=ready_for_guardian`):**

1. **`scripts/orionx-imager/orionx-imager` exists, is executable, and passes Python syntax check:** `test -x scripts/orionx-imager/orionx-imager` returns 0 AND `python3 -c "import ast; ast.parse(open('scripts/orionx-imager/orionx-imager').read())"` returns 0. Verified by unit test T8-a/b/c.

2. **`scripts/orionx-imager/orionx-imager-cli.sh` exists, is executable, passes bash syntax, and is shellcheck-clean:** `test -x scripts/orionx-imager/orionx-imager-cli.sh` returns 0 AND `bash -n scripts/orionx-imager/orionx-imager-cli.sh` returns 0 AND `shellcheck scripts/orionx-imager/orionx-imager-cli.sh` returns 0 (skip if shellcheck not installed on the CI runner — record the skip; reviewer verifies shellcheck ran on at least the implementer's local host or CI). Verified by unit test T8-b/d/e.

3. **`lib/downloader.py`, `lib/writer.py`, `lib/devices.py` all exist and pass Python syntax check:** `python3 -c "import ast; ast.parse(open('scripts/orionx-imager/lib/<name>.py').read())"` returns 0 for each. Verified by unit test T8-c (extended over the three lib files).

4. **`orionx-imager-cli.sh --help` exits 0 and prints all documented options:** `bash scripts/orionx-imager/orionx-imager-cli.sh --help` returns 0; stdout contains substrings `--iso-release`, `--target`, `--dry-run`, `--skip-verify`, `--list-devices`, `--force`. Verified by unit test T8-f.

5. **`orionx-imager-cli.sh --list-devices` runs cleanly on the host:** `bash scripts/orionx-imager/orionx-imager-cli.sh --list-devices` returns 0; may print an empty device list on CI runners with no USB (that is a PASS — the tool must not crash on no-USB). Verified by unit test T8-g.

6. **`--dry-run --iso <local-iso-path> --target /dev/diskN` prints the intended plan without writing:** For a non-refuse-list path, `bash scripts/orionx-imager/orionx-imager-cli.sh --dry-run --iso /tmp/fake.iso --target /dev/disk99 --force` returns 0 AND stdout contains `DRY RUN` OR `would run` OR `NOT executing`; NO `dd` process is spawned (reviewer greps ps output during test OR the tool logs "dry-run" before any subprocess invocation). Verified by unit test T8-i (or a strengthened variant that traces subprocess calls).

7. **Refuses write to `/dev/disk0` (macOS) or `/dev/sda` (Linux) — safety refuse-list:** `bash scripts/orionx-imager/orionx-imager-cli.sh --dry-run --iso /tmp/fake.iso --target /dev/disk0` (or `/dev/sda`) exits NON-ZERO with stderr containing a message matching `refus|system disk|internal disk`. Even with `--force`, the refuse-list requires the explicit `--i-really-know-what-im-doing` two-word confirmation flag. Verified by unit test T8-h.

8. **`README.md` names the safety model + the layer B deferrals explicitly:** `grep -F '/dev/disk0' scripts/orionx-imager/README.md` returns at least one hit; `grep -F 'SHA256' scripts/orionx-imager/README.md` returns at least one hit; `grep -F 'W11-12b' scripts/orionx-imager/README.md` returns at least one hit (documents Windows + GUI polish + packaged installers deferral). Verified by reviewer grep at review time.

**Required tests (T1-based execution order):**
- `bash tests/unit/test_orionx_imager.sh` — all 9 assertions PASS.
- Manual (implementer's dev host, macOS OR Linux): `cd scripts/orionx-imager && ./orionx-imager-cli.sh --help` prints help; `./orionx-imager-cli.sh --list-devices` prints the operator's actual USB devices; `./orionx-imager-cli.sh --dry-run --iso <any-file> --target <a-real-external-USB-path>` prints the intended plan without writing.
- Manual GUI smoke (implementer's dev host, macOS OR Linux with `python3-tk` installed): `python3 scripts/orionx-imager/orionx-imager` opens a tkinter window; device combobox populates; ISO source radio buttons visible; Write button disabled until all preconditions met.

**Required evidence (implementer paste-back at review time):**
- Output of `bash tests/unit/test_orionx_imager.sh` (9 PASS lines).
- Output of `./orionx-imager-cli.sh --help` (documents all options).
- Output of `./orionx-imager-cli.sh --list-devices` on the implementer's host (shows real USB devices).
- Output of `./orionx-imager-cli.sh --dry-run --iso /tmp/fake.iso --target /dev/disk0` on macOS OR `/dev/sda` on Linux (proves refuse-list guard).
- Screenshot (or `xdotool --print-window-info` / macOS `screencapture` invocation record) of the tkinter GUI open on the implementer's host — if unavailable in the implementer environment, a note in review saying "GUI smoke deferred to Layer B or operator verification at W11-12 landing" is acceptable at reviewer discretion.

**Real-path checks:**
- The tool actually spawns `diskutil list -plist` on macOS OR `lsblk -J` on Linux and successfully parses the output on the implementer's host (verified by the `--list-devices` command returning a real device list, not an error).
- The tool actually reaches `api.github.com/repos/jarocki/orion/releases/latest` and receives a JSON response (verified by `./orionx-imager-cli.sh --dry-run --iso-release latest --target /dev/disk99` printing the resolved ISO URL). May be gated by rate-limit; if 403, log a WARN and mark as "gated by rate-limit — retry once" in the evidence paste.

**Required authority invariants:**
- `scripts/orionx-imager/` is the ONLY path in the repo that implements ISO download / SHA256 verify / USB write logic. Reviewer greps `grep -rn 'sha256sum\|sha256Sum\|SHA256SUMS' --include='*.py' --include='*.sh' -- scripts/ tests/` — hits outside `scripts/orionx-imager/` and unit test AND `.github/workflows/release.yml` are a scope violation.
- `.github/workflows/release.yml` is NOT modified by this slice (forbidden path).
- No changes to `iso/**`, `hooks/**`, `scripts/build-iso.sh`, `scripts/release/**` (all forbidden).
- Python code passes `from __future__ import annotations` + Bullseye Python 3.9 compat (per DEC-PHASE9-019/-020/-021 hard invariants — this tool runs on host Python, not Bullseye Python, but the annotation forbearance keeps the option open for a Layer B "run on the ISO" mode).

**Required integration points:**
- GitHub Releases REST API v3 endpoint `https://api.github.com/repos/jarocki/orion/releases/latest` — verified reachable and returns JSON with `assets` array containing an `orionx-*.iso` asset + a `SHA256SUMS` asset (verify at implementer time; the current release should have these per release.yml lines 106-114).
- `SHA256SUMS` format is `<sha256>  <filename>` per line (2 spaces between; standard `sha256sum` output) — the parser matches on `filename` substring.
- Host OS `dd` binary is present at `/bin/dd` (macOS) or `/usr/bin/dd` (Linux) — the writer does not fall back to a pure-Python write in Layer A (deferred to Layer B if needed).

**Forbidden shortcuts:**
- Do NOT add `requests`, `tqdm`, `PySide6`, `pyinstaller`, or any pip-installable dependency (stdlib-only invariant per DEC-PHASE11-015).
- Do NOT hardcode any specific release tag or SHA256 into the tool — the tool ALWAYS resolves via GitHub API + SHA256SUMS asset.
- Do NOT bypass the refuse-list without the explicit two-word `--i-really-know-what-im-doing` flag on the CLI.
- Do NOT expose a skip-verify option in the tkinter GUI (Layer A defense-in-depth; skip-verify is CLI-only + two-flag opt-out).
- Do NOT touch `iso/**`, `.github/workflows/**`, `scripts/build-iso.sh`, `scripts/release/**`, `scripts/control_center/**`, `tests/integration/**`, `hooks/**`, or any runtime/agent path (all forbidden per Scope Manifest).
- Do NOT introduce a config file / TOML / YAML in Layer A — CLI flags + XDG cache + refuse-list constants are enough.
- Do NOT ship a `.desktop` entry, icon, or menu wiring in Layer A (Layer B when we have packaged installers).
- Do NOT combine this slice with W11-11 (in-ISO diag tool) — separate slice, separate branch, separate DEC. This is an EXPLICITLY host-side companion, not an in-ISO addition.

**Ready for guardian when:** Evaluation Contract items 1-8 all pass; reviewer verdict `REVIEW_VERDICT=ready_for_guardian`; test-state is `pass` on head SHA; CHANGELOG entry present; MASTER_PLAN.md this section stamped with iter-2 landing note; DEC-PHASE11-015 row appended to Decision Log; Scope Manifest untouched during implementation (no path added, none removed).

**Rollback boundary.** If the built tool fails to run on either host platform (e.g., tkinter import fails on Debian without `python3-tk` — README documents the preflight), `git revert` the branch merge — pre-W11-12 develop has NO `scripts/orionx-imager/` directory, so the revert is atomic and safe. No in-ISO surface, no CI-published-artifact surface, no runtime state to unwind. The failure surface is entirely (i) `git clone` + `./orionx-imager*` (operator invocation), (ii) network + local block I/O (operator machine, not CI); there is no way this slice breaks the ISO build, the release pipeline, or any deployed system. Rollback risk: **low**.

**Follow-up seeded (NOT this slice) — W11-12b Layer B:** Windows support (PowerShell writer OR WSL2 bridge invoking the Linux CLI); real progress bar (via `pv` piping OR reading `/sys/block/<dev>/stat`); packaged installers (pyinstaller `.app` for macOS + AppImage for Linux + `.msi` for Windows); code-signing / notarization; auto-update; verification-after-write (compare SHA256 of first N MB of the written USB against the ISO); optional icon + `.desktop` entry; possibly a menu entry in the Control Center Awareness pane inside the ISO ("prepare another USB stick" — for post-boot re-imaging workflows). **Tracker:** file a follow-up issue at implementer landing time referencing this section as the parent seed; title `W11-12b: Layer B — Windows + packaged installers + polish (defer from W11-12)`; labels `phase-11`, `w11-12b`, `priority:medium`, `deferred-from:w11-12`.

**W11-12 iter-1 (planner detail-plan): 2026-07-19.** Detail plan appended to `MASTER_PLAN.md` on `develop`; NEW DEC-PHASE11-015 added to Decision Log at implementer T10 (host-side imager tool authority); Scope Manifest at `tmp/phase11-w11-12-imager-tool-scope.json` (LEGAL keys only: `allowed_paths`, `required_paths`, `forbidden_paths`, `authority_domains`, `state_domains`); to be synced to runtime by the operator/implementer via `cc-policy workflow scope-sync phase11-w11-12-imager-tool --work-item-id wi-phase11-w11-12-imager-tool --scope-file tmp/phase11-w11-12-imager-tool-scope.json` immediately prior to `guardian:provision` dispatch. Ready for `guardian:provision` under workflow_id `phase11-w11-12-imager-tool`.

---

### Phase 11 Documentation Refresh (workflow_id `phase11-docs-refresh`)

**Slice intent (DOCS-ONLY).** Refresh operator-visible documentation to reflect the current state of `develop` @ `b25159a` after twelve Phase 11 slices landed on 2026-07-19: W11-1 (Qwen swap), W11-2 (debloat + bootloader single-authority) plus hotfixes W11-2b/c/d/f, W11-9a/9a2/9b (cyberdeck branding chain), W11-3/4/5/6 Layer A (RE toolkit, YARA rulesets, team comms, Suricata IDS), W11-7 (ClamAV drop-to-optional), W11-8 (optional installer framework), W11-11 (in-ISO `orionx-diag`), W11-12 (host-side `orionx-imager`). Operator directive 2026-07-19: "update all of the documentation (starting with README.md)". This is a **documentation-only** slice — zero source, ISO config, hook, runtime, test, or CI edits. The docs must adopt the current identity ("cyberdeck for the Good Guys"), the v2.1.0-dev version string, and the two new operator tools that shipped in W11-11 and W11-12.

**Framing.** Documentation was the last surface not refreshed by any Phase 11 slice. Individual slices updated CHANGELOG.md (well-covered — every W11-* entry present) and the plan-level DEC rows, but the operator-facing surfaces (`README.md`, `docs/User_Guide.md`, `docs/release-process.md`, plus two missing new-tool guides) still describe a `v2.0.0-rc1` product. The operator preparing hardware installation in parallel needs current, coherent documentation immediately. This slice is intentionally scoped to be shippable in a single implementer pass with no dependencies on any concurrent slice.

**Alignment with existing authorities.**
- CHANGELOG.md is the historical release-notes authority (already current — v2.1.0 unreleased section is populated by prior slices; only a brief unreleased-summary paragraph is added here).
- MASTER_PLAN.md Decision Log is the authoritative home of plan-level DECs (DEC-PHASE11-001..015 are already recorded; docs must CITE these, not restate them).
- Root `DECISIONS.md` is auto-generated by `stop.sh` from `@decision` annotations (per its header comment) — this slice adds ONLY a one-line pointer at the top noting that plan-level DECs live in MASTER_PLAN; the auto-generated body is left untouched.
- `docs/orionx-diag.md` and `docs/orionx-imager.md` are NEW authoritative operator guides for the two W11-11 / W11-12 tools; the tool sources under `scripts/` remain the code-truth authority for behavior — the docs describe *usage*, not implementation.

**State-Authority Map.**
| Doc surface | Authority | This slice's action |
|---|---|---|
| README.md | Root operator entry point | Full rewrite |
| docs/orionx-diag.md | Operator guide for W11-11 tool | NEW (create) |
| docs/orionx-imager.md | Operator guide for W11-12 tool | NEW (create) |
| docs/User_Guide.md | Comprehensive user manual | Targeted refresh (version + new sections + Nebula/comms updates) |
| docs/release-process.md | Release cadence authority | Targeted refresh (v2.1.0-rc1 next example + hotfix pattern) |
| CHANGELOG.md | Release-notes authority | Append brief unreleased-summary paragraph only |
| DECISIONS.md (root, auto-generated) | Auto-derived from `@decision` annotations | Add one-line pointer to MASTER_PLAN Decision Log |
| docs/DEVELOPMENT_CHECKLIST.md | Contributor checklist | Light touch: reference Phase 11 slice/hotfix cadence |
| docs/CONTRIBUTING.md | Contributor guide | Light touch: minor version references |

**Work items (single-slice; no sub-waves).**

| Task | Deliverable | Wave |
|---|---|---|
| T1 | Full rewrite of `README.md`: v2.1.0-dev header, "cyberdeck for the Good Guys" identity paragraph, current key-features list (Nebula Qwen-3B, MCP tool server, mesh + Matrix + gomuks + matrix-commander, Suricata IDS lazy-start, YARA + capa + FLOSS + radare2 RE toolkit, cyberdeck branding chain, `orionx-diag`, `orionx-imager`, optional installer framework), Phase 11 arc summary (~1 paragraph pointing at MASTER_PLAN Phase 11 section), pointers to `docs/User_Guide.md`, `docs/orionx-diag.md`, `docs/orionx-imager.md`, `docs/release-process.md`, `CHANGELOG.md`. Preserve the license/support pointers at the bottom. | 1 |
| T2 | CREATE `docs/orionx-diag.md`: operator guide for the in-ISO diagnostic tool per DEC-PHASE11-015. Cover: purpose, invocation (`orionx-diag`), all ten diagnostic categories (as encoded in the shipped script; do not invent categories — cite `scripts/orionx-diag` or the W11-11 detail plan as source of truth), `--json` output shape, `--category <name>` filter, `--version` sub-command showing the `/etc/orionx-version` MANIFEST, exit-code semantics, sample OK output, sample FAIL output, "what to send to support" section referencing `--json` capture. | 1 |
| T3 | CREATE `docs/orionx-imager.md`: operator guide for the host-side USB writer per DEC-PHASE11-015 (W11-12 landing). Cover: purpose (download-and-write an Orion-X ISO to a USB stick from macOS/Linux), preflight (Python 3.10+, `python3-tk` on Debian for GUI mode), CLI usage with flags, GUI usage, safety guardrails (device-selector guard, `--force`, `--dry-run` if present), SHA256 verification flow, known-limitation callout: NO Windows support in Layer A (deferred to W11-12b), NO packaged installers yet (`git clone` + `./orionx-imager*` invocation). | 1 |
| T4 | UPDATE `docs/User_Guide.md`: (a) replace `v2.0.0-rc1` version strings with `v2.1.0-dev` throughout (mechanical); (b) refresh the Nebula / AI section to reference Qwen2.5-3B-Instruct Q4_K_M (Apache-2.0) per DEC-PHASE11-002 — do NOT restate the DEC, cite it; (c) refresh the team-comms section to include `matrix-commander` (CLI) and `gomuks` (TUI) per DEC-PHASE11-007; (d) APPEND three new sections near the end (before the appendix / license section): "Diagnostic Tool (`orionx-diag`)" pointing to `docs/orionx-diag.md`; "Optional Installers" describing the `/opt/orionx/optional/` layout convention per DEC-PHASE11-011; "Cyberdeck Branding" briefly describing the Plymouth / GRUB / isolinux / LightDM / MOTD chain per DEC-PHASE11-010. | 1 |
| T5 | UPDATE `docs/release-process.md`: change the running example from `v2.0.0-rc1` → `v2.1.0-rc1` (next planned RC); add a short subsection "Hotfix cadence (Phase 11 pattern)" documenting how W11-2b/c/d/f followed W11-2 as targeted post-slice patches — small commits, `fix(phase11)` prefix, one-DEC scope, immediate re-cut of the RC when a shipped ISO needed the fix. | 1 |
| T6 | UPDATE `CHANGELOG.md`: append a 2-3 sentence unreleased-summary paragraph at the top of the `## [v2.1.0] - Unreleased` section summarizing the twelve landed slices as a Phase 11 arc. Do NOT rewrite existing entries. | 1 |
| T7 | LIGHT-TOUCH `docs/DEVELOPMENT_CHECKLIST.md` + `docs/CONTRIBUTING.md`: version-string refresh and a one-line reference to the Phase 11 slice/hotfix cadence documented in `docs/release-process.md`. | 1 |
| T8 | LIGHT-TOUCH `DECISIONS.md` (root): prepend a single note above the auto-generated body: "Plan-level Phase 11 decisions (DEC-PHASE11-001..015) live in `MASTER_PLAN.md` Decision Log; this file is auto-generated by `stop.sh` from `@decision` source annotations and may lag." Do NOT edit the auto-generated body. | 1 |
| T9 | Verify all cross-references: `grep -c v2.0.0-rc1 README.md docs/User_Guide.md docs/release-process.md` should be 0 for updated version strings (excluding historical CHANGELOG entries and MASTER_PLAN historical references); every doc pointer resolves to an existing file. `bash -n` any embedded shell snippets in the new files. | 1 |
| T10 | Append "**Phase 11 Docs Refresh iter-1 (planner detail-plan): 2026-07-19.**" stamp under this section confirming completion. Do NOT add a new DEC row (this slice cites existing DEC-PHASE11-001..015; it does not introduce a new decision). | 1 |

Waves: single wave (T1..T10 all independent doc surfaces; no ordering constraint beyond "T9 verifies after T1..T8").

**Scope Manifest (canonical; written to `tmp/phase11-docs-refresh-scope.json`; sync via `cc-policy workflow scope-sync phase11-docs-refresh --work-item-id wi-phase11-docs-refresh --scope-file tmp/phase11-docs-refresh-scope.json` before `guardian:provision`).**

- `allowed_paths`: `README.md`, `docs/**`, `CHANGELOG.md`, `DECISIONS.md`, `MASTER_PLAN.md` (annotation stamp only — do not touch Identity / Decision Log / permanent rows), `tmp/**`.
- `required_paths`: `README.md`, `docs/orionx-diag.md`, `docs/orionx-imager.md`, `docs/User_Guide.md`, `docs/release-process.md`, `CHANGELOG.md`.
- `forbidden_paths`: `iso/**`, `scripts/**`, `tests/**`, `hooks/**`, `runtime/**`, `.github/**`, `Makefile`, `manifest.json`, `settings.json`, `AGENTS.md`, `CLAUDE.md`, `agents/**`.
- `authority_domains`: `documentation_authority`.
- `state_domains`: `docs_readme`, `docs_user_guide`, `docs_release_process`, `docs_changelog`, `docs_new_tool_guides`.

**Evaluation Contract (guardian-bound; 8 items).**

1. `README.md` header line reads `# Orion-X Phoenix Edition v2.1.0-dev` (or equivalent v2.1.0-dev branding); contains the phrase "cyberdeck for the Good Guys" verbatim; mentions BOTH `orionx-diag` and `orionx-imager` by name with pointers to their new guide pages; contains a Phase 11 arc summary paragraph.
2. `docs/orionx-diag.md` exists; covers the ten diagnostic categories (names sourced from the shipped `scripts/orionx-diag` — implementer verifies count and names by inspection, not invention); documents `--json`, `--category`, and `--version` sub-commands or flags as they exist in the shipped script; includes at least one sample OK output and one sample FAIL output.
3. `docs/orionx-imager.md` exists; covers CLI usage, GUI usage, macOS + Linux preflight (calling out `python3-tk` for GUI on Debian), device-selector safety guardrails, SHA256 verification, and the Layer A / Layer B split (no Windows, no packaged installers yet).
4. `docs/User_Guide.md` title / TOC / "About" version strings updated to `v2.1.0-dev`; Nebula section cites DEC-PHASE11-002 and names Qwen2.5-3B-Instruct Q4_K_M; team-comms section names `matrix-commander` and `gomuks` and cites DEC-PHASE11-007.
5. `docs/User_Guide.md` contains three NEW sections (titles substantively matching): "Diagnostic Tool (`orionx-diag`)", "Optional Installers", "Cyberdeck Branding" — each with a pointer to the authoritative source (guide file, `/opt/orionx/optional/README.md`, and MASTER_PLAN DEC-PHASE11-010 respectively).
6. `docs/release-process.md` running example uses `v2.1.0-rc1` as the next RC; contains a "Hotfix cadence (Phase 11 pattern)" section (or equivalent titled subsection) describing the W11-2b/c/d/f pattern; historical `v2.0.0` references retained where they document the rc9 baseline (do not scrub history — only refresh the forward-facing example).
7. `grep -R --include='*.md' -n 'orionx-phoenix-edition-v2.0.0-rc1.iso' README.md docs/` returns zero hits in forward-facing example commands (historical CHANGELOG rc9 lines and MASTER_PLAN pre-Phase-11 sections are out-of-scope for this slice). No path under `iso/**`, `scripts/**`, `tests/**`, `hooks/**`, `runtime/**`, `.github/**`, `Makefile`, `manifest.json`, `settings.json`, `AGENTS.md`, `CLAUDE.md`, or `agents/**` was modified.
8. `bash -n` on each embedded fenced ` ```bash ` / ` ```sh ` block in the two NEW files (`docs/orionx-diag.md`, `docs/orionx-imager.md`) exits 0 (implementer may extract to `tmp/*.sh` and run `bash -n`; the check itself is out-of-tree).

**Real-path checks.**
- `git diff --stat develop..HEAD` shows only docs/config-of-docs surfaces (no `iso/`, `scripts/`, `tests/`, `hooks/`, `runtime/` paths in the changed-files list).
- `head -1 README.md` prints a v2.1.0-dev header.
- `test -f docs/orionx-diag.md && test -f docs/orionx-imager.md` — both new files exist.

**Authority invariants (must not be violated).**
- MASTER_PLAN.md permanent sections (Identity, Architecture, Design Principles, Decision Log rows DEC-001 through DEC-PHASE11-015) are NOT modified.
- Root `DECISIONS.md` auto-generated body is NOT modified (only a one-line pointer is prepended above it).
- CHANGELOG.md existing v2.1.0 W11-* entries are NOT rewritten; only a summary paragraph is prepended within the same section.

**Forbidden shortcuts.**
- Do NOT edit any file under `scripts/**` "to make the docs match" — code is truth; the docs describe shipped behavior. If shipped tool behavior diverges from what docs would say, the docs must describe *actual* shipped behavior and (if truly wrong) file a follow-up issue — not silently patch source in a docs slice.
- Do NOT hand-edit the auto-generated body of root `DECISIONS.md`; that is a `stop.sh` regeneration surface.
- Do NOT invent Phase 11 features or slices that did not land. Cross-reference the actual DEC-PHASE11-001..015 rows and the twelve W11-* section headers already in MASTER_PLAN.md.
- Do NOT combine this slice with any source-code change. If a doc says "the tool does X" and the tool does not do X, file an issue — do NOT patch the tool in this branch.
- Do NOT touch `AGENTS.md`, `CLAUDE.md`, `settings.json`, hooks, or the runtime — those are constitution-level and out of scope for a docs slice.

**Ready for guardian when:** all 8 Evaluation Contract items pass; reviewer verdict `REVIEW_VERDICT=ready_for_guardian`; test-state gate is `pass` (docs-only slice — no source tests to run, but shell-syntax checks per EC8 must pass); this section stamped with iter-1 landing note at T10; Scope Manifest untouched during implementation (no path added, none removed).

**Rollback boundary.** If any doc merge breaks a link or misstates shipped behavior in a way an operator would notice at the hardware test, `git revert` the branch merge — pre-slice `develop` docs are stale but *coherent* for the v2.0.0-rc1 line, so revert is safe. No hook, no service, no runtime state, no CI job depends on any of these files. Rollback risk: **very low**.

**Follow-up seeded (NOT this slice).** If the operator later requests a full `docs/` structural overhaul (multi-guide split, generated site, screenshots refresh for cyberdeck theme), file that as `W11-13` or a v2.1.0 documentation initiative — out of scope for this slice.

**Phase 11 Docs Refresh iter-1 (planner detail-plan): 2026-07-19.** Detail plan appended to `MASTER_PLAN.md` on `develop`; NO new DEC row (slice cites existing DEC-PHASE11-001..015); Scope Manifest at `tmp/phase11-docs-refresh-scope.json` (LEGAL keys only: `allowed_paths`, `required_paths`, `forbidden_paths`, `authority_domains`, `state_domains`); to be synced to runtime by the operator/implementer via `cc-policy workflow scope-sync phase11-docs-refresh --work-item-id wi-phase11-docs-refresh --scope-file tmp/phase11-docs-refresh-scope.json` immediately prior to `guardian:provision` dispatch. Ready for `guardian:provision` under workflow_id `phase11-docs-refresh`.
...
| DEC-PHASE11-015 | 2026-07-19 | [PHASE 11 W11-12 HOST-SIDE ORIONX-IMAGER TOOL — LAYER A: PYTHON+TKINTER GUI + BASH CLI (macOS/Linux); WINDOWS + PACKAGED INSTALLERS DEFERRED TO W11-12b] Operator directive verbatim 2026-07-18: "Ship a host-side tool modeled after Raspberry Pi Imager that: downloads latest release ISO from jarocki/orion GitHub Releases; verifies image integrity (SHA256); guides user through USB imaging with safety; ensures correct write to bootable USB." Scope of DEC-PHASE11-015: a NEW single-authority path `scripts/orionx-imager/` becomes the ONLY location in the repo that implements ISO download + SHA256 verify + USB write logic. Ships six files: `orionx-imager` (Python 3 executable, tkinter GUI, stdlib-only), `orionx-imager-cli.sh` (bash CLI wrapper, shellcheck-clean), `lib/{__init__.py, downloader.py, writer.py, devices.py}` (three stdlib-only Python modules — `urllib.request` + `hashlib` + `plistlib` + `json` + `subprocess` only), `README.md`. Plus one new host-side unit test at `tests/unit/test_orionx_imager.sh` (9 assertions covering file existence + executable bits + Python syntax + bash syntax + shellcheck + `--help` + `--list-devices` + refuse-list guard + dry-run). **Layer A / Layer B split rationale (DEC-PHASE7-041 cascade-consolidation discipline applied):** the full RPi-Imager equivalent (Windows support, real progress bars, packaged installers via pyinstaller/py2app/AppImage/.msi, code-signing/notarization, auto-update, verification-after-write) is XL-plus and would cascade over multiple provisioning iterations. Layer A ships tonight on the two platforms where the developer + primary operator population lives (macOS + Linux), stdlib-only, no pip install, no venv — "git clone and run" installation model. Layer B is a small well-scoped follow-up (W11-12b, est. M-weight). **Design decisions embedded:** (1) Stdlib-only Python — no third-party deps ever, keeps supply-chain audit surface at zero for a defensive-product tool; macOS ships Python 3.9+ with tkinter and plistlib native; Debian requires one `sudo apt install python3-tk` preflight documented in README. (2) GitHub Releases REST API v3 (`api.github.com/repos/jarocki/orion/releases/latest` and `/releases/tags/<tag>`), no token required, 60-req/hr public rate limit is sufficient; asset picker matches `orionx-*.iso`. (3) SHA256 integrity manifest = the `SHA256SUMS` release asset emitted by `.github/workflows/release.yml` lines 108-114 (pre-existing authority, this tool READS only — does NOT fork the SHA256SUMS format). (4) SHA256 verification is MANDATORY by default; `--skip-verify` requires a TWO-flag opt-out (`--skip-verify --yes-really-skip-verify`); tkinter GUI does NOT expose skip-verify at all (defense-in-depth for the graphical user path). (5) Safety guards = refuse-list, NOT allow-list: macOS refuses `/dev/disk0`; Linux refuses `/dev/sda` AND anything mounted at `/` per `findmnt --target / --output SOURCE`; a two-word `--i-really-know-what-im-doing` flag is the only CLI bypass (GUI has no bypass path). (6) User confirmation of target is UNSKIPPABLE in interactive modes — CLI echo-confirmation (type target path back), GUI modal + type `ERASE` to confirm; `--force` bypasses interactive prompts on the CLI only, documented as "CI/testing use only". (7) Dry-run mode is the default first-invocation shape; the tool prints "would download <URL>, verify against <SHA256>, then run: `sudo dd if=... of=...`" without executing. (8) Elevation model = tool runs unprivileged, re-execs itself via `sudo -n` then `sudo` for the write step; Layer B may add pkexec on Linux + SMJobBless on macOS. (9) XDG cache for downloaded ISOs at `${XDG_CACHE_HOME:-$HOME/.cache}/orionx-imager/orionx-<tag>.iso` + `.sha256`; re-download of same tag hits cache first. **Explicitly rejected alternatives (documented so future implementers do not re-propose them for this Layer A scope):** third-party libs (`requests`/`tqdm`/`PySide6`) — REJECTED (supply-chain audit surface); Etcher fork / Electron bundling — REJECTED (~200 MB per platform, wrong distribution model); `pv`-piped progress — REJECTED for Layer A (not universal on macOS; deferred to Layer B); BitTorrent / IPFS distribution — REJECTED for W11-12 (GitHub Releases is the current release authority); building a "full Orion-X Setup GUI" (Wi-Fi config, first-boot options) — REJECTED (that is the ISO's Phase 6 first-boot wizard's job; the imager writes the ISO, it does not configure the OS); cross-platform Electron / Tauri GUI — REJECTED for Layer A (same Etcher problem). **State-authority preservation:** `.github/workflows/release.yml` (canonical `SHA256SUMS` emitter) UNTOUCHED and forbidden-path in Scope Manifest; DEC-PHASE7-002 single version authority `ORIONX_VERSION` UNTOUCHED (tool reads release tags via GitHub API, does not fork a version authority); all in-ISO authorities (build-iso.sh, hooks, package lists, Control Center, Nebula, branding pipeline) UNTOUCHED (host-side tool, no ISO content changes); all Phase 11 in-ISO decisions (DEC-PHASE11-001..-014) UNTOUCHED. **Rollback boundary:** `git revert` the branch merge — pre-W11-12 develop has NO `scripts/orionx-imager/` directory; the revert is atomic; there is no CI-published-artifact surface, no runtime state, no ISO-build coupling. Rollback risk: low. **Follow-up seeded (NOT this slice) — W11-12b Layer B:** Windows (PowerShell writer OR WSL2 bridge OR native Python packaging), real progress bar (pv-piping OR `/sys/block/<dev>/stat` polling OR SIGINFO/SIGUSR1 tap on `dd`), packaged installers (pyinstaller `.app` + AppImage + `.msi`), code-signing/notarization, auto-update, verification-after-write (SHA256 of first N MB of written USB), optional icon + `.desktop` entry, possibly a Control Center Awareness pane menu entry inside the ISO for "prepare another USB stick" workflows. Tracker filed at implementer landing time. |
